...
public class SdaiEdit extends JTabbedPane {
  ...
  String names[] = { "Main", "Repository", "SchemaInstance", "Model", "EntityExtent", "EntityInstance", "Aggregate", "ExtentMapping", "InstanceMapping",
      "AggregateMapping", "Properties", "About", "Temporary" };
...
//About
     ...
     * if (ii < 0) {
     * JOptionPane.showMessageDialog(this, "Evaluation version. Expires in "+((-1)*ii)+" days.", "SdaiEdit",
  ...
  protected void exit() {
    try {
      ...
      if (ses.isModified()) {
        ...
        int result = JOptionPane.showConfirmDialog(frame, "Do you want to commit changes?", "The data was changed.", JOptionPane.YES_NO_CANCEL_OPTION);
        switch (result) {
  ...
  private boolean loadCustomTheme(String path2File) {
    ...
    try {
      ...
      if (skin != null) {
        ...
        JOptionPane.showMessageDialog(frame, "Failed to load new Look & Feel from specified file (" + path2File + ")!", theTitle,
      ...
      ex.printStackTrace();
      JOptionPane
          .showMessageDialog(frame, "Failed to load new Look & Feel from specified file (" + path2File + ")!", theTitle, JOptionPane.ERROR_MESSAGE);
      status = false;
    }
    catch (UnsupportedLookAndFeelException ex) {
      ex.printStackTrace();
      JOptionPane.showMessageDialog(frame, "Failed to load new Look & Feel from specified file (" + path2File + "): unsupported type of Look & Feel!",
          theTitle, JOptionPane.ERROR_MESSAGE);
      status = false;
    }
      ...
      JOptionPane.showMessageDialog(frame, "Failed to load new Look & Feel from specified file(" + path2File + ")! Maybe it's not a Look & Feel file?",
          theTitle, JOptionPane.ERROR_MESSAGE);
...
//			bCopy.setToolTipText("Copies all contents of current page");
  ...
  class ThemeActionListener implements ActionListener {
    public void actionPerformed(ActionEvent aEvent) {
        ...
        if (status == JFileChooser.APPROVE_OPTION) {
          ...
          try {
            if (loadCustomTheme(selection.getCanonicalPath())) {
              ...
              JOptionPane.showMessageDialog(frame, "A new Look & Feel has been applied. However, it's strongly recommended to restart\n"
                  + "the program to ensure all components are properly updated.", theTitle, JOptionPane.INFORMATION_MESSAGE);
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(frame, "Failed to load new Look & Feel because of file access problems.", theTitle,
                JOptionPane.ERROR_MESSAGE);
          }
        }
      ...
      try {
        MetalLookAndFeel.setCurrentTheme((MetalTheme) metalThemes.get(themeName));
...
//	}
  ...
  private ChainElement getChain(SdaiPanel panel) throws SdaiException {
    ...
    else if (panel instanceof MapEditEntitiesPanel || panel instanceof MapEditEntMappingsPanel || panel instanceof EntityExtentBean
        || panel instanceof BaseEntityBean || panel instanceof AggregateBean) {
  ...
  private void setValue(ChainElement element) throws SdaiException {
    ...
    if (element.panel instanceof SdaiSessionBean) {
      ...
      ((ExtentMappingBean) element.panel).setModels_and_definition((ASdaiModel) element.values.elementAt(0),
          (EEntity_mapping) element.values.elementAt(1));
    }
...
//					System.out.println("MOUSE");
  ...
  void setProperties(Properties props) {
    ...
    frame.setSize((width != null) ? Integer.parseInt(width) : 600, (height != null) ? Integer.parseInt(height) : 600);
  ...
  Properties getProperties(Properties props) {
    ...
    if (size.width <= screenSize.width || size.height <= screenSize.height || location.x >= 0 || location.y >= 0) {
    ...
    public void goPerformed(GoEvent e) {
      try {
        ...
        if (target.equals("Repository")) {
          ...
          setSelectedIndex(indexOfTab(names[2]));
          if (mapEditEntitiesPanel != null && mapEditEntitiesPanel.setSchemaInstance((SchemaInstance) value)) {
          ...
          setSelectedIndex(indexOfTab(names[3]));
          doRefreshData = true;
          if (mapEditEntitiesPanel != null && mapEditEntitiesPanel.setModel((SdaiModel) value)) {
            enshurePageEnabled(mapEditEntitiesPanel);
          }
        }
          ...
          aggregateMapPage.setMappingAndInstancesAndDomain(em,
              ((s[0] instanceof AEntity) ? (AEntity) s[0] : SimpleOperations.arrayToAEntity(((Collection) s[0]).toArray())), (ASdaiModel) s[2]);
          setSelectedIndex(indexOfTab(names[9]));
