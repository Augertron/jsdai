...
public class SelectPaths {
  ...
  void generateConstants(PrintWriter pw, String name) throws SdaiException {
    int i = 0;
    int i2 = 2;
    String el_str;
    ...
    for (int si = 0; si < paths.size(); si++) {
      ...
      String str_path = "\tint s";
      if (!name.equals("")) {
        str_path += normalize(name);
      }
      int node_count = 1;
...
//case 11      if ((express_type == XT_ENTITY) && (path.size() == 0)) {
      if (express_type == XT_ENTITY) {
        ...
        for (int sj = 0; sj < path.size(); sj++) {
          ...
          String str_node = ((ENamed_type) path.elementAt(sj)).getName(null);
  ...
  void generateDefinedTypeGetMethodDeclarations(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix) throws SdaiException {
    int i = 0;
    int i2 = 2;
  ...
  void generateGetMethodDeclarations(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type)
...
//case 11      if ((express_type == XT_ENTITY) && (path_string.size() == 0)) {
  ...
  void generateSetMethodDeclarations(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type)
      throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//case 11      if ((express_type == XT_ENTITY) && (path_string.size() == 0)) {
  ...
  void generateAggregateConstants(PrintWriter pw) throws SdaiException {
    int i = 0;
    int i2 = 2;
    String el_str;
  ...
  void generateAggregateMethods(PrintWriter pw) throws SdaiException {
  }
...
//case 11 - sort of      if ((express_type == XT_ENTITY) && (path_string.size() == 0)) {
  ...
  void generateGetMethods(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type, String owning_entity_name,
      String attr_internal_name) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//O            pw.println("\t\treturn (EEntity)" + attr_internal_name + ";");
  ...
  // methods for select attributes in entity classes
  void generateGetMethodsRenamed(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type,
      String owning_entity_name, String attr_internal_name, String method_suffix_prev, String owning_entity_name_prev) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//            pw.println("\t\treturn get_something_select(" + attr_internal_name + ", " + attr_internal_name + "$$, " + i2 + ");");
  ...
  void generateSetMethodsRenamed(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type,
      String owning_entity_name, String attr_internal_name, String method_suffix_prev, String owning_entity_name_prev) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//            pw.println("\t\t" + at_name + "$$ = " + (i2) + ";");
  ...
  void generateSetMethods(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type, String owning_entity_name,
      String attr_internal_name) throws SdaiException {
    int i = 0;
    int i2 = 2;
          ...
          case XT_SET:
            ...
            switch (aggr_base) {
              case AB_INSTANCE:
                ...
                //                pw.println("\t\t"+ at_name + " = create_aggregate_instances((AEntity)"+ at_name + ", " + at_name + "$, " +  return_package + return_type   + ".class, 0);");
                //                pw.println("\t\t"+ at_name + " = create_aggregate_class((AEntity)"+ at_name + ", " + at_name + "$, " +  return_type   + ".class, " + at_name + "$$ = " + i2 + ");");
                pw.println("\t\t" + at_name + " = create_aggregate_class(" + at_name + ", " + at_name + "$, " + return_type + ".class, " + at_name
                    + "$$ = " + i2 + ");");
                pw.println("\t\treturn (" + return_type + ")" + at_name + ";");
                break;
              ...
              case AB_ENUMERATION:
                ...
                //          pw.println("\t\t"+ at_name + " = create_aggregate_integer("+ at_name + ", " + at_name + "$, 0);");
                pw.println("\t\t" + at_name + " = create_aggregate_enumeration((A_enumeration)" + at_name + ", " + at_name + "$, " + at_name + "$$ = "
                    + i2 + ");");
                pw.println("\t\treturn (" + return_type + ")" + at_name + ";");
                break;
              ...
              default:
//                pw.println("\t\t" + at_name + " = create_aggregate_select((AEntity)" + at_name + ", " + at_name + "$, " + return_type + ".class, " + at_name + "$$ = " + i2 + ", 0);");
                pw.println("\t\t" + at_name + " = create_aggregate_select(" + at_name + ", " + at_name + "$, " + return_type + ".class, " + at_name
                    + "$$ = " + i2 + ", 0);");
                pw.println("\t\treturn (" + return_type + ")" + at_name + ";");
                break;
            } // switch - aggregate type
  ...
  // derived attributes
  void generateGetDerivedInterfaces(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type,
      String owning_entity_name) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//					str_path = "\tpublic " + return_type + " get" + normalize(name) + "(" + owning_entity_name + " type";
  ...
  void generateGetDerivedMethods(TheAttribute tattr, EAttribute attr, ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix,
      int select_type, String owning_entity_name, String attr_internal_name, A_string expression_java, boolean flag_expressions,
      EGeneric_schema_definition __sd, EEntity_definition __ed, SdaiModel a_model, JavaBackend jb) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//        pw.println("\t\treturn (EEntity) get" +method_suffix +"(null, _context).getInstance();" );
      ...
      /*
       * pw.println("\tpublic EEntity get" + normalize(name) + "(" + owning_entity_name +
       * " type, SdaiContext _context) throws SdaiException { // case 1");
       *
       * // if ((flag_expressions) & (expression_java != null)) {
       * if (flag_expressions) {
       * // pw.println("\t\t\tSdaiContext _context = SdaiSession.getSdaiContext();");
...
//					pw.println("\t\t\tthrow new SdaiException(SdaiException.FN_NAVL); // B");
    ...
    for (int si = 0; si < paths.size(); si++) {
      ...
      Vector path_string = (Vector) path_strings.elementAt(i);
      String return_type = (String) value_strings.elementAt(i);
      String aggregate_depth_number_string = getAggregateDepthNumberString(return_type);

      String internal_attr_parameter = "";

      if (aggregate_depth_number_string != "") {
        if (tattr != null) {
          internal_attr_parameter = ", d" + tattr.consolidated_derived_index + "$";
        }
      }
...
//								pw.println("\t\treturn (" +return_type +") get" +method_suffix +"(((" + owning_entity_name + ")null), _context).getInstanceAggregate(this);" );
                  pw.println("\t\treturn (" + return_type + ") get" + method_suffix + "(((" + owning_entity_name + ")null), _context).getInstance"
                      + aggregate_depth_number_string + "Aggregate(this" + internal_attr_parameter + ");");
                  break;
...
//								pw.println("\t\treturn (" +return_type +") get" +method_suffix +"(((" + owning_entity_name + ")null), _context).getIntegerAggregate(this);" );
                  pw.println("\t\treturn (" + return_type + ") get" + method_suffix + "(((" + owning_entity_name + ")null), _context).getInteger"
                      + aggregate_depth_number_string + "Aggregate(this);");
                  break;
...
//								pw.println("\t\treturn (" +return_type +") get" +method_suffix +"(((" + owning_entity_name + ")null), _context).getIntegerAggregate(this);" );
                  pw.println("\t\treturn (" + return_type + ") get" + method_suffix + "(((" + owning_entity_name + ")null), _context).getBoolean"
                      + aggregate_depth_number_string + "Aggregate(this);");
                  break;
                 ...
                 * pw.println("\t\treturn (" +return_type +") get" +method_suffix
...
//						pw.println("\t\t\t// attr: " + attr + ", __sd: " + __sd + ", __ed: " + __ed + ", a_model: " + a_model);
  ...
  void generateGetLaterRedeclaredMethods(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type,
      String owning_entity_name, String attr_internal_name) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//case11 - sort of      if ((express_type == XT_ENTITY) && (path_string.size() == 0)) {
  ...
  void generateSetLaterRedeclaredMethods(ESelect_type st, PrintWriter pw, String entity_name, String name, String method_suffix, int select_type,
      String owning_entity_name, String attr_internal_name) throws SdaiException {
    int i = 0;
    int i2 = 2;
...
//case 11 - sort of      if ((express_type == XT_ENTITY) && (path_string.size() == 0)) {
   ...
   * returns:
   *
   * for ACartesian_point - ""
   * for AaCartesian_point - "2"
   * for AaaCartesian_point - "3"
   *
   * implementation extended for any other number such as
   *
   * AaaaaCartesian_point - "5"
   *
   *
   * We need to count A and a letters:
   *
   * if the first is A, then check if the second is uppercase,
   * if uppercase - return ""
   * if lowercase -
   * if it is not a - then not an aggregate at all, also return ""
   * if it is a - add to the number and continue while next letters are also lowercase a letters, adding numbers.
   * The first uppercase letter ends the loop.
   * If the first letter is a lower case but not a - an internal error. Such a case is not possible.
   * If the string ends on a - then not an aggregate, return "";
   */
  String getAggregateDepthNumberString(String return_type) {

    int aggr_number = 1;
    if (return_type == null) {
      return "";
    }
    if (return_type.length() < 3) {
      return "";
    }
    char c = return_type.charAt(0);
    if (c != 'A') {
      return "";
    }
    char c2 = return_type.charAt(1);
    if (Character.isUpperCase(c2)) {
      ...
      if (c2 == 'a') {
        // continue while a
        aggr_number++;
        for (int i = 2; i < return_type.length(); i++) {
          char c3 = return_type.charAt(i);
          if (c3 == 'a') {
            aggr_number++;
          }
