...
//System.out.println("=== END OF resolve ===");  
  ...
  /*
   * to minimize conflicts with other temp object usage, we remove ECtRenamed instances from temp after the new names are
   * resolved
   * and those instances are no longer needed.
   * this way, any usage of temp_object in later stages is not affected by ECtRenamed, and we convert back all the interfacing
   * flags
...
//		    EEntity_definition entities = models.getEntityExtentInstances(EEntity_definition.class);
  ...
  static void resolveAttributeRecursively(RenamedAttribute rattr0, EEntity_definition current, EEntity_definition start, HashSet entities, SdaiModel sml)
  ...
  static void resolveAttribute(RenamedAttribute rattr, SdaiModel sml) throws SdaiException {
    ...
    while (iter_attribute.next()) {
      ...
      if (rattr.new_entity == parent) {
        ...
        else if (current_attribute.getName(null).equals(rattr.new_name)) {
