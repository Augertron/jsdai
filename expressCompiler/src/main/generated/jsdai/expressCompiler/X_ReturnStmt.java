...
// %modified: 1016210367096 %

/* Generated By:JJTree: Do not edit this line. X_ReturnStmt.java */
...
public class X_ReturnStmt extends StatementNode {
  // public class X_ReturnStmt extends SimpleNode {
  SdaiModel current_model;

  public X_ReturnStmt(int id) {
    super(id);
  }

  public X_ReturnStmt(Compiler2 p, int id) {
    super(p, id);
  }
    ...
    if (children != null) {
      ...
      for (int i = 0; i < children.length; ++i) {
        ...
        if (jc != null) {
          ...
          // jc.generated_java = "return Value.alloc()." + jc.generated_java;
        }
      }

      // 			if(jc.domain_rule){
      // 					jc.generated_java = "return Value.alloc().set(_context, " + jc.generated_java +").getInteger()";
      // 			}
    }
  ...
  String getStaticTypeString(JavaClass jc) throws SdaiException {
    ...
    if (jc.in_function) {
      if (jc.keynode instanceof X_FunctionDecl) {
        pt = ((X_FunctionDecl) jc.keynode).getReturnTypeParameter();
      }
      else {
// occuring with xim-ida library, see if it is correct - on ProcedureDecl
//				System.out.println("X_ReturnStmt - not function: " + jc.keynode);
      }
    }
    ...
    if (jc.in_function) {
      ...
      if (pt != null) { // protection agains exception which occurs while implementing nested functions
        if (pt.testType_labels(null)) {
          labels = pt.getType_labels(null);
        ...
        // return "__X_ReturnStmt_pt_is_NULL_possibly_keynode_not_X_FunctionDecl__";
      }// pt != null
    }

// System.out.println("XRTX" + rt);

    // Why it was here??? -- mutiple RETURNs possible
    //     if (rt == null) {
    //       return "_ERROR_";
    //     } else {
    //       jc.return_type = null;
...
// if generic aggregate types are returned by a function, and the function is used in derived attribute, then
// the cast in the get method of the derived attribute cannot work
...
//			}else
    ...
    // Entity_definition and Defined_type types already returned!
