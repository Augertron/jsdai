...
// pw.println("\t//---partition: " + m_p_name);
                  ...
                  pw.println(
                      "\tprotected static Value callTarget_" + tp2_name + partition_suffix + "(SdaiContext _context, Value input) throws SdaiException {");
//			pw.println("\tprotected static Value callTarget_" + tp_name + "(SdaiContext _context, Value input) {");
                  pw.println("\t\tValue result;");
...
//						jc.first_pass = false;
                    ...
                    if (children[i] instanceof X_EntityInstantiationLoop) {
                      ...
                      if (mp_from_last_binding_header == mp) {
                        ...
                        for (int si = 0; si < src_par_names.size(); si++) {
                          pw.println("\t\t\tsrc_par_id_" + (String) src_par_names.get(si) + " = ((Integer)src_id_map.get(\"" + (String) src_par_names.get(si)
                              + "\")).intValue();");
                          pw.println("\t\t\t" + (String) src_par_names.get(si) + " = (Value)((Vector)binding_inst_extent.elementAt(j)).elementAt(src_par_id_"
                              + (String) src_par_names.get(si) + ");");
                        } // for
...
//already			String add_instances="";
                ...
                while (spi.next()) {
                  ...
                  if (hasIdentify_by()) {
                    ...
                    tabByIndex(ind, pw);
                    pw.println(
                        "\tValue " + sp_name + " = Value.alloc(" + csrc_type + ".definition).set(_context, a" + ind + "_inst.getByIndex(i" + ind + "));");
                  }
                  else {
                    tabByIndex(ind, pw);
                    ...
                    while (tpi2.next()) {
                      ...
                      if (jc.identified_by == false) {
                        ...
                        if (!targets.contains(plain_target_name)) {
                          ...
                          pw.println(
                              "\t\t\t" + etar_type2 + " " + plain_target_name + "_inst = (" + etar_type2 + ")tar_model.createEntityInstance(" + etar_type2
                                  + ".class);");
                          pw.println("\t\t\tValue " + plain_target_name + " = Value.alloc(" + ctar_type2 + ".definition).set(_context, " + plain_target_name
                              + "_inst);");
