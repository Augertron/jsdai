...
//					pw.println("<1> TargetParameterRef: " + tar_par_name + ", generated: " + jc.generated_java);
        ...
        else if (children[l] instanceof X_AttributeQualifier) {
...
//							pw.println("<5> index on RIGHT side: " + jc.generated_java);
          ...
          else if (children[l] instanceof X_Expression) {
...
*/
    ...
    if (jc.identified_by) {
      ...
      for (int si = 0; si < src_par_names.size(); si++) {
        pw.println(
            "\t\t\t\t\tsrc_par_id_" + (String) src_par_names.get(si) + " = ((Integer)src_id_map.get(\"" + (String) src_par_names.get(si) + "\")).intValue();");
        pw.println("\t\t\t\t\tValue " + (String) src_par_names.get(si) + " = (Value)((Vector)binding_inst_extent.elementAt(inst_num)).elementAt(src_par_id_"
...
//			pw.println("\t\t\t\t\t" + left_str +".set(_context, " +generated_expression +");");
      ...
      pw.println("\t\t\t\t\t}else");
      pw.println("\t\t\t\t\t\tif(j>0){ ");
      pw.println("\t\t\t\t\t\t\tif(Value.alloc(ExpressTypes.BOOLEAN_TYPE).nequal(_context, expr_" + tar_par_name + ", last_expr_" + tar_par_name
          + ").getAsBoolean())");
      pw.println("\t\t\t\t\t\t\t\tindeterminate_" + tar_par_name + " = true;");
      pw.println("\t\t\t\t\t\t}");
      pw.println("\t\t\t\t}");
      ...
      for (int si = 0; si < src_par_names.size(); si++) {
        pw.println(
            "\t\t\tsrc_par_id_" + (String) src_par_names.get(si) + " = ((Integer)src_id_map.get(\"" + (String) src_par_names.get(si) + "\")).intValue();");
        pw.println(
            "\t\t\t" + (String) src_par_names.get(si) + " = (Value)((Vector)binding_inst_extent.elementAt(j)).elementAt(src_par_id_" + (String) src_par_names
                .get(si) + ");");
      }
