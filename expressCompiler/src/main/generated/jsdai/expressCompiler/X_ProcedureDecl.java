...
// %modified: 1016210368718 %

/* Generated By:JJTree: Do not edit this line. X_ProcedureDecl.java */
...
public class X_ProcedureDecl
  ...
  public X_ProcedureDecl(int id) {
    super(id);
  }

  public X_ProcedureDecl(Compiler2 p, int id) {
    super(p, id);
  }
  ...
  String constructProcedureClassName() throws SdaiException {
    return constructFunctionProcedureRuleClassName(pd);
  }

  String constructProcedureClassName_O() throws SdaiException {
    String result = "";
    EProcedure_definition fd2 = null;
    EProcedure_definition fd3 = null;
    String f_name = null;

    fd2 = pd;

    if (pd == null) {
      result = "_NULL_procedure_definition_";
      return result;
    }
    ...
    for (; ; ) {
      fd3 = Support.getParentProcedureDefinition(fd2);
      if (fd3 == null) {
        result = "P" + result;
        break;
      }
  ...
  String constructFunctionProcedureRuleClassName(EProcedure_definition fd4) throws SdaiException {
    String result = "";
//		EFunction_definition fd2 = null;
//		EFunction_definition fd3 = null;
    EEntity fd5 = null;
...
//				result = "F" + result;
  ...
  String getParameters() throws SdaiException {
    ...
    String p_name;
    EEntity pt = null;
    AParameter lp = pd.getParameters(null);
    ...
    while (ip.next()) {
      ...
      if (pt instanceof EEntity_definition) {
        ...
        // not supported
        System.out.println("XP - ProcedureDecl: not supported parameter type, EEntity used: " + pt);
  ...
  String getDefinedTypeUnderlyingTypeString(EDefined_type rt1)
    ...
    if (domain_set) {
      ...
      System.out.println("XP: In ProcedureDecl, defined type domain unset:" + rt1);
    }

    if (rt == null) {
      type = "/* underlying type of defined type is null */ EEntity";
      System.out.println("XP: ProcedureDecl: underlying type of defined type is null: " + rt1);
    }
  ...
  String getAggregationTypeBaseTypeString(EAggregation_type rt1)
    ...
    if (element_type_set) {
      ...
      System.out.println("XP: In ProcedureDecl, aggregate element type value unset:" + rt1);
    }
    ...
    for (; ; ) {
      ...
      if (an_ss instanceof EDefined_type) {
        ...
        if (domain_set) {
          ...
          System.out.println("XP: In ProcedureDecl, in aggregate nested defined type domain unset:" +
              an_ss);
        }
      }
        ...
        if (element_type_set) {
          ...
          System.out.println("XP: In ProcedureDecl, nested aggregate element type value unset:" +
              an_ss);
        }
      }
    ...
    if (rt == null) {
      ...
      System.out.println("XP: ProcedureDecl: base type of aggregation type is null: " + rt1);
    }
    else if (rt instanceof EAggregation_type) {
      type = getAggregationTypeBaseTypeString((EAggregation_type) rt);
    }
      ...
      System.out.println("XP: Procedure Declaration, not yet supported element type in aggregate: " + rt);
  ...
  void makeNonVar(JavaClass jc) throws SdaiException {
    ...
    String proc_name = pd.getName(null);
    String p_name;
    String new_name;
    ...
    while (ip.next()) {
      ...
      // we need to handle labels, here seems a good place
      // if the parameter has a label, and if it is the first occurence of this label, generate regular code
      // if the parameter has no label - generate regular code
      // if the parameter has a label and this particular label occurs not for the first time,
      // then generate Value.alloc(parameter_with_the_first_occurence_of_the_label.getActualType())
      // ho to do it? perhaps save the first occurence of parameter in a hash table with the key - the label
      // then, when a parameter with a label occurs, attempt to find it in the table, if found, get the first occurence from there,
      // if not, put it in the table, it is the first occurence

      //	  jc.pw.println("TYPE_NAME AFTER CRAP:" +static_type_string);
      if (proc_name.equalsIgnoreCase("print")) {
//				variable_string = "\t\tSystem.out.print(" + p_name + ");";

        variable_string = "\t\tif (_e_par.getActualJavaType() == 24) {\n";
        variable_string += "\t\t\tSystem.out.print(\" \" + _e_par.getString());\n";
        variable_string += "\t\t} else\n";
        variable_string += "\t\tif (_e_par.getActualJavaType() == 22) {\n";
        variable_string += "\t\t\tSystem.out.print(\" \" + _e_par.getInteger());\n";
        variable_string += "\t\t} else {\n";
        variable_string += "\t\t\tSystem.out.print(\" \" + _e_par);\n";
        variable_string += "\t\t}\n";

        jc.parameter_initializations_str = variable_string;

      }
      else if (proc_name.equalsIgnoreCase("println")) {
//				variable_string = "\t\tSystem.out.println(" + p_name + ");";

        variable_string = "\t\tif (_e_par.getActualJavaType() == 24) {\n";
        variable_string += "\t\t\tSystem.out.println(\" \" + _e_par.getString());\n";
        variable_string += "\t\t} else\n";
        variable_string += "\t\tif (_e_par.getActualJavaType() == 22) {\n";
        variable_string += "\t\t\tSystem.out.println(\" \" + _e_par.getInteger());\n";
        variable_string += "\t\t} else {\n";
        variable_string += "\t\t\tSystem.out.println(\" \" + _e_par);\n";
        variable_string += "\t\t}\n";
...
//      jc.pw.println(variable_string);
  ...
  void makeVar(JavaClass jc) throws SdaiException {
    // makes VAR parameters VAR.
		/*
				First, in makeNonVar(), temporary variable is generated for each parameter and its value assigned to it.
			  That makesz all the parameters non VAR. It is used in functions as well.
			  Here, those parameters that are VAR are assigned the temporary variable value back, so it is returned.
		*/
    String p_name;
    String new_name;
    String name_prefix = "_nonvar_";
    ...
    jc.var_parameter_post_initializations_str = "\t\t// restoring VAR parameters\n";

    while (ip.next()) {
      EParameter par = (EParameter) lp.getCurrentMemberObject(ip);
      boolean var_present = false;
      if (par.testVar_type(null)) {
        var_present = par.getVar_type(null);
      }
      if (!var_present) {
        continue;
      }
...
//      jc.pw.println(variable_string);
      jc.var_parameter_post_initializations_str += variable_string + "\n";
    }
  }

  String makeVarStr(JavaClass jc, String indent_str) throws SdaiException {
    // makes VAR parameters VAR.
		/*
				First, in makeNonVar(), temporary variable is generated for each parameter and its value assigned to it.
			  That makesz all the parameters non VAR. It is used in functions as well.
