...
//###-01 jc.generated_java: (new jsdai.SExtended_dictionary_schema.FGet_entity_definition()).run(_context, Value.alloc(jsdai.SExtended_dictionary_schema.CEntity_or_view_definition.definition).set(_context, get(jsdai.SExtended_dictionary_schema.CAttribute.attributeParent(null))))
  ...
  //going through all the attributes: #1655=EXPLICIT_ATTRIBUTE('domain',#1653,0,#1506,$,.F.);
  ...
  //going through all the attributes: #1656=EXPLICIT_ATTRIBUTE('redeclaring',#1653,1,#1653,$,.T.);
  //<01> generating methods for consolidated attribute:  redeclaring
  //<01-0> current entity
  //<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
  // attribute (current explicit or supertype explicit) : redeclaring, base type: entity explicit_attribute
  ...
  public EExplicit_attribute getRedeclaring(EExplicit_attribute type) throws SdaiException {
    return (EExplicit_attribute) get_instance(a4);
  }
  ...
  //going through all the attributes: #1657=EXPLICIT_ATTRIBUTE('optional_flag',#1653,2,#6,$,.F.);
