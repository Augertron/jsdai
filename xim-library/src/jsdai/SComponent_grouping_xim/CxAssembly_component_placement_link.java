...
import jsdai.SComponent_grouping_mim.CAssembly_group_component_definition_placement_link;
import jsdai.SComponent_grouping_mim.ELinear_array_placement_group_component;
import jsdai.SProduct_definition_schema.*;

public class CxAssembly_component_placement_link extends CAssembly_component_placement_link implements EMappedXIMEntity{
...
/*	public static int usedinFrame_of_reference(EProduct_definition type, jsdai.SApplication_context_schema.EProduct_definition_context instance, ASdaiModel domain, AEntity result) throws SdaiException {
	...
	public void createAimData(SdaiContext context) throws SdaiException {
		...
		setTemp("AIM", CAssembly_group_component_definition_placement_link.definition);
	...
	public static void setMappingConstraints(SdaiContext context,
			EAssembly_component_placement_link armEntity) throws SdaiException {
		unsetMappingConstraints(context, armEntity);
		// AIM gaps
		ALinear_array_placement_group_component_armx users = armEntity.getReferenced_by(null, null);
		...
		if(users.getMemberCount() != 1){
			...
			for(int i=1,count=apdr.getMemberCount(); i<=count; i++){
				...
				if(epdr.testName(null) && epdr.getName(null).equals("element")){
					user = (ELinear_array_placement_group_component)epdr.getRelating_product_definition(null);
					break;
				}
			}
			if(user == null){
			...
			if(user == null){
				...
				ELinear_array_placement_group_component_armx erapgca = (ELinear_array_placement_group_component_armx)
					user;
