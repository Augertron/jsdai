...
public class CxLayered_interconnect_module_design_view_armx
		extends
		CLayered_interconnect_module_design_view_armx implements EMappedXIMEntity {
...
/*	public boolean testName(jsdai.SProduct_property_definition_schema.EProperty_definition type) throws SdaiException {
	...
	public void createAimData(SdaiContext context) throws SdaiException {
		...
		setTemp("AIM", CLayered_interconnect_module_design_view.definition);
	...
	*/
	...
	public static void setUsed_stack_model(SdaiContext context,
			ELayered_interconnect_module_design_view_armx armEntity) throws SdaiException {
		unsetUsed_stack_model(context, armEntity);
		if(armEntity.testUsed_stack_model(null)){
			EDesign_stack_model_armx stackModel = armEntity.getUsed_stack_model(null);
			EProperty_definition_relationship epdr = (EProperty_definition_relationship)
				context.working_model.createEntityInstance(
					CProperty_definition_relationship.definition);
			epdr.setRelated_property_definition(null, armEntity);
			epdr.setName(null, "used stack model");
	...
	public static void unsetUsed_stack_model(SdaiContext context,
		...
		CProperty_definition_relationship.usedinRelated_property_definition(null, 
				armEntity, context.domain, apdr);
		for(int i=1,count=apdr.getMemberCount();i<=count;i++){
			EProperty_definition_relationship epdr = apdr.getByIndex(i);
			if((epdr.testName(null))&&(epdr.getName(null).equals("used stack model"))){
