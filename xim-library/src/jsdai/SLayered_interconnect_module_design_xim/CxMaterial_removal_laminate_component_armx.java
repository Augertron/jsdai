...
public class CxMaterial_removal_laminate_component_armx
		extends
			CMaterial_removal_laminate_component_armx implements EMappedXIMEntity{
...
*/
	...
	public static void setMappingConstraints(SdaiContext context,
			EMaterial_removal_laminate_component_armx armEntity) throws SdaiException {
...
///// Item_shape
	...
	*/
	...
	{
		//unset old values
		unsetDesign_intent(context, armEntity);

		if (armEntity.testDesign_intent(null)){
			AStratum_feature_template_component_or_stratum armAttributes = armEntity.getDesign_intent(null);
			SdaiIterator iter = armAttributes.createIterator();
			while(iter.next()){
				EEntity intent = armAttributes.getCurrentMember(iter);
				if(intent instanceof EStratum_feature_template_component){
					// SA
					LangUtils.Attribute_and_value_structure[] saStructure =
		            {new LangUtils.Attribute_and_value_structure(
		            		CShape_aspect.attributeOf_shape(null),
					...
					// SA2
					LangUtils.Attribute_and_value_structure[] sa2Structure =
		            {new LangUtils.Attribute_and_value_structure(
		            		CShape_aspect.attributeOf_shape(null),
								intent)};
					...
					EShape_aspect_relationship shape_aspect_relationship = (EShape_aspect_relationship) 
						context.working_model.createEntityInstance(CShape_aspect_relationship.definition);
						shape_aspect_relationship.setName(null, "design intent");
						shape_aspect_relationship.setDescription(null, "");
						shape_aspect_relationship.setRelating_shape_aspect(null, esa2);
						shape_aspect_relationship.setRelated_shape_aspect(null, esa);
				}else{
					//pdr
					...
					pdr.setName(null, "design intent");
					pdr.setDescription(null, "");
					pdr.setRelating_property_definition(null, (EProperty_definition)intent);
	...
	*/
	public static void unsetDesign_intent(SdaiContext context, EMaterial_removal_laminate_component_armx armEntity) throws SdaiException
	{
		...
		for (int j = 1; j <= asa.getMemberCount(); j++) {
			...
			for (int i = 1; i <= aRelationship.getMemberCount(); i++) {
				relationship = aRelationship.getByIndex(i);
				if (relationship.testName(null) && relationship.getName(null).equals("design intent")) {
					relationship.deleteApplicationInstance();
				}
			}
		}
	...
	*/
	public static void setExplicitly_created(SdaiContext context, EMaterial_removal_laminate_component_armx armEntity) throws SdaiException
	{
		...
		if (armEntity.testExplicitly_created(null)){
			boolean armAttribute = armEntity.getExplicitly_created(null);
			EProperty_definition pd = (EProperty_definition) 
				context.working_model.createEntityInstance(CProperty_definition.definition);
			pd.setName(null, "explicitly created");
			if(armAttribute){
				pd.setDescription(null, "true");
			}else{
				pd.setDescription(null, "false");
			}
			pd.setDefinition(null, armEntity);
		}
	}
	...
	*/
	public static void unsetExplicitly_created(SdaiContext context, EMaterial_removal_laminate_component_armx armEntity) throws SdaiException
	{
		...
		for (int i = 1, count=apd.getMemberCount(); i <= count; i++) {
			epd = apd.getByIndex(i);
			if (epd.testName(null) && epd.getName(null).equals("explicitly created")) {
