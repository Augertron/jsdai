...
import jsdai.SComponent_grouping_xim.*;
...
import jsdai.SPresentation_definition_schema.EText_literal_with_extent;
...
public class CxLaminate_text_string_component_armx
		extends
			CLaminate_text_string_component_armx implements EMappedXIMEntity{
...
//			setId_x(context, this);
			...
			// Message_contents
			setMessage_contents(context, this);
...
//			unsetId_x(context, this);			
			...
			// of_horizontal_justification
         unsetMessage_contents(context, this);
	...
	public static void setMappingConstraints(SdaiContext context,
			ELaminate_text_string_component_armx armEntity) throws SdaiException {
...
*/
	/**
	* Sets/creates data for Message_contents attribute.
	*
	* <p>
	*  attribute_mapping Message_contents (message_contents
	...
	*/
	// APGCD <- PDR -> R -> TLWE 
	public static void setMessage_contents(SdaiContext context, ELaminate_text_string_component_armx armEntity) throws SdaiException
	{
		//unset old values
		unsetMessage_contents(context, armEntity);

		if (armEntity.testMessage_contents(null))
		{

			EText_literal_with_extent message = armEntity.getMessage_contents(null);
			LangUtils.Attribute_and_value_structure[] repStructure = {new LangUtils.Attribute_and_value_structure(
					CRepresentation.attributeItems(null), message)};
			ERepresentation rep = (ERepresentation)
				LangUtils.createInstanceIfNeeded(context, CRepresentation.definition,
					repStructure);
			if(!rep.testName(null)){
				rep.setName(null, "");
			}
			...
			epd.setDescription(null, "message contents");
	...
	/**
	* Unsets/deletes data for Message_contents attribute.
	*
	* @param context SdaiContext.
	* @param armEntity arm entity.
	* @throws SdaiException
	*/
	public static void unsetMessage_contents(SdaiContext context, ELaminate_text_string_component_armx armEntity) throws SdaiException
	{
		AProperty_definition apd = new AProperty_definition();
		CProperty_definition.usedinDefinition(null, armEntity, context.domain, apd);
		for(int i=1;i <= apd.getMemberCount();){
			EProperty_definition epd = apd.getByIndex(i);
			if((epd.testName(null))&&(epd.getName(null).equals("message contents"))){
				...
				for(int j=1;j <= apd.getMemberCount();){
