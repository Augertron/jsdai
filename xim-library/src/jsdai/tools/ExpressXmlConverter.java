...
*/
	...
	public static void convertSchemaInstance(SchemaInstance sch_inst, String file_name) throws SdaiException {
		...
		if (model != null) {
			...
			convertModel(model, session, repo, work, file_name);
		} else {
	...
	private static void convertModel(SdaiModel model, SdaiSession session, SdaiRepository repo, 
			SdaiModel work, String file_name) throws SdaiException {
		int i;
...
//		String file_name = schema_name + FILE_EXTENSION;
		try {
			Utils.saveToFile(file_name, doc, Utils.EXPRESS_XML_DOCTYPE_VALUE);
		} catch (IOException e) {
			e.printStackTrace();
