...
import jsdai.SMapping_schema.CInverse_attribute_constraint;
import jsdai.SMapping_schema.CPath_constraint;
...
import jsdai.mappingCompiler.util.MappingUtils;
 ...
 *
 * application_context = description_attribute_select
...
/**
 * Searched for mappings that ends on special derived attributes (id, name, description, role)
 * and replaces the derived attribute by the explicit mapping path.
 */
public class ReplaceUsageOfDerivedAttributes {
  ...
  static final String change[][] = {
      // attribute-name, entity-name, special IR-entity for additional string, name of target attribute, name of value attribute
    ...
    while (iterator.next()) {
      ...
      if (instance instanceof EInverse_attribute_constraint) {
        ...
        if (((EConstraint_relationship) instance).testElement2(null)) {
          if (instance instanceof EInstance_constraint && !((EInstance_constraint) instance).testElement1(null)) {
          ...
          if (attribute instanceof EDerived_attribute) {
            replaceAttributeInConstraintRelationship((EDerived_attribute) attribute, mappingModel, (EConstraint_relationship) instance,
                aimEntities, true);
          }
          if (attribute2 instanceof EDerived_attribute) {
            replaceAttributeInConstraintRelationship((EDerived_attribute) attribute2, mappingModel, (EConstraint_relationship) instance,
                aimEntities, false);
          }
        }
        else {
...
// 										System.out.println(instance);
                ...
                if (!f) {
                  System.out.println("The attribute " + ((EDerived_attribute) pathElem).getParent(null).getName(null) + "."
                      + ((EDerived_attribute) pathElem).getName(null) + " (" + pathElem + ") can not be replaced in " + attributeMapping);
                }
              }
            }
          }
...
// 								System.out.println(instance);
            ...
            boolean f = false;
            for (int i = 0; i < n; i++) {
              if (derivedAttribute.getName(null).equals(change[i][0])
                  && derivedAttribute.getParent(null).getName(null).equals(change[i][1])) {
                EEntity_definition ed = MappingUtils.findEntityDefinition(change[i][2], aimEntities);
                if (ed == null) {
                  notFoundAttributes.add(change[i][2]);
                  continue;
                }
                EInverse_attribute_constraint iac = (EInverse_attribute_constraint) mappingModel
                    .createEntityInstance(CInverse_attribute_constraint.class);
            ...
            if (!f) {
              System.out.println("The attribute " + derivedAttribute.getParent(null).getName(null) + "." + derivedAttribute.getName(null) + " ("
                  + derivedAttribute + ") can not be replaced in " + attributeMapping);
            }
          }
        }
      }
...
// 						System.out.println(instance);
  ...
  public static boolean replaceAttributeInPath(EDerived_attribute attribute, SdaiModel mappingModel, AEntity path, SdaiIterator position,
      EEntity_definition aimEntities[]) throws SdaiException {
    int n = change.length;
    boolean rv = false;
    for (int i = 0; i < n; i++) {
  ...
  public static EEntity replaceAttributeInStringConstraint(EDerived_attribute attribute, SdaiModel mappingModel, EString_constraint constraint,
      EEntity_definition aimEntities[]) throws SdaiException {
        ...
        while (j.next()) {
          ...
          if (user instanceof EEntity_mapping) {
            ...
            if (user instanceof EConstraint_relationship) {
              ...
              throw new SdaiException(SdaiException.SY_ERR, "Unexpected constraint in replaceAttributeInStringConstraint: " + user.toString());
            }
          }
        }
        users.clear();
  ...
  public static EEntity replaceAttributeInConstraintRelationship(EDerived_attribute attribute, SdaiModel mappingModel, EConstraint_relationship constraint,
      EEntity_definition aimEntities[], boolean element1) throws SdaiException {
    EEntity rv = constraint;
    int n = change.length;
    for (int i = 0; i < n; i++) {
      if (attribute.getName(null).equals(change[i][0]) && attribute.getParent(null).getName(null).equals(change[i][1])) {
          ...
          if (orderChanged) {
            if (user instanceof EEntity_mapping) {
              ...
              if (user instanceof EConstraint_relationship) {
                ...
                throw new SdaiException(SdaiException.SY_ERR, "Unexpected constraint in replaceAttributeInConstraintRelationship: "
                    + user.toString());
              }
            }
          }
