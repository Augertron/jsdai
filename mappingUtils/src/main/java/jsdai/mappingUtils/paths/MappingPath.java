...
/**
 * A tool to compile XIM to MIM mapping specification files (*.path)
 * into a representation of Express Entity instances based on the
 * JSDAI repository to be used MIM-to-XIM
 * ISO 10303 Application Protocols (AP) and Application Modules (AM) contain a mapping specification in clause 5.1
 * to map from an ARM model to a AIM/MIM model.
 *
...
public class MappingPath {
  ...
  public static void main(String args[]) throws FileNotFoundException, IOException, SdaiException {
    System.out.println("JSDAI(TM) MappingPath Compiler,   Copyright (C) 1998-2013 LKSoftWare GmbH");
    System.out.println("-------------------------------------------------------------------------");
    try {
      ...
      if (!showHelp && !trackChanges) {
        MappingPathParser parser = new MappingPathParser(pathFile, mappingInfoFile, reportMissing, missingSchemaMatchers, trackChangesFile);
