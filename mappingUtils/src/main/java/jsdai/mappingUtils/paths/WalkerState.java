...
//				e.printStackTrace();
//			}
      throw new MappingSemanticException(first != null ? first : mappedValue, "Too many mapped values: " + mappedValue + " and " + other.mappedValue);
    }
  }
...
//DEBUG		System.out.println("first " + first.toStringList() + " second " + second.toStringList());
    ...
    if (first instanceof EntityAST) {
      if (!(second instanceof EntityAST)) {
        return false;
      }
      if (!((EntityAST) first).isEqualTo((EntityAST) second)) {
        return false;
      }
    }
    else {
    ...
    for (firstChild = first.getFirstChild(), secondChild = second.getFirstChild(); firstChild != null && secondChild != null; firstChild = firstChild
        .getNextSibling(), secondChild = secondChild.getNextSibling()) {
      if (!equalsAST(firstChild, secondChild)) {
        return false;
      }
    }
  ...
  static public void removeDuplicates(MappingPathParser parser, List states) throws SdaiException, MappingSemanticException {
  ...
  public EntityAST getFirst(MappingPathParser parser) throws SdaiException, MappingSemanticException {
    if (firstList == null) {
      return first;
    }
  ...
  public void addFirstToList(EntityAST first) throws MappingSemanticException {
    if (first.declaration.type != DictionaryDeclaration.ENTITY) {
      throw new MappingSemanticException(first, "Entity definition required but defined type found: " + first.getText());
    }
    if (firstList == null) {
  ...
  public void addFirstToList(LinkedList otherFirstList) {
    if (firstList == null) {
      firstList = otherFirstList;
    }
    firstList.addAll(otherFirstList);
  }

  public String toString() {
    return "WalkerState(first: " + (firstList != null ? firstList.toString() : String.valueOf(first)) + " remaining : "
        + (remaining != null ? remaining.toStringTree() : "null") + ")";
  }

  public boolean equals(MappingPathParser parser, WalkerState otherState) throws SdaiException, MappingSemanticException {
    if (!getFirst(parser).isEqualTo(otherState.getFirst(parser))) {
      return false;
    }
    if (remaining != otherState.remaining && (remaining == null || otherState.remaining == null || !equalsAST(remaining, otherState.remaining))) {
      return false;
    }
    if (mappedValue != otherState.mappedValue && (mappedValue == null || otherState.mappedValue == null || !equalsAST(mappedValue, otherState.mappedValue))) {
      return false;
    }
    return true;
  }
