...
public class TemplateCallAST extends MappingAST {
  ...
  public void resolve(AST parent, MappingPathParser parser, SourceLocator sourceLocator) throws RecognitionException, SdaiException {
    if (!resolved) {
      resolved = true;
      String name = getName();
...
// 								   (inAimList ? " in AIM list" : " in reference path"));
        ...
        throw new SemanticException("Unresolved template call: " + name, sourceLocator.getFilename(), getLine());
      }
    }
  }

  private void resolveType(AST parent, MappingPathParser parser, SourceLocator sourceLocator) throws RecognitionException, SdaiException {
    ...
    if (origEntMapAst == null) {
      throw new SemanticException("Entity mapping not found in templace call /" + getName() + "(" + typeEntity.getText() + ")/",
    ...
    if (inAimList) {
      ...
      AST firstAst = null;
      if (realNextSibling != null && (realNextSiblingType = realNextSibling.getType()) != MappingPathParser.ATTRIBUTE_MAPPING
        ...
        while ((realNextSibling = firstAstEnd.getNextSibling()) != null
            && (realNextSiblingType = realNextSibling.getType()) != MappingPathParser.ATTRIBUTE_MAPPING
            && realNextSiblingType != MappingPathParser.ID) {
          firstAstEnd = realNextSibling;
        }
        firstAstEnd.setNextSibling(null);
      }
      lastInsertedSibling = resolveTypePath(parent, origEntMapAst, parser, firstAst, sourceLocator, MappingPathParser.OR, false);
    }
    else {
      lastInsertedSibling = resolveTypePath(this, origEntMapAst, parser, null, sourceLocator, MappingPathParser.OR, true);
    }
    lastInsertedSibling.setNextSibling(realNextSibling);
  }

  private AST resolveTypeAimList(AST lastInsertedSibling, EntityMappingAST origEntMapAst, MappingPathParser parser, SourceLocator sourceLocator)
  ...
  private AST resolveTypePath(AST insertPoint, EntityMappingAST origEntMapAst, MappingPathParser parser, AST firstAst, SourceLocator sourceLocator,
      int orConstructType, boolean mergeAimList) throws RecognitionException, SdaiException {
    ...
    while (origEntMapAst != null) {
      ...
      int pathType;
      if (path != null && (pathType = path.getType()) != MappingPathParser.ATTRIBUTE_MAPPING && pathType != MappingPathParser.ID) {
        AST pathDup = MappingPathWalker.dupAttachTree(path);
        AST origLocation = origEntMapAst.dupAttachTree();
        origLocation.setFirstChild(null);
        origLocation.setType(MappingPathParser.ORIGINAL_LOCATION);
        origLocation.setNextSibling(pathDup);
        while ((pathType = path.getType()) == MappingPathParser.TEMPLATE_CALL || pathType == MappingPathParser.ORIGINAL_LOCATION) {
        ...
        if (mergeAimList) {
          Collection toIncludeAim = mergePathWithAimList(path, aimAst.getFirstChild(), origEntMapAst, parser);
        ...
        for (AST a = aimAst.getFirstChild(); a != null; a = a.getNextSibling()) {
          int aType;
          if ((aType = a.getType()) != MappingPathParser.TEMPLATE_CALL && aType != MappingPathParser.ORIGINAL_LOCATION) {
      ...
      for (EntityMappingAST e = origEntMapAst.extended; e != null; e = e.extended) {
        AST newLastInstPoint = resolveTypePath(insertPoint, e, parser, firstAst, sourceLocator, MappingPathParser.OR_EXTENDED, mergeAimList);
...
// 	}
  ...
  private static Collection mergePathWithAimList(AST path, AST aim, EntityMappingAST origEntMapAst, MappingPathParser parser) throws RecognitionException,
      SdaiException {
    ...
    for (; aim != null; aim = aim.getNextSibling()) {
      int aimType;
      if ((aimType = aim.getType()) != MappingPathParser.TEMPLATE_CALL && aimType != MappingPathParser.ORIGINAL_LOCATION) {
        ...
        for (Iterator i = topList.iterator(); i.hasNext(); ) {
          ...
          if (member instanceof EntityAST) {
            if (member.getNextSibling() != null) {
              ...
              String complexName = EntityAST.makeComplexName(new AbstractCollection() {
  ...
  private void resolveAimComplex(AST parent, MappingPathParser parser, SourceLocator sourceLocator) throws RecognitionException, SdaiException {
    AST realNextSibling = getNextSibling();
    ...
    for (AST child = getFirstChild(); child != null; child = child.getNextSibling()) {
      ...
      if (childType == MappingPathParser.TEMPLATE_CALL) {
        ...
        if (!templateCall.resolved) {
          ...
          if (alts.isEmpty()) {
            for (AST newChild = child.getNextSibling(); newChild != tempCallNext; newChild = newChild.getNextSibling()) {
