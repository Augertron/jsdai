...
public class AttributeAST extends EntityAST {
  ...
  public String getText() {
    try {
      String tokenText = declaration != null && attribute != null ? declaration.definition.getName(null) + "." + attribute.getName(null) : "";
      if (inPath == IN_PATH) {
        tokenText = "(" + tokenText + ")";
      }
      return tokenText;
  ...
  public void setIdentifiers(MappingPathParser parser, EntityAST entity, Token id, boolean aim) throws SemanticException {
    String entityText;
    ...
    if (declaration != null && declaration.type == DictionaryDeclaration.ENTITY) {
      ...
      if (attribute == null) {
        parser.reportError(new SemanticException("Attribute " + entity.getText() + "." + id.getText() + " not found", parser.getFilename(), id
            .getLine()));
      }
    }
    else if ((entityText = entity.getText()).length() > 0) {
      parser.reportError(new SemanticException(entityText + " should be an entity for attribute " + id.getText(), parser.getFilename(), id.getLine()));
    }
    else {
      parser.reportError(new SemanticException("Attribute " + id.getText() + " not found because of unknown entity", parser.getFilename(), id.getLine()));
    }
  }

  public void setIdentifiers(MappingPathParser parser, Token id1, Token id2, boolean aim) throws SemanticException {
    setIdentifier(parser, id1, aim);
    if (declaration != null) {
      setIdentifiers(parser, this, id2, aim);
    }
  }
  ...
  private EEntity getMappedValueDomain(EEntity domain, EntityAST dataType) throws SdaiException {
    if (domain instanceof EAggregation_type) {
      ...
      EEntity typeDomain = getMappedValueDomain(definedType.getDomain(null), dataType);
      return typeDomain instanceof ESelect_type && dataType != null ? getSelectionMappedValueDomain(declaration.getSelections(definedType), dataType)
          : typeDomain;
    }
    else {
      return domain;
  ...
  private EEntity getSelectionMappedValueDomain(ANamed_type selections, EntityAST dataType) throws SdaiException {
