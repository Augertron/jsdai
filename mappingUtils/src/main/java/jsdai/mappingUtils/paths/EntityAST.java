...
import jsdai.SMapping_schema.AAttribute_mapping;
...
public class EntityAST extends BaseAST implements ParserAST {
  ...
  public static EntityAST construct(MappingPathParser parser, List nameList, boolean aim) throws MappingSemanticException, SdaiException {
    ...
    if (!newEntity.setIdentifierInternal(parser, newEntityName, aim)) {
      throw new MappingSemanticException(newEntity, "Complex entity " + newEntityName + " not found");
    }
    else {
      return newEntity;
    }
  ...
  public void setIdentifier(MappingPathParser parser, Token token, boolean aim) throws SemanticException {
  ...
  public void setIdentifier(MappingPathParser parser, String name, boolean aim) {
    if (!setIdentifierInternal(parser, name, aim)) {
      parser.reportError(new SemanticException("Identifier " + name + " not found", parser.getFilename(), line));
    }
  }

  private boolean setIdentifierInternal(MappingPathParser parser, String name, boolean aim) {
    declaration = (DictionaryDeclaration) (aim ? parser.aimDeclarations.get(name.toLowerCase()) : parser.armDeclarations.get(name.toLowerCase()));
  ...
  public boolean isSupertypeOf(EntityAST other) throws SdaiException {
    if (declaration.type != DictionaryDeclaration.ENTITY || other.declaration.type != DictionaryDeclaration.ENTITY) {
      return false;
    }
  ...
  private static boolean areCompatible(EEntity_definition entityOne, EEntity_definition entityTwo) throws SdaiException {
    ...
    while (entityOneIter.next()) {
      ...
      while (entityTwoIter.next()) {
        ...
        if (entityOne == entityTwo || isSupertypeOf(entityOne, entityTwo) || isSupertypeOf(entityTwo, entityOne)) {
          return true;
        }
      }
    }
  ...
  private static boolean isCloserThan(EEntity_definition testEntity, EEntity_definition entityOne, EEntity_definition entityTwo) throws SdaiException {
    if (entityOne == entityTwo) {
      return false;
    }
    if (testEntity == entityOne) {
      return true;
    }
    ...
    while (testEntityIter.next()) {
      ...
      if (testEntity != entityTwo) {
        if (isSupertypeOf(testEntity, entityTwo)) {
          if (isSupertypeOf(entityOne, entityTwo)) {
            return true;
          }
        }
        else {
          if (isSupertypeOf(entityTwo, entityOne)) {
            return true;
          }
        }
      }
  ...
  private boolean isSelectOf(EDefined_type thisType, ENamed_type otherType) throws SdaiException {
    ...
    while (selectionIterator.next()) {
      ...
      if (selection == otherType) {
        return true;
      }
      if (selection instanceof EDefined_type && isSelectOf((EDefined_type) selection, otherType)) {
        return true;
      }
    }
    return false;
  ...
  public boolean isExtensionOf(EntityAST other) throws SdaiException {
    if (declaration.type != DictionaryDeclaration.TYPE || other.declaration.type != DictionaryDeclaration.TYPE) {
      return false;
    }
    EDefined_type thisType = (EDefined_type) declaration.definition;
  ...
  private static boolean isExtensionOf(EDefined_type thisType, EDefined_type otherType) throws SdaiException {
  ...
  private static boolean isSimpleType(EDefined_type type) throws SdaiException {
    ...
    if (domain instanceof ESimple_type || domain instanceof EEnumeration_type || domain instanceof EAggregation_type) {
      return true;
    }
  ...
  private static boolean isSelectType(EDefined_type type) throws SdaiException {
    ...
    if (domain instanceof EDefined_type) {
      return isSelectType((EDefined_type) domain);
    }
    return false;
  }
  ...
  static public void processExtAttrMappings(AST path, AST last, Collection extAttrList, MappingPathWalker pathWalker) throws RecognitionException,
      MappingSemanticException, SdaiException {
...
// 										   "states: " + extAttribMapping.states);
            int pathWalkerLine = pathWalker.line >= 0 ? pathWalker.line : pathWalker.sourceLocator.getLine();
            StateLink stateLink = new StateLink(state.dup(), last instanceof ParserAST ? ((ParserAST) last).getLine() : pathWalkerLine);
            if (lastChild != null) {
  ...
  static public void attachEntityMappings(LinkedList pathStates, MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException,
      SdaiException {
        ...
        if (state.getFirst(pathWalker.parser).isSame(aimEntity)) {
          if (aimEntity.pathState != null) {
            if (aimEntity.isSame(aimEntity.pathState.getFirst(pathWalker.parser))) {
              pathWalker.reportError(null, "Duplicated entity paths at " + aimEntity.getText());
            }
            else {
    ...
    if (pathStates.size() != 0) {
      StringBuffer error = new StringBuffer("Some entity paths were not attached to entity mappings: ");
...
// 			pathWalker.reportError(null, "Some entity paths were not attached to entity mappings");
    ...
    while (aimEntityIter.hasNext()) {
      EntityAST aimEntity = (EntityAST) aimEntityIter.next();
      if (aimEntity.pathState == null) {
        pathWalker.reportWarning(null, "Warning: Mapping to " + aimEntity.getText() + " has no entity path");
      }
    }
  }

  static public void attachAttributeMappings(AttributeAST arm, EntityAST dataType, LinkedList aimList, LinkedList pathStates, MappingPathWalker pathWalker)
  ...
  private static void attachAimListAttributeMappings(AttributeAST arm, EntityAST dataType, LinkedList aimList, MappingPathWalker pathWalker)
      throws MappingSemanticException, RecognitionException, SdaiException {
    if (aimList == null) {
      throw new MappingSemanticException(arm, "AIM list can not be empty when mapping path is omitted");
    }
    if (aimList.size() > 0) {
      ...
      while (aimListIter.hasNext()) {
        ...
        if (!(aimObject instanceof AttributeAST)) {
          throw new MappingSemanticException((AST) aimObject, "Attribute expected but entity found");
        }
        AttributeAST aimAttribute = (AttributeAST) aimObject;
        Iterator aimEntityIter = pathWalker.aimEntityList.iterator();
        while (aimEntityIter.hasNext()) {
          EntityAST aimEntity = (EntityAST) aimEntityIter.next();
          EntityAST startEntity = AttributeMappingsForEntity.extractStartEntity(aimEntity, pathWalker.parser);
          if (startEntity.declaration.definition == aimAttribute.declaration.definition || aimAttribute.isSupertypeOf(startEntity)) {
            ...
            MappingForAttribute mappingForAttribute = new MappingForAttribute(arm, dataType, state);
      ...
      while (aimEntityIter.hasNext()) {
        EntityAST aimEntity = (EntityAST) aimEntityIter.next();
        MappingForAttribute mappingForAttribute = new MappingForAttribute(arm, dataType, null);
        if (aimEntity.attachedAttributeMappings == null) {
          aimEntity.attachedAttributeMappings = new LinkedList();
        }
        aimEntity.attachedAttributeMappings.add(mappingForAttribute);
  ...
  private static boolean attachAttributeOneState(AttributeAST arm, EntityAST dataType, LinkedList attributeMappingsForEntities,
      EntityAST attributeStartEntity, WalkerState state, MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException,
      SdaiException {
    ...
    while (attributeMappingsIter.hasNext()) {
      AttributeMappingsForEntity attributeMappings = (AttributeMappingsForEntity) attributeMappingsIter.next();
      if (attributeMappings.startEntity.declaration.definition == attributeStartEntity.declaration.definition
        ...
        if (attributeMappings.mappingsForAttributes.size() == 0) {
          ...
          if (attributeStartEntity.declaration.definition == entityState.declaration.definition) {
            attributeMappings.addMapping(new MappingForAttribute(arm, dataType, state));
            stateAttached = true;
          }
  ...
  private static void attachStatesAttributeMappings(AttributeAST arm, EntityAST dataType, LinkedList aimList, LinkedList pathStates,
      MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException, SdaiException {
    MappingDataWalker mappingDataWalker = new MappingDataWalker(pathWalker);
    ...
    try {
      ...
      while (stateIter.hasNext()) {
        ...
        boolean stateAttached = attachAttributeOneState(arm, dataType, attributeMappingsForEntities, realStartEntity, realState, pathWalker);
        if (!stateAttached && startEntity.declaration.definition != realStartEntity.declaration.definition) {
          stateAttached = attachAttributeOneState(arm, dataType, attributeMappingsForEntities, startEntity, state, pathWalker);
          if (!stateAttached) {
            pathWalker.reportWarning(arm,
                "This kind of attribute path is unsupported. " + "Attribute mapping requires " + realStartEntity.getText() + " as AIM entity");
          }
        }
        if (stateAttached) {
          stateIter.remove();
      ...
      while (stateIter.hasNext()) {
        ...
        pathWalker.notReported = state.notReported;
        EntityAST startEntity = state.getFirst(pathWalker.parser);
        pathWalker.reportError(arm, "Attribute mapping was not attached to entity mappings: " + startEntity.declaration.definition.getName(null));
      }
    }
    finally {
      pathWalker.notReported = null;
    ...
    while (attributeMappingsIter.hasNext()) {
      ...
      if (attributeMappings.entity.attachedAttributeMappings == null) {
        attributeMappings.entity.attachedAttributeMappings = attributeMappings.mappingsForAttributes;
      }
      else {
        attributeMappings.entity.attachedAttributeMappings.addAll(attributeMappings.mappingsForAttributes);
      }
    }
  }

  static public void createMappings(MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException, SdaiException {
    EEntity_definition armDefinition = (EEntity_definition) pathWalker.armEntity.declaration.definition;
    MappingDataWalker mappingDataWalker = new MappingDataWalker(pathWalker);
    ListIterator aimEntityIter = pathWalker.aimEntityList.listIterator();
    while (aimEntityIter.hasNext()) {
      EntityAST aimEntity = (EntityAST) aimEntityIter.next();
      EEntity_definition aimDefinition = (EEntity_definition) (aimEntity.pathState != null ?
          aimEntity.pathState.getFirst(pathWalker.parser) :
          aimEntity).declaration.definition;
      EEntity_mapping entityMappingInstance = (EEntity_mapping) (pathWalker.armEntity.declaration.entityMappings != null ?
          pathWalker.armEntity.declaration.entityMappings
              .get(aimDefinition.getName(null)) :
          null);
      if (entityMappingInstance == null) {
        entityMappingInstance = (EEntity_mapping) pathWalker.parser.mappingModel.createEntityInstance(EEntity_mapping.class);
        ...
        if (aimEntity.pathState != null) {
          constraints = mappingDataWalker.topPathElement(aimEntity.pathState.remaining, null, aimEntity.pathState);
        }
        if (aimEntity.exactType) {
          EType_constraint newTypeConstraint = pathWalker.parser.constraintFactory.createExactTypeConstraint(
              (EEntity_definition) aimEntity.declaration.definition, constraints);
        ...
        if (aimEntity.attachedAttributeMappings != null) {
          ...
          while (attributeMappingIter.hasNext()) {
            MappingForAttribute mappingForAttribute = (MappingForAttribute) attributeMappingIter.next();
            pathWalker.line = ((ParserAST) mappingForAttribute.attribute).getLine();
            try {
              if (mappingForAttribute.state == null || mappingForAttribute.state.mappedValue == null) {

                mappingForAttribute.mappingInstance = createAttributeMapping(entityMappingInstance, mappingForAttribute, mappingDataWalker,
                    pathWalker);
              }
              else {
                mappingForAttribute.mappingInstance = createAttributeValueMapping(entityMappingInstance, mappingForAttribute,
          ...
          if (pathWalker.armEntity.declaration.attributeMappings == null) {
            pathWalker.armEntity.declaration.attributeMappings = aimEntity.attachedAttributeMappings;
          }
          else {
            pathWalker.armEntity.declaration.attributeMappings.addAll(aimEntity.attachedAttributeMappings);
          }
        }
        if (pathWalker.armEntity.declaration.entityMappings == null) {
          pathWalker.armEntity.declaration.entityMappings = new HashMap();
        }
        pathWalker.armEntity.declaration.entityMappings.put(aimDefinition.getName(null), entityMappingInstance);
      }
      if (pathWalker.sourceLocator instanceof EntityMappingAST) {
        AEntity_mapping uofInstances = ((EntityMappingAST) pathWalker.sourceLocator).getUofInstances();
  ...
  private static EGeneric_attribute_mapping createAttributeMapping(EEntity_mapping entityMappingInstance, MappingForAttribute mappingForAttribute,
      MappingDataWalker mappingDataWalker, MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException, SdaiException {
    EAttribute_mapping attributeMappingInstance = (EAttribute_mapping) pathWalker.parser.mappingModel.createEntityInstance(EAttribute_mapping.class);
    ...
    try {
      ...
      if (path != null) {
        AAttribute_mapping_path_select pathAggregate = attributeMappingInstance.createPath(null);
          ...
          if (!(pathElement instanceof EType_constraint)) {
            try {
              ...
              if (e.getErrorId() == SdaiException.VT_NVLD) {
                pathWalker.reportWarning(mappingForAttribute.attribute, "!!!! SdaiException was thrown setting " + "the path using "
                    + pathElement + " !!!!");
  ...
  private static EGeneric_attribute_mapping createAttributeValueMapping(EEntity_mapping entityMappingInstance, MappingForAttribute mappingForAttribute,
      MappingDataWalker mappingDataWalker, MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException, SdaiException {
    EEntity realDomain = mappingForAttribute.attribute.getMappedValueDomain(mappingForAttribute.dataType);
    EAttribute_mapping_value attributeMappingInstance = null;
    boolean attributeMappingInstanceComplete = false;
    try {
      if (realDomain instanceof EEnumeration_type) {
        attributeMappingInstance = (EAttribute_mapping_value) pathWalker.parser.mappingModel
            .createEntityInstance(EAttribute_mapping_enumeration_value.class);
        ((EAttribute_mapping_enumeration_value) attributeMappingInstance).setMapped_value(null, mappingForAttribute.state.mappedValue.getText()
            .toLowerCase());
      }
        ...
        ((EAttribute_mapping_logical_value) attributeMappingInstance).setMapped_value(null,
        ...
        ((EAttribute_mapping_boolean_value) attributeMappingInstance).setMapped_value(null, boolString.equals("TRUE") ? true : false);
      }
      else {
        throw new MappingSemanticException(mappingForAttribute.attribute, "Attribute value mapping is unsupported " + "for this attribute");
      }
      attributeMappingInstance.setParent_entity(null, entityMappingInstance);
      attributeMappingInstance.setSource(null, mappingForAttribute.attribute.attribute);
      if (mappingForAttribute.dataType != null) {
    ...
    while (complexEntityIter.hasNext()) {
      ...
      if (entity.declaration != null) {
        ...
        while (leaveIterator == null || leaveIterator.next()) {
          EEntity_definition leave = leaveIterator != null ? (EEntity_definition) leaves.getCurrentMemberObject(leaveIterator) : entityDefinition;
          ...
          while (otherEntityIter.hasNext()) {
            ...
            if (entity != otherEntity && otherEntity.declaration != null) {
              EEntity_definition otherDefinition = (EEntity_definition) otherEntity.declaration.definition;
              ...
              while (otherLeaveIterator == null || otherLeaveIterator.next()) {
                EEntity_definition otherLeave = otherLeaveIterator != null ? ((EEntity_definition) otherLeaves
                    .getCurrentMemberObject(otherLeaveIterator)) : otherDefinition;
                if (isSupertypeOf(leave, otherLeave)) {
                  include = false;
                  break;
                }
                if (otherLeaveIterator == null) {
                  break;
                }
              }
              if (!include) {
          ...
          if (include) {
            names.add(leave.getName(null));
          }
          if (leaveIterator == null) {
            break;
          }
        }
      }
  ...
  private static EEntity getMappingTarget(EEntity path) throws SdaiException {
    if (path instanceof EAggregate_member_constraint) {
      ...
      return constraint instanceof EType_constraint ? ((EType_constraint) constraint).getDomain(null) : null;
    }
    else if (path instanceof EEntity_constraint) {
      ...
      return mappingTarget != null ? mappingTarget : getMappingTarget(pathConstraint.getElement1(null));
    }
    else if (path instanceof ESelect_constraint) {
      ...
      return mappingTarget != null ? mappingTarget : typeConstraint.getDomain(null);
    }
  ...
  static public void setAttributeMappingDomains(MappingPathParser parser) throws SdaiException {
    AAttribute_mapping attributeMappings = (AAttribute_mapping) parser.mappingModel.getInstances(EAttribute_mapping.class);
    ...
    while (attributeIterator.next()) {
      ...
      if (aimEntity instanceof ENamed_type) {
        ...
        if (attributeMapping.testData_type(null)) {
          ...
          Object attributeDomain = armAttribute.get_object(armAttribute.getAttributeDefinition("domain"));
        ...
        if (dataType != null) {
          DictionaryDeclaration dataTypeDeclaration = (DictionaryDeclaration) parser.armDeclarations.get(dataType.getName(null));
          if (dataTypeDeclaration != null && dataTypeDeclaration.type == DictionaryDeclaration.ENTITY && dataTypeDeclaration.entityMappings != null) {
            EEntity_mapping targetMapping = null;
            Iterator entityMappingIter = dataTypeDeclaration.entityMappings.values().iterator();
            while (entityMappingIter.hasNext()) {
              ...
              if (mappingAimType == aimDataType
                  || (mappingAimType instanceof EEntity_definition && aimDataType instanceof EEntity_definition && areCompatible(
                  (EEntity_definition) aimDataType, (EEntity_definition) mappingAimType))) {
                if (targetMapping != null) {
                  ...
                  if (aimDataType instanceof EEntity_definition
                      ...
                      && isCloserThan((EEntity_definition) aimDataType, (EEntity_definition) mappingAimType,
  ...
  public void createDerivedVariant(LinkedList pathStates, MappingPathWalker pathWalker) throws MappingSemanticException, RecognitionException, SdaiException {
    MappingDataWalker mappingDataWalker = new MappingDataWalker(pathWalker);
    Iterator stateIter = pathStates.iterator();
    while (stateIter.hasNext()) {
      WalkerState state = WalkerState.next(stateIter);
      if (state.remaining == null) {
        throw new MappingSemanticException(state.getFirst(pathWalker.parser), "Path needs to be specified");
      }
      EntityAST relatingAimEntity = state.getFirst(pathWalker.parser);
      EEntity_mapping relatingMapping = (EEntity_mapping) (declaration.entityMappings != null ? declaration.entityMappings
          .get(relatingAimEntity.declaration.definition.getName(null)) : null);
      if (relatingMapping == null) {
        throw new MappingSemanticException(state.getFirst(pathWalker.parser), declaration.definition.getName(null) + " has no mapping to "
      ...
      EEntity constraints = mappingDataWalker.topPathElement(state.remaining, path, state);
// 			if(path.size() == 0) {
...
// 			}
      ...
      if (!(relatedAim instanceof ENamed_type)) {
        throw new MappingSemanticException(state.getFirst(pathWalker.parser), "Path can not point to " + relatedAim);
      }
      ENamed_type relatedAimEntity = (ENamed_type) relatedAim;
      EEntity_mapping relatedMapping = (EEntity_mapping) declaration.entityMappings.get(relatedAimEntity.getName(null));
      if (relatedMapping == null) {
        throw new MappingSemanticException(state.getFirst(pathWalker.parser), declaration.definition.getName(null) + " has no mapping to "
            + relatedAimEntity.getName(null));
      }
      EDerived_variant_entity_mapping derivedVariantInstance = (EDerived_variant_entity_mapping) pathWalker.parser.mappingModel
  ...
  public EEntity getDomain() throws SdaiException {
    if (!(declaration.definition instanceof EDefined_type)) {
      return null;
    }
  ...
  private boolean isDomainValid(EEntity domain, ENamed_type type) throws SdaiException {
    ...
    if (domain instanceof EEntity_definition && type instanceof EEntity_definition) {
      ...
      if (definedTypeDomain instanceof ESelect_type) {
        ...
        while (selectionIterator.next()) {
          ENamed_type selection = selections.getCurrentMember(selectionIterator);
          if (isDomainValid(selection, type)) {
            return true;
          }
        }
      }
  ...
  private boolean getPathToElement(EEntity domain, ENamed_type targetType, List selectPath) throws SdaiException {
    if (domain == targetType) {
      ...
      return getPathToElement(((EAggregation_type) domain).getElement_type(null), targetType, selectPath);
    }
    return false;
  }
