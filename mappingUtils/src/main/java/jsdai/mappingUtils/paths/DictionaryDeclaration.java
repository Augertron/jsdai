...
public class DictionaryDeclaration {
  ...
  static public HashMap create(SdaiModel model) throws SdaiException {
    ...
    AType_declaration typeDeclarations = (AType_declaration) model.getInstances(EType_declaration.class);
    SdaiIterator declarationIterator = typeDeclarations.createIterator();
    while (declarationIterator.next()) {
      EType_declaration typeDeclaration = typeDeclarations.getCurrentMember(declarationIterator);
    ...
    while (definedTypeIter.hasNext()) {
      DictionaryDeclaration definedTypeDeclaration = (DictionaryDeclaration) definedTypeIter.next();
      ...
      if (domain instanceof EExtended_select_type) {
        EDefined_type extensibleSelectDefType = ((EExtended_select_type) domain).getIs_based_on(null);
        String definedTypeName = extensibleSelectDefType.getName(null);
        DictionaryDeclaration extensibleDeclaration = (DictionaryDeclaration) map.get(definedTypeName);
    ...
    AEntity_declaration entityDeclarations = (AEntity_declaration) model.getInstances(EEntity_declaration.class);
    declarationIterator = entityDeclarations.createIterator();
    while (declarationIterator.next()) {
      EEntity_declaration entityDeclaration = entityDeclarations.getCurrentMember(declarationIterator);
  ...
  static private void collectAttributes(HashMap attributeMap, EEntity_definition entity) throws SdaiException {
  ...
  private boolean isContainedInSupertype(ENamed_type listDefinition) throws SdaiException {
    if (listDefinition instanceof EEntity_definition) {
      ...
      if (entity.testGeneric_supertypes(null)) {
        ...
        while (supertypeIterator.next()) {
          EEntity_definition supertype = (EEntity_definition) supertypes.getCurrentMemberObject(supertypeIterator);
          if (supertype == definition) {
            return true;
          }
          if (isContainedInSupertype(supertype)) {
            return true;
          }
        }
      }
  ...
  static private void addSelections(ANamed_type allSelections, ANamed_type selections, DictionaryDeclaration typeDeclaration) throws SdaiException {
    ...
    if (typeDeclaration.attributes != null) {
      ...
      while (extendedTypeIter.hasNext()) {
        DictionaryDeclaration extendedTypeDeclaration = (DictionaryDeclaration) extendedTypeIter.next();
        EDefined_type extendedType = (EDefined_type) extendedTypeDeclaration.definition;
        addSelections(allSelections, ((ESelect_type) extendedType.getDomain(null)).getSelections(null), extendedTypeDeclaration);
      }
    }
  }
