...
public class ExtClsGenerator extends ExtGenerateMethods {
  /**
   *
   */
  public ExtClsGenerator(String apPartNumber, String genTargetDir, String mainSchema, boolean genMainFunction) throws SdaiException, Exception {
   ...
   *
   */
  public ExtClsGenerator(String apPartNumber, String mainSchema, String docSourcePath) throws SdaiException, Exception {
  ...
  public void generateClassSkeleton(FileWriter outFile, String className, EEntity_definition entDef) throws Exception {
    ...
    this.currentlyGeneratedClassName = className.substring(1).toLowerCase();
    ...
    String constantsDescr = " <DD/>As class can have several similar looking getX methods for each attribute,"
        ...
        + " For convenience of users of Mx classes a general getXX method is available for"
        + " each attribute. This method returns an Object, which can be processed in any" + " desired way.<BR>";
  ...
  public void generateClassMethods(Vector classMethods, EEntity_definition entDef) throws SdaiException, Exception {
    ...
    generateFindCompatibleMethod(classMethods, entDef);
...
//			generateSetXXAttributeMethods(classMethods, entDef);
  ...
  public void generateClasses() throws SdaiException, Exception {
    ...
    for (int i = 0; i < count; i++) {
      ...
      // skip generation of static classes for arm complex types. No visible
      // usage for them yet.
      if (defName.indexOf('+') != -1) {
        continue;
      }
      // capitalize first letter of name:
      defName = capitalize(defName);
      //System.out.println("defName is "+defName);
      //if (!defName.equalsIgnoreCase("assembly_composition_relationship"))
      //	continue;
      //System.out.println("defName is "+defName);
      /*
       * if ((defName.equalsIgnoreCase("poly_loop")) ||
       * (defName.equalsIgnoreCase("kinematic_pair")) ||
       * (defName.equalsIgnoreCase("light_source")) ||
       * (defName.equalsIgnoreCase("specification")) ||
       * (defName.equalsIgnoreCase("specification_category")) ||
       * (defName.equalsIgnoreCase("curved_distance_dimension")) ||
       * (defName.equalsIgnoreCase("angular_location_dimension"))
       * )
       * continue;
       */
      if (outTestFile != null) {
  ...
  public static final void main(String args[]) throws SdaiException, Exception {
    ...
    if (args.length < 3) {
      System.out.println("USAGE: java jsdai.mappingUtils.ExtClsGenerator apnumber mainschemaname targetpath");
      System.out.println("OR USAGE: java jsdai.mappingUtils.ExtClsGenerator apnumber mainschemaname -documentation docSourcePath [-read_only_op]");
      return;
    }
    ...
    if (!isDocumenting) {
      gen = new ExtClsGenerator(apNumber, outputPath, mainSchemaName, generateMain);
    }
    else {
      // we are documenting. So:
      gen = new ExtClsGenerator(apNumber, mainSchemaName, docSourcePath);
