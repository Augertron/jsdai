	...
	SourceLocatorMark mark = null;
		...
		(	enum1:ENUM
			{	if(states.size() == 1 && mappedValueSet != null) {
					if(mappedValueSet.contains(enum1.getText().toLowerCase())) {
						((WalkerState)states.getFirst()).mappedValue = enum1;
						constraintLevel++;
					} else {
						throw new MappingSemanticException(enum1, "Mapped value invalid");
					}
				} else {
					throw new MappingSemanticException(enum1, "Mapped value can not be used here");
				}
			}
		)?
	;
...
{	AST remaining = null;
	AST possibleMappedValue = null;
		...
		| enum1:ENUM 
			{	remaining = #(auxiliaryAST(enum1, ENUMERATION_CONSTRAINT), astFactory.dup(enum1));
				possibleMappedValue = enum1;
			}
		)
