...
/**
 * Class that centres dialog windows and shows popup menus.
 */
public final class WindowCenterer {
  /**
   * Shows dialog centred to parent component.
   *
   * @param parent parent component.
   * @param child  dialog window to centre.
   */
  ...
  private static void showCenteredImpl(Component parent, Component child) {
    ...
    child.setLocation(calculate(parent.getToolkit().getScreenSize(), new Rectangle(loc, parent.getSize()), child.getSize()));
  ...
  public static void showCenteredOnScreen(Component comp) {
    ...
    comp.setLocation((screenDim.width - winDim.width) / 2, (screenDim.height - winDim.height) / 2);
   ...
   */
  private static Point calculate(Dimension screenSize, Rectangle parentBnd, Dimension childDim) {
    Point ret = new Point();
   ...
   * @param iposy
   */
  public static void showPopup(JPopupMenu menu, Component component, int iposx, int iposy) {
   ...
   */
  private static class ParentRepainter implements ComponentListener {
    /**
     * Parent component.
     */
    ...
    private void repaintParent() {
      mParent.repaint(mDarkZone.x - dx, mDarkZone.y - dy, mDarkZone.width, mDarkZone.height);
    }
  }
}
