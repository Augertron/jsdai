    ...
    public void actionPerformed(ActionEvent e) {
      try {
        ...
        if (comps[0] instanceof JComboBox) {
          tik = (JComboBox) comps[0];
        }
        if (comps[1] instanceof JTextField) {
          dik = (JTextField) comps[1];
        }
...
//		inverses.addKeyListener(findKeyListener);
    ...
    bget.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        try {
          if (entity != null) {
            getSession().getClipboard().addByIndex(getSession().getClipboard().getMemberCount() + 1, entity, null);
          }
        }
        catch (SdaiException h) {
          processMessage(h);
...
//		if (this.entity == entity) return;
    try {
      ...
      if (dataDomainOrigin instanceof SchemaInstance) {
        ...
        dataDomain.addByIndex(1, dataDomainOrigin, null);
      }
      else {
...
//printing attributes of supertypes
    ...
    if (definition.getComplex(null)) {
      return; // complexes don't have to be displayed as themsevles
    }
...
//texts
     ...
     * if ((domain instanceof EAggregation_type) || (domain instanceof EEntity_definition) || (domain instanceof
     * ESelect_type)) {
...
//processMessage(ex); }
   ...
   * // buildAttributes(attributes.getCurrentMember(it_attributes), definition,
   * ((EDerived_attribute)attributes.getCurrentMember(it_attributes)).testDomain());
   * }
   * else if (attributes.getCurrentMember(it_attributes) instanceof EInverse_attribute){
   * // buildAttributes(attributes.getCurrentMember(it_attributes), definition,
   * ((EInverse_attribute)attributes.getCurrentMember(it_attributes)).getInverted_attr().testDomain());
   * }
   * }
   * } else {
   ...
   * }
   *
   * private void buildAttribute(JPanel labels, JPanel texts, JPanel unsets, EAttribute attribute, EEntity_definition
   * definition, EEntity type) throws SdaiException {
  ...
  static public String getLocationString(EEntity entity, Object dataDomainOrigin) throws SdaiException {
    if (dataDomainOrigin instanceof SchemaInstance) {
      SchemaInstance currentSchemaInstance = (SchemaInstance) dataDomainOrigin;
      return "/" + currentSchemaInstance.getRepository().getName() + "/" + currentSchemaInstance.getName() + "/" + entity.getInstanceType().getName(null)
          + "/" + entity.getPersistentLabel();
    }
    else if (dataDomainOrigin instanceof SdaiModel) {
      SdaiModel currentModel = (SdaiModel) dataDomainOrigin;
      return "/" + currentModel.getRepository().getName() + "/" + currentModel.getName() + "/" + entity.getInstanceType().getName(null) + "/"
          + entity.getPersistentLabel();
    }
    else {
...
//		paintAll(getGraphics());
  ...
  public String copyContentsAsText() {
    ...
    try {
      ...
      result += "=== SdaiEdit {" + getSession().getSdaiImplementation().getLevel() + "} - EntityInstance - " + dateFormatter.format(date)
          + "\n";
  ...
  class ButtonWithMenu extends JPanel {
    ...
    void showPopup() {
      try {
        ...
        JMenuItem menuItem;
        int i = 0;
        Aggregate agg = getSession().getClipboard();
