...
public class GoTable extends JTable {
  ...
  void setUnderlyingRow(int i) {
    if ((i == underlyingRow) || !isUnderlineOn) {
      return;
    }
    repaint((int) getCellRect(underlyingRow, 0, false).getX(), (int) getCellRect(underlyingRow, 0, false).getY(),
        (int) getCellRect(underlyingRow, getColumnCount() - 1, false).getMaxX(), getRowHeight());
    underlyingRow = i;
    if (i != -1) {
      requestFocus();
      ...
      g2.drawRect((int) getCellRect(underlyingRow, 0, false).getX(), (int) getCellRect(underlyingRow, 0, false).getY(),
          (int) getCellRect(underlyingRow, getColumnCount() - 1, false).getMaxX() - 1, getRowHeight() - 2);
    }
  }
  ...
  MouseListener mouseListener = new MouseAdapter() {
    ...
    public void mousePressed(MouseEvent e) {
      if (((e.getModifiers() & InputEvent.BUTTON3_MASK) == 0) && timer.isRunning() && (row == getSelectedRow()) && (getSelectedRow() != -1)) {
        timer.stop();
  ...
  javax.swing.Timer mouseMotionTimer = new javax.swing.Timer(1000, new ActionListener() {
    public void actionPerformed(ActionEvent e) {
      if (underlyingRow == mouseMotionTimerIndex) {
        if ((mouseMotionTimerIndex >= 0) && (mouseMotionTimerIndex < getRowCount())) {
