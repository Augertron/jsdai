...
//        labelAlternatives.addKeyListener(keyListener);
    labelAlternatives.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent e) {
        Object o[] = { alternatives.getSelectedItem(), mappingDomain };
...
//        pas.add(b, BorderLayout.EAST);
    ...
    get.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          if (selection.size() > 0) {
            getSession().getClipboard()
                .addByIndex(getSession().getClipboard().getMemberCount() + 1, selection.elementAt(selection.size() - 1), null);
          }
          /*
           * // Object result = instances[ii].getMappedAttribute((EAttribute_mapping)encodeToken(tokens[1]),
           ...
           * if (selection.size() > 0) {
           * SdaiSession.getSession().getClipboard().addByIndex(SdaiSession.getSession().getClipboard().getMemberCount()+
           * 1, selection.elementAt(selection.size()-1), null);
           * }
           */
        }
  ...
  public void setInstanceAndMappingAndDomain(EEntity entity, EEntity_mapping mapping, ASdaiModel domain) {
    ...
    MappingOperationsThreadManager.invokeMethod(this, "setInstanceAndMappingAndDomainSingleThreaded", types, params);
  }
  ...
  public void setInstanceAndMappingAndDomainSingleThreaded(EEntity entity, EEntity_mapping mapping, ASdaiModel domain) {
    try {
      ...
      mappingDomain = MappingOperations.getMappingDomain(getSession().getMappings(model.getUnderlyingSchema()));
...
//             } else
  ...
  private void buildAttribute(JPanel plabels, JPanel ptexts, JPanel ptypes, JPanel punsets, EAttribute attribute, EEntity_definition definition)
    ...
    //		plabels.add(label);
    //types
    GoLabel ltype = new GoLabel(ident + SimpleOperations.getAttributePrefix(attribute) + " " + SimpleOperations.getAttributeName(attribute) + ": "
        + SimpleOperations.getAttributeDomainString(attribute));
    if (isDeveloperMode) {
    ...
    try {
      ...
      if (mapping != null) {
        ...
        try {
          ...
          inverses.setPreferredScrollableViewportSize(new Dimension(0, inverses.getRowHeight()
              * ((inverses.getRowCount() < 20) ? inverses.getRowCount() + 1 : 20)));
        }
        catch (SdaiException ex) {
  ...
  public String getTreeLeave() throws SdaiException {
    if ((entity == null) || (mapping == null)) {
      ...
      return entity.findEntityInstanceSdaiModel().getRepository().getName() + "/" + entity.findEntityInstanceSdaiModel().getName() + "/"
          + MappingOperations.findSchema_mappingFor(mapping).getId(null) + "/" + mapping.getSource(null).getName(null) + "/"
          + entity.getPersistentLabel();
    }
  }
      ...
      for (int i = 0; i < textes.size(); i++) {
        if (lastSelection == textes.elementAt(i)) {
          ...
          if ((target instanceof EEntity_definition) && (source instanceof EEntity_definition)) {
            AEntity_mapping mapps = MappingOperations.getMappingForARM_And_AIM((EEntity_definition) target, (EEntity_definition) source,
                mappingDomain, new AEntity_mapping());
            ...
            if (attr.testData_type(null)) {
              ...
              if (ant.getMemberCount() > 0) {
                ...
                if (nt instanceof EEntity_definition) {
                  AEntity_mapping mapps = MappingOperations.getMappingForARM_And_AIM((EEntity_definition) target, (EEntity_definition) nt,
                      mappingDomain, new AEntity_mapping());
                  if (mapps.getMemberCount() > 0) {
                    processed_mapping = mapps.getByIndex(1);
                  }
  ...
  public String copyContentsAsText() {
    ...
    try {
      ...
      result += "=== SdaiEdit {" + getSession().getSdaiImplementation().getLevel() + "} - InstanceMapping - " + dateFormatter.format(date)
          + newLine;
      ...
      try {
        ...
        if (value instanceof EEntity) {
          ...
          if ((s[0] instanceof ArrayList) || (s[0] instanceof AEntity)) {
            ...
            MappingOperations.getMostSpecificMappings(en, ((s[1] != null) ? ((EEntity_mapping) s[1]).getSource(null) : null), dataDomain,
                mappingDomain, aem);
