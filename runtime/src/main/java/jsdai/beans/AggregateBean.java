...
//		bar.add(bset);
    ...
    bget.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          if (selected != null) {
            getSession().getClipboard().addByIndex(getSession().getClipboard().getMemberCount() + 1, selected, null);
          }
        }
      ...
      synchronized public void actionPerformed(ActionEvent e) {
        try {
          ...
          if (domain instanceof EAggregation_type) {
            SimpleOperations.createElementAggregate(getAggregate(),
                SimpleOperations.correctAggregateIndex(list.getSelectedIndex(), getAggregate().getAggregationType(), 0), null);
            ...
            if (!entityAggregate) {
              SimpleOperations.addElementString(instance, getAggregate(),
                  SimpleOperations.correctAggregateIndex(list.getSelectedIndex(), getAggregate().getAggregationType(), 0), value, defs);
            }
            else {
              SimpleOperations.addElementObject(getAggregate(),
                  SimpleOperations.correctAggregateIndex(list.getSelectedIndex(), getAggregate().getAggregationType(), 0),
                  getClipboardElement(), null);
            }
          }
          fireAggregateChanged();
          list.setSelectedValue(instance, true);
    ...
    bcreate.addActionListener(new ActionListener() {
      synchronized public void actionPerformed(ActionEvent e) {
        try {
          ...
          Collection entDefinitions = AttributePanel.getDomainEntityDefinitions(getSession().getSdaiContext(), domain, null);
          ...
          if (edList != null) {
            ...
            EEntity_definition selectedEntity = AttributePanel.getEntityDefinitionDialog(AggregateBean.this, availableTypes);
            if (selectedEntity != null) {
              EEntity newInstance = owning_model.createEntityInstance(selectedEntity);
              SimpleOperations.addElementObject(getAggregate(),
                  SimpleOperations.correctAggregateIndex(list.getSelectedIndex(), getAggregate().getAggregationType(), 0), newInstance, null);
            }
            fireAggregateChanged();
            list.setSelectedValue(instance, true);
            fireSdaiSelectionProcessed();
      ...
      public void actionPerformed(ActionEvent e) {
        try {
          ...
          if (!list.isSelectionEmpty() && (list.getSelectedIndex() != -1)) {
            SimpleOperations.removeElement(getAggregate(),
                SimpleOperations.correctAggregateIndex(list.getSelectedIndex(), getAggregate().getAggregationType(), 0));
            fireAggregateChanged();
          }
        }
      ...
      if (aggregate.getAggregationType() != null) {
        ...
        if (aggregate.getAggregationType().testElement_type(null)) {
          ...
          if (st != null) {
            ...
            if (SimpleOperations.getNodes(getSession().getSdaiContext(), st, nodes, new Vector())) {
  ...
  public String getTreeLeave() throws SdaiException {
    if (instance == null) {
      ...
      return BaseEntityBean.getLocationString(instance, dataDomainOrigin) + "/Aggregate";
    }
  }
  ...
  public String copyContentsAsText() {
    ...
    try {
      ...
      result += "=== SdaiEdit {" + getSession().getSdaiImplementation().getLevel() + "} - Aggregate - " + dateFormatter.format(date) + "\n";
