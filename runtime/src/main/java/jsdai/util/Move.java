...
public class Move {
   ...
   * @param repoTarget
   */
  public static void moveRepoContents(SdaiRepository repoSource, SdaiRepository repoTarget) throws SdaiException {
    int i, j;
    boolean found;
    String str;
    ...
    for (i = 0; i < ms_count; i++) {
      ...
      while (it_decls.next()) {
        ...
        for (j = 1; j <= init_count; j++) {
          t_mod.substituteInstance(insts.getByIndexEntity(1));
        }
      }
    }
    ...
    for (i = 1; i <= schInstsSource.getMemberCount(); i++) {
      ...
      for (j = 1; j <= s_assocMods.getMemberCount(); j++) {
        ...
        if (!found) {
          SdaiSession.println("Associated model '" + mod.getName() + "' not found." + line_separator);
        }
      }
    }
    mods_arr = null;
   ...
   * @return
   */
  public static SdaiRepository moveSchemaInstance(SchemaInstance si, SdaiRepository repo) throws SdaiException {
    return moveSchemaInstanceInternal(si, repo, true);
  }

  protected static SdaiRepository moveSchemaInstanceInternal(SchemaInstance si, SdaiRepository repo, boolean delete) throws SdaiException {
    int i;
    String str;
    SdaiSession.setLogWriter(new PrintWriter(System.out, true));
    if (!repo.isActive()) {
