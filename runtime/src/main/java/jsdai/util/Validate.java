 ...
 * <P>
 * <code>Validate</code> is an application program used to validate exchange structures, that is, files prepared according to 'ISO
 * 10303-21: Implementation methods: Clear text encoding of the exchange structure'. After importing the submitted exchange
 * structure to a temporary repository, validation is performed in two stages: <br>
 * - validation of all entity instances contained in all models of the repository; <br>
 * - validation of all global and uniqueness rules for all schema instances of the repository. <br>
 * In the first stage, for each entity instance, the following operations are performed: <br>
 * - validate required explicit attributes assigned; <br>
 * - validate inverse attributes; <br>
 * - validate explicit attributes references; <br>
 * - validate aggregates size; <br>
 * - validate aggregates uniqueness; <br>
 * - validate array not optional; <br>
 * - validate where rule. <br>
 * In the second stage, the following two operations on schema instances are performed: <br>
 * - validate global rule; <br>
 ...
 * <P>
 * <pre>    java jsdai.util.Validate abc.stp -stdout Valid.log</pre> Here 'Valid.log' is (an arbitrary) name of a log
 * file produced by the program. <br>
 * <strong>NOTE:</strong> similarly as other JSDAI-based applications, <code>Validate</code> also requires configuration file
 * <code>jsdai.properties</code> to be in its disposition. <br>
 * The log file includes the following information: <br>
 * - the name of the exchange structure and the name of the repository to which this structure is imported; <br>
 * - for each model, the total number of instances and a list of instances not conforming to the validation; for each such
 * instance, also a more specific information (like violated rules or atributes with mandatory values not set) is displayed; in
 * the case where a where rule is violated, its owner (either partial entity data type or defined type) is indicated; <br>
 * - for each model, the number of instances not conforming to the validation; <br>
 * - for each schema instance, the total number of global rules and a list of global rules which are not satisfied by the
 * population associated with this schema instance; for each such global rule, violated where rules are given; <br>
 * - for each schema instance, the number of global rules for which validation has failed (<code>false</code> was returned); <br>
 * - for each schema instance, a list of violated uniqueness rules (if any); for each such rule, entity instances not conforming
 * to the validation are displayed; <br>
 * - for each schema instance, the number of violated uniqueness rules.
 * <P>
 * You should see in the log file something similar to:
 * <P>
 * <br>
 * 'Validate' program. Copyright 1999-2012, LKSoftWare GmbH <br>
 * --- Exchange structure: allied_203.stp <br>
 * --- Imported to the repository: allied_203_org <br>
 * --- Reading time=4sec <br>
 * count of instances in model "default": 785 <br>
 * #28=MECHANICAL_CONTEXT('CONFIGURATION CONTROL DESIGN',#26,'MECHANICAL'); <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" in entity "mechanical_context" is violated <br>
 * #36=PRODUCT_DEFINITION_SHAPE('DESIGN SHAPE','SHAPE FOR part',#35); <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" in entity "product_definition_shape" is violated <br>
 * count of erroneous instances: 2 <br>
 * count of global rules in schema instance "schema1": 79 <br>
 * For the global rule "restrict_date_time_role" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "restrict_person_organization_role" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "restrict_security_classification_level" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "security_classification_requires_date_time" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "restrict_approval_status" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "product_version_requires_person_organization" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr2" is violated <br>
 * For the global rule "product_requires_product_category" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "restrict_product_category_value" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * For the global rule "design_context_for_property" validation gives FALSE <br>
 * &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; where rule "wr1" is violated <br>
 * count of violated global rules: 9 <br>
 * count of violated uniqueness rules: 0 <br>
 * --- Validation time=32sec
 * <p>
 * <code>Validate</code> allows the user to indicate those rules which he/she wants to exclude from the complete validation
 * process. All such rules should be listed in the text file that should be declared in <code>jsdai.properties file</code> with
 * the keyword <code>excluded_rules</code>. For example, if the text file is named <code>tough_rules.txt</code> and placed in the
 * <code>C:\temp</code> directory, then the instruction included in <code>jsdai.properties</code> has to be
 * <code>excluded_rules=c:\\temp\\tough_rules.txt</code>.
 * </p>
 * <p>
 * The excluded rules file consists of strings with usual separators between them. A string may take one of the following forms:
 * </p>
 * <ol>
 * <li>
 * <code>entity_name.where_rule_name</code> <br>
 * This string specifies the where rule with the name <code>where_rule_name</code> in the entity <code>entity_name</code>.</li>
 * <li>
 * <code>entity_name</code> <br>
 * Specifies the non-named where rule in the entity <code>entity_name</code>. Notice that for such a rule, method
 * <code>testLabel(null)</code> returns <code>false</code> value.</li>
 * <li>
 * <code>global_rule_name.where_rule_name</code> <br>
 * Specifies the where rule with the name <code>where_rule_name</code> in the global rule <code>global_rule_name</code>.</li>
 * <li>
 * <code>global_rule_name</code> <br>
 * This string indicates that all where rules owned by the global rule <code>global_rule_name</code> must be excluded.</li>
 * </ol>
 * <p>
 * All where rules documented in the file are bypassed during validation. This feature of <code>Validate</code> is useful when
 * some rules in STEP are broken or take too long for practical validation.
 * </p>
 * <br>
 *
 * <pre>
 * Example of the excluded rules file:
 *     mapped_item.wr1 datum.wr4 datum_feature.wr2
 *     founded_item.wr1
 *     application_protocol_definition_required.wr1
 *     restrict_representation_for_document_properties
 * </pre>
 * <p>
 * The last two strings in this file present the name of a global rule.
 */
...
//	List queue;
  ...
  private void identifyPartialEntities(EEntity_definition entity, AEntity_definition result) throws SdaiException {
// all supertypes are processed recursively
  ...
  private int validateWhereRulesXim(EEntity instance, EEntity_definition type, AWhere_rule w_rules_viol, ASdaiModel models_dom) throws SdaiException {
...
// Preparing a string for output in the case when parent is of an entity data type
      ...
      if (parent instanceof CEntity_definition) {
        ...
        if (rule.testLabel(null)) {
          if (e == null) {
            str = " \t   Violation of where rule \"" + rule.getLabel(null) + "\" in entity \"" + ((CEntity_definition) parent).getName(null)
                + "\" ";
          }
          else {
            str = " \t   An outcome from where rule \"" + rule.getLabel(null) + "\" in entity \"" + ((CEntity_definition) parent).getName(null)
                + "\" is an error: " + mess;
          }
        }
        else {
          if (e == null) {
            str = " \t   Violation of where rule without name in entity \"" + ((CEntity_definition) parent).getName(null) + "\" ";
          }
          else {
            str = " \t   An outcome from where rule without name in entity \"" + ((CEntity_definition) parent).getName(null) + "\" is an error: "
                + mess;
          }
        }
// Preparing a string for output in the case when parent is of a defined data type
        ...
        if (rule.testLabel(null)) {
          if (e == null) {
            str = " \t   Violation of where rule \"" + rule.getLabel(null) + "\" in defined type \"" + ((CDefined_type) parent).getName(null)
                + "\" ";
          }
          else {
            str = " \t   An outcome from where rule \"" + rule.getLabel(null) + "\" in defined type \"" + ((CDefined_type) parent).getName(null)
                + "\" is an error: " + mess;
          }
        }
        else {
          if (e == null) {
            str = " \t   Violation of where rule without name in defined type \"" + ((CDefined_type) parent).getName(null) + "\" ";
          }
          else {
            str = " \t   An outcome from where rule without name in defined type \"" + ((CDefined_type) parent).getName(null) + "\" is an error: "
                + mess;
          }
        }
...
// Directing the name of the violated uniqueness rule and its parent to the output stream
    if (rule.testLabel(null)) {
      pw.println("For the uniqueness rule \"" + rule.getLabel(null) + "\" defined in entity data type \"" + edef.getName(null)
      ...
      pw.println("For the uniqueness rule without name defined in entity data type \"" + edef.getName(null) + "\" validation gives FALSE");
...
// Is 'true' if at least one violation for the current instance was reported
    ...
    if (monitor != null) {
      monitor.worked(1);
      monitor.subTask("Entity instance " + inst_counter + " out of " + inst_count + ", type " + cur_name, all_steps_count, inst_counter);
    }
...
// Selection of the value (true/false) to be returned
    if (res1 == ELogical.FALSE || !res2 || !res3 || res4 == ELogical.FALSE || res5 == ELogical.FALSE || !res6 || !res7 || res8 == ELogical.FALSE) {
...
// Running through the set of rule declarations
      while (it_decls.next()) {
        ...
        if (monitor != null) {
          monitor.worked(1);
          monitor.subTask("Global rule: " + gl_rule.getName(null) + " (" + gl_rules_index + " out of " + gl_rules_count + ")", all_steps_count,
              tot_counter);
        }
...
// Validation of the uniqueness rule
          ...
          if (monitor != null) {
            ...
            if (u_rule.testLabel(null)) {
              monitor.subTask("Uniqueness rule: " + u_rule.getLabel(null) + " (" + un_rules_index + " out of " + un_rules_count + ")",
                  all_steps_count, tot_counter);
            }
            else {
              monitor.subTask("Uniqueness rule: " + "no name" + " (" + un_rules_index + " out of " + un_rules_count + ")", all_steps_count,
                  tot_counter);
            }
          }
          nonConf_ent.clear();
...
// Ending the transaction, deleting the repository, closing the session
      ...
      if (eee instanceof SdaiException && ((SdaiException) eee).getErrorId() == SdaiException.SY_ERR && validation != null
          && validation.identifyMissingSchema((SdaiException) eee) != null) {
        String sch = validation.identifyMissingSchema((SdaiException) eee);
        pw.println("The schema \"" + sch + "\" required to import the selected exchange structure" + " does not exist in your library jar file."
            + System.getProperty("line.separator") + "Please try again with appropriately chosen jar file.");
      }
      else {
  ...
  public static Runnable initAsRunnable(final String sdaireposDirectory, final String[] args, final UtilMonitor monitor) throws SdaiException {
    ...
    for (int ihi = 0; ihi < args.length - 1; ihi++) {
       ...
       * NOTICE: it is quite possible, that -dummy is never used, for mim-xim validation perhaps -MIM or -XIM is always
       * present
      ...
      if ((args[ihi].equals("-XIM")) || (args[ihi].equals("-MIM")) || (args[ihi].equals("-dummy"))) {
        jsdaiProperties.setProperty("jsdai.SIda_step_schema_xim",
            "AC*;AI*;AP*;ASS*;AU*;B*;C*;D*;E*;F*;G*;H*;IDA_STEP_AIM*;ISO*;IN*;J*;K*;L*;M*;N*;O*;P*;Q*;R*;S*;T*;U*;V*;W*;X*;Y*;Z*;");
        break;
      }
    } // end for
