 ...
 */
public class SdaiException extends RuntimeException {
  ...
  public SdaiException() {
    super(error_descriptions.get(SY_ERR));
  }
  ...
  public SdaiException(int id) {
    super(error_descriptions.get(id));
    error_id = id;
  }
  ...
  public SdaiException(int id, Object base) {
    super(error_descriptions.get(id) + ". " + base);
    error_id = id;
    error_base = base;
  }
  ...
  public SdaiException(int id, Object base, String text) {
    super(error_descriptions.get(id) + ". " + text);
    error_id = id;
    error_base = base;
  }
  ...
  public SdaiException(int id, Exception exception) {
    super(error_descriptions.get(id) + ". UnderlyingException:\n" + exception);
    error_id = id;
  ...
  protected SdaiException(int id, Object base, String text, boolean makeDescription) {
    super(makeDescription ? error_descriptions.get(id) + ". " + text : text);
    error_id = id;
    error_base = base;
  }

  /**
   * Creates a new instance of <code>SdaiException</code> with specified error indicator and specified instance related
   * to or causing the error. Also, some text extending the standard error description can be submitted.
   *
   * @param id        the error indicator.
   * @param base      an instance related to or causing the error.
   * @param text      some text to be added to the standard error description.
   ...
   * @see #SdaiException(int id, Object base, Exception exception)
   */
  public SdaiException(int id, Object base, String text, Exception exception) {
    super(error_descriptions.get(id) + ". " + text + ". UnderlyingException:\n" + exception);
    error_id = id;
    error_base = base;
    underlyingException = exception;
  }
   ...
   * does not assume the existence of such an <code>Exception</code>, then <code>null</code> is returned.
   *
   ...
   *
   * @return an indicator for the error represented by this <code>SdaiException</code> object.
   * @see "ISO 10303-22::11 SDAI errors"
   */
   ...
   * If this object of <code>SdaiException</code> is for error which type
   * does not assume the existence of such an instance, then <code>null</code> is returned.
   *
  ...
  private static void initError_descriptions() {
    error_descriptions.put(SS_OPN, "SS_OPN - Session open");
    error_descriptions.put(SS_NAVL, "SS_NAVL - Session not available");
    error_descriptions.put(SS_NOPN, "SS_NOPN - Session is not open");
    error_descriptions.put(RP_NEXS, "RP_NEXS - Repository does not exist");
    error_descriptions.put(RP_NAVL, "RP_NAVL - Repository not available");
    error_descriptions.put(RP_OPN, "RP_OPN - Repository open");
    error_descriptions.put(RP_NOPN, "RP_NOPN - Repository is not open");
    error_descriptions.put(RP_DUP, "RP_DUP - Repository duplicate");
    error_descriptions.put(TR_EAB, "TR_EAB - Transaction ended abnormally");
    error_descriptions.put(TR_EXS, "TR_EXS - Transaction exists");
    error_descriptions.put(TR_NAVL, "TR_NAVL - Transaction currently not available");
    error_descriptions.put(TR_RW, "TR_RW - Transaction read-write");
    error_descriptions.put(TR_NRW, "TR_NRW - Transaction not read-write");
    error_descriptions.put(TR_NEXS, "TR_NEXS - Transaction does not exist");
    error_descriptions.put(MO_NDEQ, "MO_NDEQ - SDAI-model not domain equivalent");
    error_descriptions.put(MO_NEXS, "MO_NEXS - SDAI-model does not exist");
    error_descriptions.put(MO_NVLD, "MO_NVLD - SDAI-model invalid");
    error_descriptions.put(MO_DUP, "MO_DUP - SDAI-model duplicate");
    error_descriptions.put(MX_NRW, "MX_NRW - SDAI-model access not read-write");
    error_descriptions.put(MX_NDEF, "MX_NDEF - SDAI-model access not defined");
    error_descriptions.put(MX_RW, "MX_RW - SDAI-model access read-write");
    error_descriptions.put(MX_RO, "MX_RO - SDAI-model access read-only");
    error_descriptions.put(SD_NDEF, "SD_NDEF - Schema definition not defined");
    error_descriptions.put(ED_NDEF, "ED_NDEF - Entity definition not defined");
    error_descriptions.put(ED_NDEQ, "ED_NDEQ - Entity definition not domain equivalent");
    error_descriptions.put(ED_NVLD, "ED_NVLD - Entity definition invalid");
    error_descriptions.put(RU_NDEF, "RU_NDEF - Rule not defined");
    error_descriptions.put(EX_NSUP, "EX_NSUP - Expression evaluation not supported");
    error_descriptions.put(AT_NVLD, "AT_NVLD - Attribute invalid");
    error_descriptions.put(AT_NDEF, "AT_NDEF - Attribute not defined");
    error_descriptions.put(SI_DUP, "SI_DUP - Schema instance duplicate");
    error_descriptions.put(SI_NEXS, "SI_NEXS - Schema instance does not exist");
    error_descriptions.put(EI_NEXS, "EI_NEXS - Entity instance does not exist");
    error_descriptions.put(EI_NAVL, "EI_NAVL - Entity instance not available");
    error_descriptions.put(EI_NVLD, "EI_NVLD - Entity instance invalid");
    error_descriptions.put(EI_NEXP, "EI_NEXP - Entity instance not exported");
    error_descriptions.put(AI_NEXS, "AI_NEXS - Aggregate instance does not exist");
    error_descriptions.put(AI_NVLD, "AI_NVLD - Aggregate instance invalid");
    error_descriptions.put(AI_NSET, "AI_NSET - Aggregate instance is empty");
    error_descriptions.put(VA_NVLD, "VA_NVLD - Value invalid");
    error_descriptions.put(VA_NEXS, "VA_NEXS - Value does not exist");
    error_descriptions.put(VA_NSET, "VA_NSET - Value not set");
    error_descriptions.put(VT_NVLD, "VT_NVLD - Value type invalid");
    error_descriptions.put(IR_NEXS, "IR_NEXS - Iterator does not exist");
    error_descriptions.put(IR_NSET, "IR_NSET - Current member is not defined");
    error_descriptions.put(IX_NVLD, "IX_NVLD - Index invalid");
    error_descriptions.put(ER_NSET, "ER_NSET - Event recording not set");
    error_descriptions.put(OP_NVLD, "OP_NVLD - Operator invalid");
    error_descriptions.put(FN_NAVL, "FN_NAVL - Function not available");
    error_descriptions.put(LC_NVLD, "LC_NVLD - Location invalid");
    error_descriptions.put(RP_RO, "RP_RO - read-only access to repository");

    error_descriptions.put(SI_LOCK, "SI_LOCK  - Schema instance locked by another user");
    error_descriptions.put(MO_LOCK, "MO_LOCK - Model locked by another user");
    error_descriptions.put(RP_LOCK, "RP_LOCK - Repository locked by another user");
    error_descriptions.put(SS_RO, "SS_RO - read-only access to remote session");

    error_descriptions.put(SY_ERR, "SY_ERR - Underlying system error");
    error_descriptions.put(SY_SEC, "SY_SEC - Security violation");
    error_descriptions.put(SY_LOC, "SY_LOC - Could not obtain lock");
  }
