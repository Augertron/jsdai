...
class Create_instance {
  ...
  /**
   * An instance of either class <code>PhFileReader</code> or class <code>Value</code> to which this class is attached.
   */
...
//	private PhFileReader myReader;
   ...
   * available entity is taken and its instance is created. The parameter <code>model_app</code> gives the model into which the
   * new instance
   ...
   */
  CEntity object(byte[] key, int key_length, SdaiModel model, SdaiModel model_app, long instance_identifier, int items_count, int mark) throws SdaiException,
    ...
    if (staticFields.short_names) {
      ...
      if (SdaiSession.debug2) {
        String ss = new String(key, 0, key_length);
        System.out.println("   FINDING: " + ss + "   index= " + result + " entity: " + schemaData.entities[result].getName(null) + "   ind = "
            + schemaData.entities[result].index + "    " + schemaData.entities[result]);
        for (int i = 0; i < schemaData.noOfEntityDataTypes; i++) {
          System.out.println(" index = " + i + "    *****NORMAL NAME: " + schemaData.sNames[i] + "   LONG NAME: " + schemaData.sLongNames[i]);
        }
        System.out.println();
      }
    }
    if (result < 0) {
      ...
      if (mark > 1) {
        ...
        if (new_key == null) {
          ...
          CEntity entity_returned = object(new_key, key_length, model, model_app, instance_identifier, items_count, -1);
      ...
      if (try_to_convert) {
        ...
        if (new_index < 0) {
          ...
          if (inst != null) {
            EntityValue.printWarningToLogo(model_app.repository.session, AdditionalMessages.RD_MIPE, instance_identifier);
            return inst;
          }
          failure = true;
        }
          ...
          if (model_app.getSubMode() == SdaiModel.MODE_SUBMODE_PARTIAL
              && (model_app.sim_status[new_index] & SdaiModel.SIM_LOADED_MASK) == SdaiModel.SIM_LOADED_NONE) {

            model_app.sim_status[new_index] = (short) ((model_app.sim_status[new_index] & ~SdaiModel.SIM_LOADED_MASK) | SdaiModel.SIM_LOADED_PARTIAL);
          }
          return inst;
        }
      }
      if (failure) {
        ...
        if (result >= 0) {
          ...
          AdditionalMessages.printWarningToLogo(model_app.repository.session, AdditionalMessages.RD_CONF, instance_identifier, new String(key, 0,
              key_length), cor_ent_name, new String(schemaData.bLongNames[ind_long]));
          ...
          for (int i = 0; i < items_count; i++) {
            if (staticFields.complex_name[i] == null) {
              entity_type_name = new String(complex_name_byte[i].entity_name, 0, complex_name_byte[i].length_of_entity_name);
              if (first) {
          ...
          message += ".";
          AdditionalMessages.printWarningToLogo(model_app.repository.session, message, instance_identifier, new String(key, 0, key_length));
          return null;
        }
        else {
          rd.inst_is_required = false;
...
//}System.out.println();
        AdditionalMessages.printWarningToLogo(model_app.repository.session, AdditionalMessages.RD_ENNF, instance_identifier, new String(key, 0,
            key_length));
        return null;
      }
    }
...
//System.out.println("  Create_instance   entity wanted: " + entity_name111);
    Class class_found = schemaData.getEntityClassByIndex(true_index);
    if (SdaiSession.debug2) {
      System.out.println("  CLASS FOUND: " + class_found.getName());
    }
    CEntity inst = schemaData.super_inst.makeInstance(class_found, model_app, result, 0);
    if (model_app.getSubMode() == SdaiModel.MODE_SUBMODE_PARTIAL && (model_app.sim_status[result] & SdaiModel.SIM_LOADED_MASK) == SdaiModel.SIM_LOADED_NONE) {

      model_app.sim_status[result] = (short) ((model_app.sim_status[result] & ~SdaiModel.SIM_LOADED_MASK) | SdaiModel.SIM_LOADED_PARTIAL);
    }
    return inst;
  }

  private CEntity recoverMissingPartialEntities(StaticFields staticFields, SdaiModel model, SdaiModel model_app, long instance_identifier, int items_count)
...
//System.out.println("Create_instance ...................aaaaa  VIOLA on instance: #" + instance_identifier);
    for (m = 0; m < items_count; m++) {
      int result;
      if (staticFields.short_names) {
        result = find_class_short(0, schemaData.noOfShortNames - 1, complex_name[m].entity_name, complex_name[m].length_of_entity_name, model);
        if (result >= 0) {
          result = schemaData.fromShort[result];
        }
        else {
          result = find_class(0, schemaData.noOfEntityDataTypes - 1, complex_name[m].entity_name, complex_name[m].length_of_entity_name, model);
          if (result >= 0) {
            result = schemaData.fromLong[result];
            staticFields.short_names = false;
          }
...
//"    def2: " + ((CEntity_definition)complex_name[j].def).getName(null));
      ...
      byte[] new_name = rd.extendComplexName(model_app, items_count, staticFields.sep_defs, staticFields.sep_def_inds, sep_count, instance_identifier);
      if (new_name == null) {
...
//System.out.println("Create_instance   new_name: " + str);
      CEntity entity_returned = object(new_name, rd.enl_instance_name_length, model, model_app, instance_identifier, items_count, -2);
      if (entity_returned == null) {
  ...
  boolean findPartialEntity(PartialEntityName[] complex_name, int items_count, CEntityDefinition partial_def) throws SdaiException {
    for (int i = 0; i < items_count; i++) {
  ...
  boolean identifyAsPartialEntity(CEntityDefinition def, CEntityDefinition partial_def) throws SdaiException {
  ...
  boolean checkSupertypes(PartialEntityName[] complex_name, int items_count, CEntityDefinition def, CEntityDefinition partial_def) throws SdaiException {
    for (int i = 0; i < def.noOfPartialEntityTypes; i++) {
      if (def == def.partialEntityTypes[i]) {
        continue;
      }
      if (findPartialEntity(complex_name, items_count, def.partialEntityTypes[i]) && identifyAsPartialEntity(def.partialEntityTypes[i], partial_def)) {
        return true;
      }
    }
    return false;
   ...
   */
  private int convertToSimpleEntity(StaticFields staticFields, SdaiModel model, int items_count) throws SdaiException {
    PartialEntityName[] complex_name;
    if (owner instanceof PhFileReader) {
...
//count_of_partial_def + "   items_count = " + items_count);
    ...
    for (int i = 0; i < count_of_partial_def; i++) {
      boolean found = false;
      for (int m = 0; m < items_count; m++) {
        if (!complex_name[m].mark && def_selected.partialEntityTypes[i] == complex_name[m].def) {
          found = true;
...
//System.out.println("Create_instance   sss: " + sss);
      if (staticFields.short_names) {
        res_index = find_class_short(0, schemaData.noOfEntityDataTypes - 1, complex_name_byte[i].entity_name,
        ...
        res_index = find_class(0, schemaData.noOfEntityDataTypes - 1, complex_name_byte[i].entity_name, complex_name_byte[i].length_of_entity_name,
            model);
        if (res_index >= 0) {
      ...
      if (res_index < 0) {
        AdditionalMessages.printWarningToLogo(session, AdditionalMessages.RD_ENNF, instance_identifier, new String(complex_name_byte[i].entity_name, 0,
            complex_name_byte[i].length_of_entity_name));
...
//			if (def.noOfPartialEntityTypes <= 0) {
      ...
      for (j = 0; j < items_count; j++) {
        ...
        res_index = def.find_partial_entity_upper_case(0, def.noOfPartialEntityTypes - 1, staticFields.complex_name[j]);
        if (res_index >= 0) {
   ...
   * through the last parameter. The name of the entity is given by parameters <code>key</code> and <code>key_length</code>. The
   * array of entities is
  ...
  CEntity create_inst_expr(byte[] key, int key_length, SdaiModel model, SdaiModel model_app, long instance_identifier, int items_count, Value val)
      throws SdaiException, java.lang.IllegalAccessException, java.lang.InstantiationException {
