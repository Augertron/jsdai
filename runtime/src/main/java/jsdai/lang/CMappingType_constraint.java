...
public class CMappingType_constraint extends CConstraint implements MappingConstraintPath, MappingConstraintMatcher {
  ...
  public void mapUsersBackward(ObjectMapping mappingContext, Set instances) throws SdaiException {
    ...
    if (selfInterface.testConstraints(null)) {
      ((MappingConstraintPath) selfInterface.getConstraints(null)).mapUsersBackward(mappingContext, instances);
    }
    EEntity_definition domain = selfInterface.getDomain(null);
    //FIXME: test type of instances in the Set
  }

  private MatcherInstances generalFind(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt, boolean path) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      if (selfInterface.testConstraints(null)) {
        ...
        instances = path ? childMatcher.findPathForward(mappingContext, instances, false) : childMatcher.findForward(mappingContext, instances, false);
        dependentSelfInstances = new EEntity[] { constraints };
      }
      EEntity_definition domain = selfInterface.getDomain(null);
      outInstances = this instanceof EExact_type_constraint ? instances.extractExactType(mappingContext, domain) : instances.extractType(mappingContext,
          domain);
      mappingContext.setCacheInstances(this, instances, outInstances, decCacheUseCnt, dependentSelfInstances);
      return outInstances;
    }
  }
  ...
  public EEntity[] findDependentInstances() throws SdaiException {
    EType_constraint selfInterface = (EType_constraint) this;
    return selfInterface.testConstraints(null) ? new EEntity[] { selfInterface.getConstraints(null) } : null;
  }
