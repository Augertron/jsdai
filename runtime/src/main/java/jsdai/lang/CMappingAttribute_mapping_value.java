...
public abstract class CMappingAttribute_mapping_value extends CAttribute_mapping_value implements MappingConstraintMatcher {
  ...
  public MatcherInstances findForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      EAttribute_mapping_value selfInterface = this;

      if (selfInterface.testConstraints(null)) {
        CachedInstances cacheInstances = mappingContext.getCacheInstances(this);
        outInstances = ((MappingConstraintMatcher) selfInterface.getConstraints(null)).findForward(mappingContext, instances, cacheInstances == null);
      }
      else {
      ...
      outInstances = outInstances.dup(new FixedValueMap(outInstanceMapKeys, getMappedValueObject()), outInstances.status);
  ...
  public MatcherInstances findPathForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    return findForward(mappingContext, instances.dup(null, MatcherInstances.STATUS_FORWARD), false);
  }
