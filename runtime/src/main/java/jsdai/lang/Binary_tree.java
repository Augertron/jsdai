...
class Binary_tree {
   ...
   * representing the new object is attached to <code>insertion_node</code> in the tree.
   */
   ...
   * to an instance whose encoding is missing. The method is invoked in <code>PhFileReader</code> after the exchange structure
   * has been read.
   */
   ...
   * this node is saved as <code>insertion_node</code> and <code>true</code> is returned.
   */
  private boolean insert_internal(Node current_node, long index) throws SdaiException {
    ...
    if (current_node == null) {
      ...
      new_index = ((CEntity) current_node.content).instance_identifier;
      if (index == new_index) {
        insertion_node = current_node;
        insertion_side = 0;
        return true;
...
//System.out.println(" Binary_tree    check successfull for CREATED inst: #" + given_index);
   ...
   * instance is replaced by the submitted (through the parameter) object of <code>CEntity</code>. The latter is just created
   * upon finding an encoding
