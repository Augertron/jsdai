...
//		} // syncObject
  ...
  final protected void changeInverseReferences(InverseEntity old, InverseEntity newer, boolean print_to_logo, boolean save4undo) throws SdaiException {
//		synchronized (syncObject) {
        ...
        if (inst != old || allow_self_refs) {
          if (inst instanceof CEntity) {
            ...
            if (ref_owner != null && ref_owner.mode == SdaiModel.READ_ONLY) {
              String base = SdaiSession.line_separator + AdditionalMessages.EI_ROMD + SdaiSession.line_separator + AdditionalMessages.RD_MODL
                  + ref_owner.name + SdaiSession.line_separator + AdditionalMessages.RD_INST + instance.instance_identifier;
...
//}
            ...
            if (print_to_logo) {
              if (old_inst.owning_model != null) {
                printWarningToLogo(old_inst.owning_model.repository.session, AdditionalMessages.RD_MINS, ((CEntity) old).instance_identifier,
                    instance.instance_identifier);
              }
              else {
                printWarningToLogo(null, AdditionalMessages.RD_MINS, ((CEntity) old).instance_identifier, instance.instance_identifier);
              }
            }
          }
          else {
  ...
  final protected void unsetInverseReferences(boolean replace_by_connector, boolean aborting) throws SdaiException {
    ...
    while (o != null) {
      ...
      if (o instanceof Inverse) {
        inv = (Inverse) o;
        inst = (CEntity) inv.value;
        if (inst.owning_model != null && inst.owning_model != old_inst.owning_model && !inst.owning_model.closingAll) {
          if (aborting && inst.owning_model.modified) {
          }
          else if (replace_by_connector) {
            con = inst.owning_model.newConnector(old_inst.owning_model, ((CEntityDefinition) old_inst.getInstanceType()).getCorrectName(),
      ...
      if (o instanceof Inverse) {
        ...
        if (own_mod != null && own_mod.repository != repo) {
          if (delete_repo) {
            ...
            con = own_mod.newConnector(inv_owner.owning_model, ((CEntityDefinition) inv_owner.getInstanceType()).getCorrectName(),
...
//		} // syncObject
   ...
   */
  private Value processInverseReference(CEntity ref_owner, CEntity old, CEntity_definition new_def) throws SdaiException {
    CEntityDefinition owner_def = (CEntityDefinition) ref_owner.getInstanceType();
    StaticFields staticFields = StaticFields.get();
    if (ref_owner == null) {
      String base = SdaiSession.line_separator + "Entity instance user not found." + SdaiSession.line_separator + "   Instance to be substituted: " + old
       ...
       * old.printInverses();
       */
      String base = SdaiSession.line_separator + "Entity instance user is invalid." + SdaiSession.line_separator + "   Instance to be substituted: "
          + old + SdaiSession.line_separator + "   Its user's persistent label: " + ref_owner.instance_identifier + SdaiSession.line_separator
...
//System.out.println("InverseEntity ********  res.reference: " + res.reference);
  ...
  static void printWarningToLogo(SdaiSession session, String text, long inst_ident, long referencing_inst_ident) throws SdaiException {
...
//				AdditionalMessages.RD_PHFI + PhFileReader.phys_file + SdaiSession.line_separator +
          AdditionalMessages.RD_INSM + inst_ident + SdaiSession.line_separator + AdditionalMessages.RD_INSR + referencing_inst_ident);
    }
    else {
  ...
  final void printInverses() {
    ...
    while (o != null) {
      if (o instanceof Inverse) {
        ...
        System.out.println("InverseEntity Place 1  inst: #" + inst.instance_identifier + "   its model: " + inst.owning_model);
//				System.out.println("\t instance: " + inv.value);
        o = inv.next;
      }
      else {
        CEntity inst = (CEntity) o;
        System.out.println("InverseEntity Place 2  inst: #" + inst.instance_identifier + "   its model: " + inst.owning_model);
  ...
  final protected void checkInverses() throws SdaiException {
    ...
    while (o != null) {
      if (o instanceof Inverse) {
        ...
        InverseEntity inst = (InverseEntity) inv.value;
        if (inst == null) {
          System.out.println("InverseEntity Place 1  inst is NULL    this instance: " + old_inst + "   its model: " + old_inst.owning_model.name);
        }
        o = inv.next;
      }
      else {
        InverseEntity inst = (InverseEntity) o;
        if (inst == null) {
          System.out.println("InverseEntity Place 2  inst is NULL    this instance: " + old_inst + "   its model: " + old_inst.owning_model.name);
        }
        o = null;
      }
    }
