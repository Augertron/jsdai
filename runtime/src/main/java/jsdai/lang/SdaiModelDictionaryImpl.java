...
class SdaiModelDictionaryImpl extends SdaiModel {
  ...
  protected boolean committingHeaderInternal(boolean commit_bridge, boolean modif_remote) throws SdaiException {
    throw new SdaiException(SdaiException.SS_NAVL, "This method can not be used with system repostory");
  }
...
//		synchronized (syncObject) {
    throw new SdaiException(SdaiException.FN_NAVL, "The system repository cannot be modified by the user.");
//		} // syncObject
  }

  public void startPartialReadOnlyAccess() throws SdaiException {
    throw new SdaiException(SdaiException.FN_NAVL, "Partial access is not available for dictionary model " + name);
  }
...
//		synchronized (syncObject) {
    throw new SdaiException(SdaiException.FN_NAVL, "A model in the system repository cannot be closed by the user.");
//		} // syncObject
  }
...
//		long time = System.currentTimeMillis();
    ...
    if (istream == null) {
      ...
      String text = SdaiSession.line_separator + AdditionalMessages.BF_DDM1 + sch_name + AdditionalMessages.BF_DDM2 + " (" + full_name + ")";
...
//"   m.schemaData.noOfEntityDataTypes = " + m.schemaData.noOfEntityDataTypes);
      if (m != SdaiSession.baseDictionaryModel && m.schemaData.noOfEntityDataTypes < 0) {
        ...
        if (m != SdaiSession.baseComplexModel) {
          count += m.lengths[SdaiSession.ENTITY_DECL_REFERENCED_DECL] + m.lengths[SdaiSession.ENTITY_DECL_USED_DECL]
...
//		time3 = System.currentTimeMillis();
    for (i = 0; i < iInterfacedModel; i++) {
      m = aInterfacedModel[i];
      if (m != SdaiSession.baseDictionaryModel && !(m.early_binding_linked_init) && m.described_schema != null) {
        if (m == this) {
...
//		time4 = System.currentTimeMillis();
    for (i = 0; i < iInterfacedModel; i++) {
      m = aInterfacedModel[i];
      if (m != SdaiSession.baseDictionaryModel && !(m.early_binding_linked) && m.described_schema != null) {
        if (m == this) {
...
//(time5 - time1)/1000 + " sec");
  ...
  protected Connector newConnector(SdaiRepository repository, long modelId, long instanceIdentifier, CEntity owningInstance) throws SdaiException {
    throw new SdaiException(SdaiException.SY_ERR, "Dictionary model can not handle this type of connector");
  }
