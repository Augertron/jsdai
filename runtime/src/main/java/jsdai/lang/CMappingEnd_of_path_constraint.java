...
public class CMappingEnd_of_path_constraint extends CConstraint implements MappingConstraintPath, MappingConstraintMatcher {
  ...
  public void mapUsersBackward(ObjectMapping mappingContext, Set instances) throws SdaiException {
    throw new SdaiException(SdaiException.FN_NAVL, "End of path constraint can not be called as part of backward references");
  }
  ...
  public MatcherInstances findForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    throw new SdaiException(SdaiException.FN_NAVL, "This constraint can not be called as part of forward references");
  }
  ...
  public MatcherInstances findPathForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      if (cachedInstances != null) {
        constraintsOutInstances = cachedInstances.getChildCacheInstances(constraints, allInInstances);
      }
      if (constraintsOutInstances == null) {
        constraintsOutInstances = ((MappingConstraintMatcher) constraints).findForward(mappingContext, allInInstances, true);
        foundConstraintsOutInstances = true;
      }
      outInstances = instances.intersect(constraintsOutInstances);
      cachedInstances = mappingContext.setCacheInstances(this, instances, outInstances, decCacheUseCnt);
      if (foundConstraintsOutInstances) {
        cachedInstances.addChildCacheInstances(constraints, allInInstances, constraintsOutInstances);
      }
      return outInstances;
    }
  }
