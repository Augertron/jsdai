...
/**
 * An <code>SdaiRepository</code> is the <b>physical container</b> to store persistently: <br>
 * - <code>SdaiModel</code>s with entity instances within them; <br>
 * - <code>SchemaInstance</code>s which logically group <code>SdaiModel</code>s. <br>
 * Throughout this documentation repository is used as a synonym for <code>SdaiRepository</code>.
 * <p>
 * <p>
 * Conceptionally, the <code>SdaiRepository</code> can be represented by the following EXPRESS entity:
 *
 * <p>
 * <pre>ENTITY SdaiRepository;
 *  {@link #getName name} : String;
 *  {@link #getLocation location} : OPTIONAL Object;
 *  {@link #getDescription description} : {@link A_string LIST} [1:?] OF String;
 *  {@link #getModels models}       : {@link ASdaiModel SET} [0:?] OF {@link SdaiModel SdaiModel};
 *  {@link #getSchemas schemas}      : {@link ASchemaInstance SET} [0:?] OF {@link SchemaInstance SchemaInstance};
 *  {@link #getChangeDate changeDate} : String;
 *  {@link #getAuthor author}       : {@link A_string LIST} [1:?] OF String;
 *  {@link #getOrganization organization}       : {@link A_string LIST} [1:?] OF String;
 ...
 * INVERSE
 *  {@link #getSession session}  : {@link SdaiSession SdaiSession} FOR {@link SdaiSession#getKnownServers knownServers};
 * UNIQUE
 *  UR1: name, session;
 * END_ENTITY;</pre>
 * <p>
 * <p>
 * The attributes <code>description</code>, <code>author</code>, <code>organization</code>, <code>defaultLanguage</code> and
 * <code>contextIdentifiers</code> are read-write, whereas all other attributes are read-only. For each attribute a corresponding
 * get method (or methods in the case of <code>changeDate</code>) is defined. The <code>changeDate</code> can be retrieved either
 * as <code>String</code> or as <code>long</code>. The attributes <code>preprocessorVersion</code>, <code>originatingSystem</code>, <code>authorization</code> and <code>defaultLanguage</code> also have corresponding set methods.
 *
 * <p>
 * The <strong>persistent storage</strong> is realized through the special directories containing binary data files. For every
 * <code>SdaiRepository</code> a corresponding directory with the <code>SdaiRepository</code> name exists. By default these
 * directories are placed into a special repositories-directory in the local file system. This is specified by the property
 * "repositories" in the configuration file jsdai.properties.
 * <p>
 * <B>Example:</B>
 * <p>
 * <pre>    repositories=D:\\repos</pre> Other locations are also possible. A user should never manipulate these
 * directories and binary data files. An access to them is allowed only through the JSDAI methods.
 * <p>
 * In general, the repositories can be accessed from either the local file system or remotely from a JSDAI-Server. After opening a
 * session all repositories are closed, except a special one named "SystemRepository" which contains dictionary and mapping data.
 * Each model in this repository can be at most read-only, that is, an attempt to set read-write mode to such models will lead to
 * an <code>SdaiException</code>. Repositories can be opened with openRepository() and closed with closeRepository() method.
 * <p>
 * Repositories are mainly selected by SdaiSession.getKnownServers() to get all available repositories or by
 * SdaiSession.getActiveServers() to get only the currently open ones. Initially, after openSession(), the set returned by
 * getKnownServers() contains all <code>SdaiRepositories</code> located in the special repositories-directory. The remote
 * repositories or repositories located at other places on the file system can be added to the known servers with linkRepository()
 * method. New repositories are created with the <code>SdaiSession</code> methods createRepository() and
 * importClearTextEncoding(). No longer needed repositories are removed with deleteRepository() method.
 * <p>
 * Each repository has a unique non-empty <strong>name</strong> for identification. The name is assigned by createRepository() or
 * importClearTextEncoding() methods and cannot be changed afterwards. During creation of a repository an empty string can be
 * passed to the name parameter to signal JSDAI to create a temporary repository. The name of the local repository can consist
 * only of the following characters: upper case letters from 'A' to 'Z', lower case letters from 'a' to 'z', digits '0', '1',...,
 * '9' and underscore '_'. Moreover, the name must start from a letter. If in importClearTextEncoding() method the repository name
 * is extracted from the header of the imported file, this name is corrected to conform the above rules. In the case of the remote
 * repository, its name, in addition, is allowed to contain the following characters: "file://", "/" and ".".
 * <p>
 * The repositories need to be <strong>located</strong> somewhere. The location is either <br>
 * - local in the default repositories-directory; <br>
 * - local in the specified directory as <a href="../../../guides/SDAIFile.html">SDAI file</a>; <br>
 * - remote on a JSDAI-Server; <br>
 * - unknown in the case of a degenerated repository (see below).
 * <p>
 * The actual location can be accessed with getLocation() method. By default the repositories are created locally in the specific
 * repositories-directory. But it is possible to put repositories in other places, either locally or remotely. Locally, on an
 * available file system by specifying a path during creation, or remotely by specifying the domain or INET of the JSDAI-Server.
 * However, only repositories located in the default repositories-directory are by default available after openSession(). The
 * location of a repository cannot be changed once it is created except for that the location gots invalid.
 * <p>
 * It is not possible to create a repository on an unknown location. But a repository may contain instances which reference other
 * instances in other repositories which do not belong to <code>knownServers</code>. <br>
 * In this case only the name of the referenced repositories is known, but not their location. We call them degenerated
 * repositories. Trying to open a degenerated repository results in the <code>SdaiException</code>
 * "RP_NAVL - Repository not available". If getLocation() is applied to such a repository, then the <code>SdaiException</code>
 * "VA_NEXS - Value does not exist" is thrown. The degenerated repositories can be promoted to ordinary repositories with
 * importClearTextEncoding() or linkRepository() methods by using the same repository name.
 * <p>
 * With linkRepository() method it is possible to link local or remote repositories to the current session. With
 * unlinkRepository() closed and for the user not needed repositories can be made unknown to the current session.
 * <p>
 * The methods createRepository(), importClearTextEncoding() and linkRepository() have parameters for the repository name and the
 * repository location. The following combinations of values for these parameters are allowed:
 * <p>
 * 1) null, xxx : This is possible only for importClearTextEncoding() method. If this case happens, the repository name is taken
 * from the entity <code>file_name</code> in the header of an exchange structure from which the encoding is imported.
 * <p>
 * 2) String1, xxx: String1 is used as the repository name.
 * <p>
 * 3) "", null : A temporary repository is created with an artificial name. Temporary repositories are automatically removed
 * during closeSession() and are not available after the next openSession().
 * <p>
 * 4) xxx, String2: The repository is created at the location indicated by String2. If String2 ends with ".sdai" then repository
 * stored in <a href="../../../guides/SDAIFile.html">SDAI file</a> is created. If String2 ends with ".sdai" and the first
 * parameter is null or empty string then repository name is derived from <a href="../../../guides/SDAIFile.html">SDAI file</a>
 * name.
 * <p>
 * 5) String1, String2: String1 is the name of a remote repository. String2 is the username and password with the prefix "//" and
 * the last character "@". Example: "//guest:passwd@". A server for remote access should be named by prefixing it with "//" and
 * giving the domain name. Example: Repository "test1" on the domain "lksoft.de" has to be named "//test1.lksoft.de".
 * <p>
 * Repositories have two sets of attributes: header properties and contents. <br>
 * The header properties are the following: <br>
 * - description <br>
 * - time_stamp <br>
 * - author <br>
 * - organization <br>
 * - preprocessor_version <br>
 * - originating_system <br>
 * - authorization <br>
 * - default_language <br>
 * - context_identifiers. <br>
 * The contents consists of: <br>
 * - models <br>
 * - schemas.
 * <p>
 * To access the header properties for reading, the repository must be opened. To modify the header attributes, a read-write
 * transaction has to be started additionally.
 * <p>
 * To access the contents data, the repository must be opened and either a read-only or a read-write transaction has to be
 * started. When transaction access is ended, the contents of a repository is no longer available. The repository contents cannot
 * be manipulated directly by an application. Manipulations are only possible with the following methods: <br>
 * - createSdaiModel() <br>
 * - deleteSdaiModel() <br>
 * - renameSdaiModel() <br>
 * - createSchemaInstance() <br>
 * - delete() (in class <code>SchemaInstance</code>).
 * <p>
 *
...
// End of a new code due to ejbclient
   ...
   */
  protected boolean created_or_imported;
  ...
  void setNameLocation(String name, Object location) throws SdaiException {
    ...
    for (int i = 0; i < session.known_servers.myLength; i++) {
      ...
      if (rep != this && rep.name.equals(name)) {
        throw new SdaiException(SdaiException.RP_DUP, rep, AdditionalMessages.AX_NAME + name);
      }
    }
    if (location != null) {
...
//			dir_name = "r" + session.repo_count;
   ...
   *
   * @param session
   * @param fCheckNameLocation
   * @param name
   * @param location
   */
  protected SdaiRepository(SdaiSession session, String name, Object location, boolean fCheckNameLocation) throws SdaiException {
  ...
  protected void initLocal(SdaiSession session, String name, Object location, boolean fCheckNameLocation) throws SdaiException {
...
//		} // syncObject
   ...
   * <p>
   * The repositories known to an <code>SdaiSession</code> must have unique names. An exception will be thrown if the repository
   * is deleted or if the <code>SdaiSession</code> is closed.
   *
   ...
   * <p>
   * The repositories within a <code>SdaiSession</code> have unique identifiers, which are not reused when some repositories are
   * deleted and some new created or imported.
   *
   ...
   * This is either <br>
   * - null for unknown location or <br>
   * - a path within a locally available file system or <br>
   * - when starting with the prefix "//", the INET or domain of a remote JSDAI-Server.
...
//		} // syncObject
   ...
   * <p>
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} this aggregate is
   * always empty, whereas after invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} it contains
   * strings taken from the field <code>description</code> of the entity <code>file_description</code> in the header of the
   * exchange structure (see ISO 10303-21::8.2.1 file_description). That is, this aggregate always exists, but may be empty.
   * During a read-write transaction the aggregate can be modified by adding/modifying/removing its members. When applying
   * {@link #exportClearTextEncoding exportClearTextEncoding}, the content of this aggregate will be written to the exchange
   * structure created by the method being applied. The method is available only if the repository is open.
   *
   ...
   * The aggregate is automatically updated when new models are created with {@link #createSdaiModel createSdaiModel} or removed
   * by {@link SdaiModel#deleteSdaiModel deleteSdaiModel}.
   ...
   * created with {@link #createSchemaInstance createSchemaInstance} or removed by {@link SchemaInstance#delete delete}.
   ...
   * <p>
   * The time stamp is updated each time when {@link SdaiTransaction#commit commit} method is applied, provided either the
   * repository has been modified since the most recent commit or abort operation was performed, or no such operation was
   * performed thus far but the repository was established by {@link SdaiSession#createRepository createRepository} method. If,
   * however, for the repository no such case has appeared, then the time stamp is set as follows. If the repository was
   * introduced by {@link SdaiSession#createRepository createRepository} method, then the time of its creation is taken. The
   * same rule applies also to the "SystemRepository". If the repository was created using
   * {@link SdaiSession#importClearTextEncoding importClearTextEncoding} method, then the time stamp is borrowed from the entity
   * <code>file_name</code> in the header of an exchange structure from which the encoding was imported. Finally, if the
   * repository was established using {@link SdaiSession#linkRepository linkRepository} method, then the time stamp is extracted
   * from the data of the repository linked to the session. The method is available only if the repository is open.
   * <p>
   * <B>Example of the time stamp:</B>
   * <p>
   * <pre>    Calendar date: 28 June 2000
   *    Time of the day: 15 minutes 12 seconds past 17 hours
   *    Above date and time encoded according to ISO 8601:
   *        2000-06-28T17:15:12</pre>
   *
   ...
   * This value is updated each time when {@link SdaiTransaction#commit commit} method is applied,
   ...
   * established by {@link SdaiSession#createRepository createRepository} method. If, however, for the repository no such case
   * has appeared,
   * then this value is set as follows.
   * If the repository was introduced by {@link SdaiSession#createRepository createRepository} method, then
   * the time of its creation is taken. The same rule applies also to
   * the "SystemRepository". If the repository was created using {@link SdaiSession#importClearTextEncoding
   * importClearTextEncoding} method,
   * then the date is borrowed from the entity <code>file_name</code> in the header of an exchange structure from
   * which the encoding was imported.
   * Finally, if the repository was established using {@link SdaiSession#linkRepository linkRepository} method, then
   * the date is extracted from the data of the repository linked to
   * the session.
   * <p>
   * If time value is obtained as a string from an exchange structure processed by {@link SdaiSession#importClearTextEncoding
   * importClearTextEncoding} method and this string violates the rules for time stamps stated in "ISO 10303-21::8.2.2", then
   * SdaiException VA_NVLD is thrown. The method is available only if the repository is open.
   *
...
//			} // syncObject
   ...
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} this aggregate is
   * always empty, whereas after invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} it contains
   * strings taken from the field <code>author</code> of the entity <code>file_name</code> in the header of the exchange
   * structure (see ISO 10303-21::8.2.2 file_name). That is, this aggregate always exists, but may be empty. During a read-write
   * transaction the aggregate can be modified by adding/modifying/removing its members. It is a responsibility of the
   * application to fill this aggregate with useful information. When applying {@link #exportClearTextEncoding
   * exportClearTextEncoding}, the content of this aggregate will be written to the exchange structure created by the method
   * being applied. The method is available only if the repository is open.
   *
   ...
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} this aggregate is
   * always empty, whereas after invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} it contains
   * strings taken from the field <code>organization</code> of the entity <code>file_name</code> in the header of the exchange
   * structure (see ISO 10303-21::8.2.2 file_name). That is, this aggregate always exists, but may be empty. During a read-write
   * transaction the aggregate can be modified by adding/modifying/removing its members. It is a responsibility of the
   * application to fill this aggregate with useful information. When applying {@link #exportClearTextEncoding
   * exportClearTextEncoding}, the content of this aggregate will be written to the exchange structure created by the method
   ...
   * <p>
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} some default
   * value to this string is assigned, whereas after invocation of {@link SdaiSession#importClearTextEncoding
   * importClearTextEncoding} this string has value taken from the field <code>preprocessor_version</code> of the entity
   * <code>file_name</code> in the header of the exchange structure (see ISO 10303-21::8.2.2 file_name). The method is available
   * only if the repository is open.
   *
   ...
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} some default
   * value to this string is assigned, whereas after invocation of {@link SdaiSession#importClearTextEncoding
   * importClearTextEncoding} this string has value taken from the field <code>originating_system</code> of the entity
   * <code>file_name</code> in the header of the exchange structure (see ISO 10303-21::8.2.2 file_name). The method is available
   * only if the repository is open.
   *
   ...
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} some default
   * value to this string is assigned, whereas after invocation of {@link SdaiSession#importClearTextEncoding
   * importClearTextEncoding} this string has value taken from the field <code>authorization</code> of the entity
   * <code>file_name</code> in the header of the exchange structure (see ISO 10303-21::8.2.2 file_name). The method is available
   * only if the repository is open.
   *
   ...
   * <p>
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} this string is
   * set to null, whereas after invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this string
   * can take value extracted from the header of the exchange structure (see ISO 10303-21::8.2.4 section_language) provided an
   * appropriate value is given there (otherwise it is set to null). The method is available only if the repository is open.
   *
   ...
   * <p>
   * After invocation of <code>SdaiSession</code> method {@link SdaiSession#createRepository createRepository} this aggregate is
   * empty, whereas after invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this aggregate will
   * contain information extracted from the header of the exchange structure (see ISO 10303-21::8.2.5 section_context) provided
   * an appropriate information is given there. That is, this aggregate always exists, but may be empty. During a read-write
   * transaction the aggregate can be modified by adding/modifying/removing its members. If for at least one model in this
   * repository its own aggregate describing contexts is empty and the repository aggregate contains at least one string, then
   * when applying {@link #exportClearTextEncoding exportClearTextEncoding}, the content of this aggregate will be written to
   * the exchange structure created by the method being applied. The method is available only if the repository is open.
   *
   ...
   * For the latter alternative the parameter value shall be set to <code>true</code>. This information is used
   * by {@link #exportClearTextEncoding exportClearTextEncoding} and {@link CEntity#toString CEntity.toString} methods.
   ...
   * schema. Otherwise, if schema instance could be created, it returns <code>true</code>.
   *
   ...
   * on remote repository. This method always returns <code>null</code> for local repository.
   *
  ...
  /**
   * Makes the <code>SdaiModels</code> and <code>SchemaInstances</code> contained in this repository available for subsequent
   * access.
  ...
  /*
   * @Todo: these methods should go to SdaiRepositoryBase of jsdai.clientbase package..
   *
   * @throws SdaiException
   */
  protected abstract boolean updateRemoteModel(SdaiModel model, String name, SdaiModelRemote modRem) throws
      SdaiException;
  ...
  protected boolean openRepositoryRemote() throws SdaiException {
    SdaiSession session = getSession();
    ...
    return committed || !created_or_imported;
  }
   ...
   */
  public abstract SdaiModel createSdaiModel(String model_name, ESchema_definition schema) throws SdaiException;

  protected boolean createSdaiModelCommonChecking(String model_name, ESchema_definition schema, boolean remote, int transactionMode) throws SdaiException {
    if (session == null) {
      throw new SdaiException(SdaiException.RP_NEXS);
    }
  ...
  protected SdaiModel createSdaiModelCommonCreating(String model_name, ESchema_definition schema, boolean remote) throws SdaiException {
   ...
   * @see "ISO 10303-22::10.5.1 Create SDAI-model"
   */
  public abstract SdaiModel createSdaiModel(String model_name, Class schema) throws SdaiException;
...
// during loading of binary files are found).
  ...
  protected SdaiModel createOrPromoteModelRemote(SdaiModel model, SdaiModelHeader modelHeader) throws SdaiException {
    ...
    if (model != null) {
      ...
      try {
        createSdaiModelCommonChecking(modelHeader.name, dictionary.described_schema, true, SdaiTransaction.READ_ONLY);
...
// 				model = createSdaiModelCommon(modelHeader.name, dictionary.described_schema, true);
   ...
   */
  public abstract SchemaInstance createSchemaInstance(String name, ESchema_definition schema) throws SdaiException;

  protected boolean createSchemaInstanceCommon(String name, ESchema_definition schema, boolean remote, int transactionMode) throws SdaiException {
   ...
   * @see "ISO 10303-22::10.5.2 Create schema instance"
   */
  public abstract SchemaInstance createSchemaInstance(String name, Class schema) throws SdaiException;
...
// in the constructor of SdaiSession.
   ...
   * After closing a repository, <code>SdaiModels</code> and <code>SchemaInstances</code> contained in it become inaccessible.
   * In turn, entity instances of any <code>SdaiModel</code> in this
   * repository also become inaccessible. For most methods in class <code>EEntity</code>, if applied to such instances,
   * SdaiException EI_NEXS will be thrown.
   * The largest number appearing in the name of an instance within
   * the repository is stored by {@link SdaiTransaction#commit commit} in 'contents' binary file of the repository and will be
...
//long start, end, cumulat = 0;
    ...
    SdaiModel model;
    SchemaInstance sch;
    if (session.active_transaction != null && session.active_transaction.mode == SdaiTransaction.READ_WRITE && !delete_repo) {
...
//		} // syncObject
  ...
  protected EEntity getSessionIdentifierRemote(long identifier, int typeIndex, long modelId) throws SdaiException {
    int i;
    EEntity inst;
    SdaiModel model = null;
    if (modelId >= 0) {
      ...
      if (model != null && model.repository == null) {
        throw new SdaiException(SdaiException.EI_NEXS, "Instance identifier " + identifier + " is in deleted model " + model);
      }
    }
    ...
    if (model == null) {
      ...
      if (mod_remote == null) {
        throw new SdaiException(SdaiException.EI_NEXS, "Instance identifier " + identifier + " does not have owning model");
      }
      boolean found = false;
    ...
    if (inst == null) {
      throw new SdaiException(SdaiException.EI_NEXS, "Instance identifier " + identifier + " not found in model " + model);
    }
    return inst;
  }
   ...
   * It is not possible to get back it using {@link SdaiSession#linkRepository linkRepository} method.
...
//		} // syncObject
   ...
   * If the repository was created during this session by applying {@link SdaiSession#importClearTextEncoding
   ...
   * when the repository was either created by {@link SdaiSession#createRepository createRepository} or linked by
   ...
   * values:
   * <ul>
   * <li>"2;1" if the repository contains at most one model;
   * <li>"3;1" otherwise; in this case for each schema instance in the repository an instance of the entity
   * <code>file_population</code> in the header of the exchange structure (see ISO 10303-21::8.2.4 file_population) is created;
   * </ul>
   ...
   * are ignored. However, if all models of this repository belong to <code>associated_models</code> of the schema instance
   * considered, then
   ...
   * <p>
   * The repository may contain instances which reference instances in other repositories. In this case the strategy is as
   * follows:
   * <ul>
   * <li>if such a reference is a non-aggregate value of an attribute or belongs to an aggregate of Express type ARRAY, then
   * this reference in an exchange structure being formed is replaced by an unset value (dollar sign "$");
   * <li>if such a reference belongs to an aggregate of Express type either LIST or SET or BAG, then when writing data to an
   * exchange structure this reference is simply dropped;
   * </ul>
   * <p>
   * After execution of this method the repository itself remains unchanged. In particular, all references (if any) from the
   * current repository to other repositories are retained. The method is disabled for special repository "SystemRepository",
   * containing dictionary and mapping data. In this case SdaiException FN_NAVL is thrown.
   *
  ...
  /**
   * Creates a STEP file in a form of the exchange structure containing the data of this repository. At the moment of
   * invocation of the method the repository must be opened. If parameter <code>location</code> gets some
   * <code>String</code> value, then this value is used as a name for the resulting file. If, however,
   * <code>location</code> is null, then the name is chosen as follows. If the repository was created during this
   * session by applying {@link SdaiSession#importClearTextEncoding
   * importClearTextEncoding}, then the name of the file is the same as that of the input file for
   * importClearTextEncoding method. In the remaining cases, that is, when the repository was either created by
   * {@link SdaiSession#createRepository createRepository} or linked by
   * {@link SdaiSession#linkRepository linkRepository}, the name of the resulting file coincides with the name of the
   * repository. The second parameter of the method can be used to set the value of the attribute <code>name</code> of
   * the header entity <code>file_name</code> (see ISO 10303-21::8.2.2 file_name). If this parameter is
   * <code>null</code>, then the value of <code>name</code> depends on the origin of the repository. If it has been
   * imported from an exchange structure, then the existing value of <code>name</code> is taken. If, however, the
   * repository was created, then its name as the value of the attribute <code>name</code> is used.
   * <p>
   * <p>
   * The contents of each model of the repository is written down to the file as a separate data section. The header of
   * the file includes time-stamp which specifies the date and time when this repository was created or most recently
   * modified. The value of the attribute <code>implementation_level</code> of the entity <code>file_description</code>
   * in the header of the exchange structure (see ISO 10303-21::8.2.1 file_description) can take one of the following
   * values:
   * <ul>
   * <li>"2;1" if the repository contains at most one model;
   * <li>"3;1" otherwise; in this case for each schema instance in the repository an instance of the entity
   * <code>file_population</code> in the header of the exchange structure (see ISO 10303-21::8.2.4 file_population) is
   * created;
   * </ul>
   * During creation of an instance of <code>file_population</code> for a schema instance only those models in the set
   * <code>associated_models</code> which belong to this repository are listed. Models from the other repositories
   * simply are ignored. However, if all models of this repository belong to <code>associated_models</code> of the
   * schema instance considered, then the value standing for <code>associated_models</code> in the corresponding
   * instance of <code>file_population</code> is missing, that is, dollar sign is used (see ISO 10303-21::8.2.4
   * file_population).
   * <p>
   * The repository may contain instances which reference instances in other repositories. In this case the strategy is
   * as follows:
   * <ul>
   * <li>if such a reference is a non-aggregate value of an attribute or belongs to an aggregate of Express type ARRAY,
   * then this reference in an exchange structure being formed is replaced by an unset value (dollar sign "$");
   * <li>if such a reference belongs to an aggregate of Express type either LIST or SET or BAG, then when writing data
   * to an exchange structure this reference is simply dropped;
   * </ul>
   * <p>
   * Applying this method, it is possible to write entity names either in usual form or in short form. For example, for
   * entity <code>cartesian_point</code> any of the following cases can be chosen:
   * <ul>
   * <li>#1016=CARTESIAN_POINT('cp5',(0.0,0.0,-0.25));
   * <li>#1016=CRTPNT('cp5',(0.0,0.0,-0.25));
   * </ul>
   * To change the current alternative, {@link #shortNameSupport shortNameSupport} method should be used. Selecting the
   * short form can give the desired result only if the short names of entities are defined in the data dictionary
   * (provided by an Express compiler). If, for an entity, the method fails to identify its short name, then the full
   * entity name is used instead of.
   * <p>
   * After execution of this method the repository itself remains unchanged. In particular, all references (if any) from
   * the current repository to other repositories are retained. The method is disabled for special repository
   * "SystemRepository", containing dictionary and mapping data. In this case SdaiException FN_NAVL is thrown.
   *
   * @param location  the name of the file created by the method.
  ...
  public void exportClearTextEncoding(String location, String file_name) throws SdaiException {
    exportClearTextEncoding(location, file_name, FileImplementationLevel.AUTO);
  }
   ...
   * {@link SdaiSession#linkRepository linkRepository}, the name of the
   * resulting file coincides with the name of the repository.
   * The second parameter of the method can be used to set the value of the attribute <code>name</code> of the header entity
   * <code>file_name</code> (see ISO 10303-21::8.2.2 file_name). If this parameter is <code>null</code>,
   ...
   * was created, then its name as the value of the attribute <code>name</code> is used.
   *
   * <p>
   * The contents of each model of the repository is written down to the file as a separate data section. The header of the file
   * includes time-stamp which specifies the date and time when this repository was created or most recently
   * modified.
   * <p>
   * During creation of an instance of <code>file_population</code> for a schema instance only those models in the set
   * <code>associated_models</code> which belong to this repository are listed. Models from the other repositories simply are
   * ignored. However, if all models of this repository belong to <code>associated_models</code> of the schema instance
   * considered, then the value standing for <code>associated_models</code> in the corresponding instance of
   * <code>file_population</code> is missing, that is, dollar sign is used (see ISO 10303-21::8.2.4 file_population).
   * <p>
   * The repository may contain instances which reference instances in other repositories. In this case the strategy is as
   * follows:
   * <ul>
   ...
   * </ul>
   * <p>
   * Applying this method, it is possible to write entity names either in usual form or in short form. For example, for entity
   * <code>cartesian_point</code> any of the following cases can be chosen:
   * <ul>
   * <li>#1016=CARTESIAN_POINT('cp5',(0.0,0.0,-0.25));
   * <li>#1016=CRTPNT('cp5',(0.0,0.0,-0.25));
   * </ul>
   * To change the current alternative, {@link #shortNameSupport shortNameSupport} method should be used. Selecting the short
   * form can give the desired result only if the short names of entities are defined in the data dictionary (provided by an
   * Express compiler). If, for an entity, the method fails to identify its short name, then the full entity name is used
   * instead of.
   * <p>
   * After execution of this method the repository itself remains unchanged. In particular, all references (if any) from the
   * current repository to other repositories are retained. The method is disabled for special repository "SystemRepository",
   ...
   * @param location            the name of the file created by the method.
   * @param file_name           the name of the exchange structure to which this repository is exported.
   * @param implementationLevel the file implementation level (see see ISO 10303-21::8.2.1 file_description).
   * @throws SdaiException RP_NEXS, repository does not exist.
   * @throws SdaiException RP_NOPN, repository is not open.
   * @throws SdaiException TR_NEXS, transaction does not exist.
   * @throws SdaiException TR_NAVL, transaction currently not available.
   ...
   * @see "ISO 10303-21"
   */
  public void exportClearTextEncoding(String location, String file_name, FileImplementationLevel implementationLevel) throws
      SdaiException {
    if (SdaiSession.debug2) {
      for (int j = 0; j < models.myLength; j++) {
        SdaiModel model = (SdaiModel) models.myData[j];
        System.out.println("   SdaiRepository   *****  model: " + model.name + "   its schema_name: " + model.schema_name);
      }
    }
...
//		synchronized (syncObject) {
    ...
    try {
      ...
      writer.output_to_physical_file(output_file_name, this, file_name, implementationLevel);
      if (handle_back.exists()) {
        handle_back.delete();
      }
    }
...
//		} // syncObject
   ...
   * For more details about the exchange format see {@link #exportClearTextEncoding(String location, String file_name)}.
   * Invoking this method is equivalent to making this call: <code>exportClearTextEncoding(location, null);</code>
   *
  ...
  /**
   * Creates a STEP file in a form of the exchange structure containing the data of this repository. At the moment of
   * invocation of the method the repository must be opened. The exchange structure is written to the supplied stream.
   * For more details about the exchange format see {@link #exportClearTextEncoding(String location, String file_name)}
   * <p>
   *
   * @param location            an <code>OutputStream</code> value
   * @param file_name           the name of the exchange structure to which this repository is exported. It sets the value of the
   *                            attribute <code>name</code> of the header entity <code>file_name</code> (see ISO 10303-21::8.2.2 file_name). If
   *                            this parameter is <code>null</code>, then the value of <code>name</code> depends on the origin of the repository.
   *                            If it has been imported from an exchange structure, then the existing value of <code>name</code> is taken. If,
   *                            however, the repository was created, then its name as the value of the attribute <code>name</code> is used.
   *                            <p>
   * @param implementationLevel The implementation level and conformance class for clear text encoded file.
   *                            <p>
   * @throws SdaiException if an error occurs during the operation or in underlying JSDAI operations
   * @since 4.4.1
   */
  public void exportClearTextEncoding(OutputStream location, String file_name,
      FileImplementationLevel implementationLevel) throws SdaiException { //NOPMD
    if (session == null) {
      throw new SdaiException(SdaiException.RP_NEXS);
    }
//		synchronized (syncObject) {
    ...
    try {
      PhFileWriter writer = new PhFileWriter();
      writer.output_to_physical_file(location, this, file_name, implementationLevel);
    }
    catch (java.io.IOException ex) {
      throw new SdaiException(SdaiException.SY_ERR, ex);
    }
//		} // syncObject
   ...
   * @param location  an <code>OutputStream</code> value
   * @param file_name the name of the exchange structure to which this repository is exported.
   *                  It sets the value of the attribute <code>name</code> of the header entity <code>file_name</code> (see ISO
   *                  10303-21::8.2.2 file_name).
   *                  If this parameter is <code>null</code>, then the value of <code>name</code> depends on the origin of the repository.
   *                  If it has been
   *                  imported from an exchange structure, then the existing value of <code>name</code> is taken. If, however, the
  ...
  public void exportClearTextEncoding(OutputStream location, String file_name) throws SdaiException {
    exportClearTextEncoding(location, file_name, FileImplementationLevel.LEVEL_21);
  }
   ...
   *
   * @return <code>true</code> if this repository is open, and <code>false</code> otherwise.
   * @throws SdaiException RP_NEXS, repository does not exist.
   */
   ...
   *
   * @return <code>true</code> if this repository is modified, and <code>false</code> otherwise.
   * @throws SdaiException RP_NEXS, repository does not exist.
   * @throws SdaiException RP_NOPN, repository is not open.
   */
...
//		}
  ...
  void saveRepositoryStream(DataOutput stream) throws SdaiException {
    ...
    try {
      ...
      for (i = 1; i <= description.myLength; i++) {
        stream.writeUTF(description.getByIndex(i));
      }
      stream.writeByte('B');
      ...
      for (i = 1; i <= strings.myLength; i++) {
        stream.writeUTF(strings.getByIndex(i));
      }
      ...
      for (i = 0; i < contexts_count; i++) {
        if (contexts[i].section == null) {
          ...
          for (j = 1; j <= strings.myLength; j++) {
            stream.writeUTF(strings.getByIndex(j));
          }
          break;
        }
      }
      ...
      for (i = 0; i < models.myLength; i++) {
        ...
        if (def != null) {
          ...
          sch_name = mod.dictionary.name.substring(0, mod.dictionary.name.length() - SdaiSession.ENDING_FOR_DICT);
        }
        ...
        for (j = 0; j < contexts_count; j++) {
          ...
          if (sec_con.section != null && sec_con.section.equals(mod.name)) {
            ...
            for (int k = 1; k <= strings.myLength; k++) {
              stream.writeUTF(strings.getByIndex(k));
            }
            break;
          }
        }
      ...
      for (i = 0; i < schemas.myLength; i++) {
        ...
        if (def != null) {
          ...
          sch_name = sch.dictionary.name.substring(0, sch.dictionary.name.length() - SdaiSession.ENDING_FOR_DICT);
        }
        index2name = session.find_string(0, schemas_count - 1, sch_name, staticFields.schema_names);
        if (index2name < 0) {
          throw new SdaiException(SdaiException.SY_ERR);
        ...
        if (sch.description == null) { // new code added from here
          ...
          for (j = 1; j <= sch.description.myLength; j++) {
            stream.writeUTF(sch.description.getByIndex(j));
          }
        }
        if (sch.author == null) {
          stream.writeShort(0);
          ...
          for (j = 1; j <= sch.author.myLength; j++) {
            stream.writeUTF(sch.author.getByIndex(j));
          }
        }
        if (sch.organization == null) {
          ...
          for (j = 1; j <= sch.organization.myLength; j++) {
            stream.writeUTF(sch.organization.getByIndex(j));
          }
        }
        ...
        if (sch.context_identifiers == null) {
          ...
          for (j = 1; j <= sch.context_identifiers.myLength; j++) {
            stream.writeUTF(sch.context_identifiers.getByIndex(j));
          }
  ...
  private void sortStrings(String[] s_strs, String[] strs, int start_index, int end_index) {
    ...
    for (i = start_index, m = start_index, n = middle; i < end_index; i++) {
      if (n >= end_index || m < middle && s_strs[m].compareTo(s_strs[n]) <= 0) {
        strs[i] = s_strs[m++];
      }
      else {
        strs[i] = s_strs[n++];
  ...
  void takeServerProp() throws SdaiException {
    try {
      if (((String) location).indexOf("@") == -1) {
        location = SdaiSession.LOCATION_PREFIX + SdaiSession.DEF_USER_NAME + ":" + SdaiSession.DEF_PASSWORD + "@"
            + ((String) location).substring(SdaiSession.LOCATION_PREFIX_LENGTH);
      }
  ...
  void loadHeaderRemote() throws SdaiException {
    ...
    try {
      ...
      while (repeat) { // In mutithreading mode if server waiting for listener connection can be refused
        ...
        istr = new DataInputStream(is_temp);
        ostr = new DataOutputStream(os_temp);
        ostr.writeUTF(user);
  ...
  void loadHeaderStream(DataInput stream) throws SdaiException {
    ...
    try {
      ...
      if (i < SdaiSession.valid_from) {
        String base = SdaiSession.line_separator + AdditionalMessages.BF_OLD0 + location + ".header" + AdditionalMessages.BF_OLD1 + i + ". "
            + SdaiSession.line_separator + AdditionalMessages.BF_OLD2 + SdaiSession.valid_from;
        throw new SdaiException(SdaiException.SY_ERR, base);
      }
      bt = stream.readByte();
      if (bt != 'B') {
  ...
  SdaiModel findDictionarySdaiModel(String model_name) throws SdaiException {
    if (SdaiSession.debug2) {
      System.out.println("   REPOSIT new model in data_dictionary: " + model_name);
    }
    if (model_name == null) {
      throw new SdaiException(SdaiException.SY_ERR);
    }
    SdaiModel model = findSdaiModel(model_name);
    if (model == null) {
      model = createModel(model_name, SdaiSession.baseDictionaryModel.described_schema);
      models.addUnorderedRO(model);
      insertModel();
...
//				if (line.length == 1 && sch_name.equals(line[0])) {
  ...
  SdaiModel verify_schema_name(String pattern_schema, String sch_name) throws SdaiException {
    ...
    SdaiModelDictionaryImpl mod = (SdaiModelDictionaryImpl) SdaiSession.systemRepository.findSdaiModel(search_mod_name);
  ...
  void validateRepoAfterImport() throws SdaiException {
    for (int i = 0; i < models.myLength; i++) {
      SdaiModel model = (SdaiModel) models.myData[i];
      model.validateModelAfterImport();
    }
  }
...
//		name = repoHeader.name;
   ...
   */
  static void printWarningToLogo(SdaiSession session, String text, String repo_name, String mod_name, SchemaInstance schema) throws SdaiException {
    if (session != null && session.logWriterSession != null) {
      session.printlnSession(text + SdaiSession.line_separator + AdditionalMessages.BF_REP + repo_name + SdaiSession.line_separator
          + AdditionalMessages.BF_MOD + mod_name + SdaiSession.line_separator + AdditionalMessages.BF_SCH + schema.name + SdaiSession.line_separator
          + AdditionalMessages.BF_REPS + schema.repository.name);
    }
    else {
      SdaiSession.println(text + SdaiSession.line_separator + AdditionalMessages.BF_REP + repo_name + SdaiSession.line_separator
          + AdditionalMessages.BF_MOD + mod_name + SdaiSession.line_separator + AdditionalMessages.BF_SCH + schema.name + SdaiSession.line_separator
          + AdditionalMessages.BF_REPS + schema.repository.name);
    }
  }

  static void printWarningToLogo2(SdaiSession session, String text, String repo_name, SchemaInstance schema, String included_schema) throws SdaiException {
    if (session != null && session.logWriterSession != null) {
      session.printlnSession(text + SdaiSession.line_separator + AdditionalMessages.BF_REP + repo_name + SdaiSession.line_separator
          + AdditionalMessages.BF_SCH + schema.name + SdaiSession.line_separator + AdditionalMessages.BF_REPS + schema.repository.name
          + SdaiSession.line_separator + AdditionalMessages.BF_ISCH + included_schema);
    }
    else {
      SdaiSession.println(text + SdaiSession.line_separator + AdditionalMessages.BF_REP + repo_name + SdaiSession.line_separator
          + AdditionalMessages.BF_SCH + schema.name + SdaiSession.line_separator + AdditionalMessages.BF_REPS + schema.repository.name
  ...
  protected abstract SdaiModel createModel(String model_name, CSchema_definition schema) throws SdaiException;
  ...
  ExternalData getEntityExternalData(CEntity instance) throws SdaiException {
    ...
    if (externalDataObject != null) {
      ...
      if (entityRemovedExternalData != null && entityRemovedExternalData.containsKey(instanceIdentifier)) {
  ...
  void restoreExternalDataForInstance(CEntity instance) {
    Long instanceIdentifier = new Long(instance.instance_identifier);
    ExternalData externalData = (ExternalData) ((entityRemovedExternalData != null) ? entityRemovedExternalData.get(instanceIdentifier) : null);
  ...
  void restoreUndoRedoExternalData(long inst_id) {
    Long instanceIdentifier = new Long(inst_id);
    Object externalDataObject = (entityRemovedExternalData != null) ? entityRemovedExternalData.remove(instanceIdentifier) : null;
    if (externalDataObject != null) {
      if (entityExternalData == null) {
        entityExternalData = new HashMap();
      }
...
//	}
  ...
  protected abstract ExternalData createNewEntityExternalData(CEntity entity, boolean existing) throws SdaiException;
...
//    }
   ...
   * copies are taken from the original instances similarly as in {@link SdaiModel#copyInstances copyInstances} method.
   * <p>
   * This method is an extension of JSDAI, which is not a part of the standard.
   *
  ...
  protected static final void forwardExternalDataSetOwningEntity(ExternalData externalData, CEntity owningEntity) throws SdaiException {
    externalData.owningEntity = owningEntity;
  }

  protected static final void forwardExternalDataSetStoreStream(ExternalData externalData, InputStream storeStream) throws SdaiException {
