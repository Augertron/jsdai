...
public class ComplexEntityValue {
  ...
  /**
   * The constructor of this class. Used in <code>CAggregate</code>, <code>CEntity</code>, <code>PhFileReader</code>,
   * <code>PhFileWriter</code> and <code>SdaiModel</code>.
   */
  ...
  public void attachEntity(CSchema_definition schema) throws SdaiException {
    ...
    for (int i = 0; i < sch_data.noOfEntityDataTypes; i++) {
      ...
      for (int j = 0; j < def.noOfPartialEntityTypes; j++) {
        res_index = def_ch.find_partial_entity(start_index, def_ch.noOfPartialEntityTypes - 1, entityValues[j].def.getCorrectName());
        if (res_index < 0) {
          found = false;
          break;
        }
