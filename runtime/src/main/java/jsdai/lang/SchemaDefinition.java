...
/**
 * It is a supertype of the class <code>CSchema_definition</code> contained in the <code>jsdai.dictionary</code> package. This
 * class is designed primarily
 * to hold some non-public data fields. Also, the public method {@link #getEntityDefinition getEntityDefinition} defined in
...
//		} // syncObject
  ...
  protected AEntity_definition getEntities() throws jsdai.lang.SdaiException {
    if (entities == null) {
      ...
      AEntity ents = entities;
      ents.myType = SdaiSession.setType0toN;
      ents.owner = this;
      if (SdaiSession.debug2) {
        System.out.println("  SchemaDefinition    schema: " + ((ESchema_definition) this).getName(null));
      }
      if (SdaiSession.debug2) {
        System.out.println("  In SchemaDefinition    modelDictionary: " + modelDictionary.name);
      }
    ...
    for (i = 0; i < data.entities.length; i++) {
      ...
      for (j = 1; j <= ((AEntity) supertypes).myLength; j++) {
        ...
        if (sch_data == data) {
          ...
          index = data.find_entity(0, data.sNames.length - 1, super_entity);
...
//System.out.println("*****MODEL: " + modelDictionary.name/* + "    SCHEMA: " + schema.getName(null)*/);
        ...
        if (index < 0) {
          String base = SdaiSession.line_separator + "Entity not found: " + super_entity.getNameUpperCase();
    ...
    for (i = 0; i < data.entities.length; i++) {
      if (SdaiSession.debug2) {
        System.out.println(" ind = " + i + "   ENTITY: " + data.entities[i].getCorrectName() + "    data.model.name = " + data.model.name);
      }
      ...
      if (SdaiSession.debug2) {
        System.out.println(" index = " + i + "   !!!!entity: " + data.entities[i].getCorrectName());
      }
      if (SdaiSession.debug) {
        System.out.print(" subtypes (count = " + count[i] + ") : ");
      }
      for (j = 0; j < count[i]; j++) {
        subtypes[i][j] = temp[i][j];
        if (SdaiSession.debug) {
          System.out.print("  " + subtypes[i][j]);
        }
      }
      if (SdaiSession.debug) {
        System.out.println();
