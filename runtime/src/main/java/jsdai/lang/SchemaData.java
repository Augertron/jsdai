...
import jsdai.dictionary.CEntity_declaration$implicit_declaration;
import jsdai.dictionary.CEntity_declaration$local_declaration;
import jsdai.dictionary.CEntity_declaration$referenced_declaration;
import jsdai.dictionary.CEntity_declaration$used_declaration;
...
class SchemaData {
  ...
  void initializeSchemaData(int noOfEntityDataTypes, int noOfComplexEntityDataTypes) {
    ...
    if (SdaiSession.debug2) {
      System.out.println("  SchemaData   noOfEntityDataTypes = " + noOfEntityDataTypes + "    initialized schema for model: " + model.name);
    }
  }

  void initializeDefinedTypes() throws SdaiException {
    defTypesCount = model.lengths[SdaiSession.DEFINED_TYPE] + model.lengths[SdaiSession.REFERENCED_DECL_TYPE_DECL]
        + model.lengths[SdaiSession.TYPE_DECL_USED_DECL] + model.lengths[SdaiSession.IMPLICIT_DECL_TYPE_DECL];
...
//System.out.println("  SchemaData   ***** MODEL: " + model.name);
      ...
      for (i = 0; i < model.lengths[SdaiSession.ENTITY_DECL_LOCAL_DECL]; i++) {
        CEntity_declaration$local_declaration dec_loc = (CEntity_declaration$local_declaration) entity_instances[i];
      ...
      if (model != SdaiSession.baseComplexModel) {
        ...
        for (i = 0; i < model.lengths[SdaiSession.ENTITY_DECL_REFERENCED_DECL]; i++) {
          CEntity_declaration$referenced_declaration dec_ref = (CEntity_declaration$referenced_declaration) entity_instances[i];
        ...
        for (i = 0; i < model.lengths[SdaiSession.ENTITY_DECL_USED_DECL]; i++) {
          CEntity_declaration$used_declaration dec_used = (CEntity_declaration$used_declaration) entity_instances[i];
        ...
        for (i = 0; i < model.lengths[SdaiSession.ENTITY_DECL_IMPLICIT_DECL]; i++) {
          CEntity_declaration$implicit_declaration dec_imp = (CEntity_declaration$implicit_declaration) entity_instances[i];
...
//}System.out.println();
    ...
    for (i = 0; i < count; i++) {
      ...
      if (SdaiSession.debug2) {
        System.out.println("  entity : " + edef.getCorrectName() + "  complex= " + edef.complex + "   sNames: " + sNames[i] + "  entity ident: "
            + ((CEntity) edef).instance_identifier);
      }
...
//edef.del_supertype = false;
        ...
        for (j = 0; j < types.length; j++) {
          CEntityDefinition edef_in_super = types[j];
...
//"  ****short: " + short_name_from_super);
      ...
      fromLong[i] = i;
      if (SdaiSession.debug2) {
        System.out.println("  parent schema = " + edef.owning_model.described_schema.getName(null));
      }
      if (SdaiSession.debug2) {
        System.out.println(" class name = " + sClassNames[i] + "  i: " + i);
      }
    }
    ...
    for (i = 0; i < count; i++) {
      ...
      if (SdaiSession.debug2) {
        System.out.println(" LONG NAME: " + sLongNames[i] + "   NORMAL NAME: " + sNames[i]);
      }
    }
    ...
    if (SdaiSession.debug2) {
      System.out.println("MODEL: " + model.name + "    SCHEMA: " + schema.getName(null) + "   noOfShortNames = " + noOfShortNames);
    }
    if (SdaiSession.debug2) {
      for (i = 0; i < count; i++) {
...
//"   SHORT NAME: " + sShortNames[i]);
    ...
    this.schema = schema;
    SdaiSession.tm1 += (time2 - time1);
    SdaiSession.tm2 += (time3 - time2);
    SdaiSession.tm3 += (time4 - time3);
    SdaiSession.tm4 += (time5 - time4);
    SdaiSession.tm0 += (time5 - time1);
    if (SdaiSession.debug2) {
      SdaiSession.println("init() =" + model.name + ": " + SdaiSession.tm1 + "+" + SdaiSession.tm2 + "+" + SdaiSession.tm3 + "+" + SdaiSession.tm4 + "="
...
//if (j==0 || j==1) 
//System.out.println("SchemaData    k: " + k + "  data_types[k]: " + data_types[k]); 
...
//}
  ...
  private void setAggregationTypes(SdaiModel model, int extent, int e_type) throws SdaiException {
    for (int i = 0; i < model.lengths[extent]; i++) {
      ...
      nesting_depth = 1;
      if (SdaiSession.debug2) {
        System.out.println("  index i = " + i + "    extent = " + extent + "   model.name = " + model.name + "   name: " + aggr_type.getName(null));
      }
...
//+ " name: " + el_type.getClass().getName());
    if (el_type instanceof ESimple_type) {
      ...
      nesting_depth++;
      if (SdaiSession.debug2) {
        System.out.println("  Before resolveType again   nesting_depth = " + nesting_depth);
      }
      resolveType((CEntity) type_next.getElement_type(null));
    }
    else if (el_type instanceof CEntity_definition) {
      if (SdaiSession.debug2) {
        System.out.println(" case of CEntity_definition = " + el_type.instance_identifier);
      }
...
//System.out.println("  +++++ SchemaData  model: " + model.name);
      ...
      try {
        Class cl = Class.forName(str, true, SdaiClassLoaderProvider.getDefault().getClassLoader());
...
//System.out.println(" In schemaData ***** super_inst: " + super_inst);
        ...
        throw new SdaiException(SdaiException.SY_ERR, "Special class for schema definition not found: " + str);
      }
...
//time1 = System.currentTimeMillis();
    ...
    for (i = 0; i < entities.length; i++) {
      if (SdaiSession.debug) {
        System.out.println(" SchemaData BEGIN entity: " + entities[i].getCorrectName() + "   its owning_model: " + entities[i].owning_model.name);
      }
...
//		model.described_schema.prepareSubtypes(origin);
    ...
    if (model == SdaiSession.baseDictionaryModel) {
      ...
      for (i = 0; i < model.lengths[SdaiSession.ENTITY_DEFINITION]; i++) {
        ...
        if (SdaiSession.debug2) {
          System.out.println(" ENTITY CONSIDERED: " + edef.instance_identifier + "  name: " + edef.getCorrectName());
        }
    ...
    int ext_count = model.lengths[select_refs[1]] + model.lengths[select_refs[2]] + model.lengths[select_refs[3]] + model.lengths[select_refs[5]]
        + model.lengths[select_refs[6]] + model.lengths[select_refs[7]] + model.lengths[select_refs[8]];
...
//		}
    ...
    SdaiSession.tmbind1 += (time2 - time1);
    SdaiSession.tmbind2 += (time3 - time2);
    SdaiSession.tmbind3 += (time4 - time3);
    SdaiSession.tmbind4 += (time5 - time4);
    SdaiSession.tmbind5 += (time6 - time5);
    SdaiSession.tmbind0 += (time6 - time1);
    if (SdaiSession.debug2) {
      SdaiSession.println("linkEarlyBinding() =" + model.name + ": " + SdaiSession.tmbind1 + "+" + SdaiSession.tmbind2 + "+" + SdaiSession.tmbind3 + "+"
          + SdaiSession.tmbind4 + "+" + SdaiSession.tmbind5 + "=" + SdaiSession.tmbind0 + " milisec");
    }
  }
...
//		}
  ...
  private void sortEntities(CEntityDefinition[] s_entities, CEntityDefinition[] entities, int start_index, int end_index) {
    int i;
    int gap = end_index - start_index;
    if (gap < 7) {
      for (i = start_index; i < end_index; i++) {
        for (int j = i; j > start_index && entities[j - 1].getNameUpperCase().compareTo(entities[j].getNameUpperCase()) > 0; j--) {
    ...
    for (i = start_index, m = start_index, n = middle; i < end_index; i++) {
      if (n >= end_index || m < middle && s_entities[m].getNameUpperCase().compareTo(s_entities[n].getNameUpperCase()) <= 0) {
  ...
  private void sortEntitiesByNames(String[] s_names, String[] names, int[] s_indices, int[] indices, int start_index, int end_index) {
    int i;
    int m, n;
    int gap = end_index - start_index;
    if (gap < 7) {
  ...
  private static void sortTypes(CDefined_type[] def_types_sorting, CDefined_type[] def_types, int start_index, int end_index) {
    int i;
    int gap = end_index - start_index;
    if (gap < 7) {
      for (i = start_index; i < end_index; i++) {
        for (int j = i; j > start_index && def_types[j - 1].getNameUpperCase().compareTo(def_types[j].getNameUpperCase()) > 0; j--) {
    ...
    int m, n;
    for (i = start_index, m = start_index, n = middle; i < end_index; i++) {
      if (n >= end_index || m < middle && def_types_sorting[m].getNameUpperCase().compareTo(def_types_sorting[n].getNameUpperCase()) <= 0) {
  ...
  private static void sortData_types(DataType[] data_types_sorting, DataType[] data_types, int start_index, int end_index) {
    int i;
    int gap = end_index - start_index;
    if (gap < 7) {
      for (i = start_index; i < end_index; i++) {
        for (int j = i; j > start_index && data_types[j - 1].getNameUpperCase().compareTo(data_types[j].getNameUpperCase()) > 0; j--) {
    ...
    int m, n;
    for (i = start_index, m = start_index, n = middle; i < end_index; i++) {
      if (n >= end_index || m < middle && data_types_sorting[m].getNameUpperCase().compareTo(data_types_sorting[n].getNameUpperCase()) <= 0) {
...
//		int index = find_entity(0, noOfEntityDataTypes - 1, edef.getNameUpperCase());
    ...
    if (SdaiSession.debug2) {
      System.out.println(" index = " + index + "   for: " + edef.getNameUpperCase());
    }
    if (SdaiSession.debug2) {
      System.out.println("  entities[index] ident = " + entities[index].instance_identifier + "  owning model: " + entities[index].owning_model.name);
    }
...
//}
  ...
  int findEntityExtentIndex(Class provided_class) throws SdaiException {
    ...
    String class_name = full_name.substring(full_name.lastIndexOf(".") + 2, full_name.length()).toUpperCase();
...
//"   defined type: " + ((CDefined_type)type).getName(null));
  ...
  void collect_typeOF_types_for_entity(CEntityDefinition type, SdaiContext context) throws SdaiException {
    ...
    for (i = 0; i < defTypesCount; i++) {
      ...
      sels = ((ESelect_type) underlying_type).getSelections(null, context);
      if (sels.myLength < 0) {
        sels.resolveAllConnectors();
      }
      if (sels.myLength <= 0) {
...
//System.out.println("SchemaData !!!+++++++++++++++++++++ExternalMapping   def = " + ((CEntity_definition)def).getName(null)); 
      ...
      for (int j = 0; j < n_defs; j++) {
        res_index = def.find_partial_entity(start_index, def.noOfPartialEntityTypes - 1, sup.partialEntityTypes[j].getCorrectName());
        if (res_index < 0) {
          found = false;
          break;
        }
...
//System.out.println("   Subtype = " + ((CEntity_definition)def).getName(null));
  ...
  int getBypassedRulesCount(int index, SdaiSession ss) throws SdaiException {
    ...
    String[][] byp_rules = ss.getBlackList();
    if (ss.byp_rules_count < 0) {
      return 0;
    }
