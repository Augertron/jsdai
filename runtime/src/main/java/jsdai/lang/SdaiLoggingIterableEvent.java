...
abstract class SdaiLoggingIterableEvent extends SdaiLoggingEvent implements SdaiOperationIterator {
  ...
  /**
   * Constructs a new object of <code>SdaiLoggingIterableEvent</code> with methods which allow to iterate over records in the
   * undo/redo log file.
   *
   ...
   */
  public SdaiLoggingIterableEvent(SdaiSession source, int id, int item, Object argument) {
  ...
  protected boolean readInstanceLog(SdaiSession session, boolean readInstDel, boolean doubleBypass) throws IOException, SdaiException {
    ...
    /* int inst_index = */
    session.undo_redo_file.readInt();
    /* boolean modif = */
    session.undo_redo_file.readBoolean();
    if (readInstDel) {
      /* boolean inst_del = */
      session.undo_redo_file.readBoolean();
    }
