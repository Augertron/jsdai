...
//	static boolean CATCH_EXCEPTIONS = true;
  ...
  protected CEntity(SdaiModel mod) throws SdaiException {
    mod.putEntityInstance(this);
  }
...
//		synchronized (syncObject) {
    ...
    for (int i = 0; i < length; i++) {
      if (edef.attributesInverse[i] == attribute) {
        ...
        if (ref_index >= 0) {
          int count = countSubtypesWithDerived(referencing_type, role, sch_data, ref_index);
          if (count > 0) {
            ent_types = new CEntityDefinition[count];
            der_attrs = new CDerived_attribute[count];
            findSubtypesWithDerived(referencing_type, role, sch_data, ref_index, ent_types, der_attrs);
...
//		} // syncObject
  ...
  int countSubtypesWithDerived(CEntity_definition referencing_type, CExplicit_attribute role,
      SchemaData sch_data, int ref_index) throws SdaiException {
    int count = 0;
    ...
    for (int i = 0; i < subtypes.length; i++) {
      ...
      if (edef.attributes == null) {
        Class cl = sch_data.getEntityClassByIndex(subtypes[i]);
      }
      ...
      if (expl_redecl) {
        if (edef.getIfDerived(role) != null) {
          count++;
        }
      }
    }
  ...
  int findSubtypesWithDerived(CEntity_definition referencing_type, CExplicit_attribute role,
      SchemaData sch_data, int ref_index, CEntityDefinition[] ent_types,
      CDerived_attribute[] der_attrs) throws SdaiException {
    int count = 0;
    int subtypes[] = sch_data.schema.getSubtypes(ref_index);
    for (int i = 0; i < subtypes.length; i++) {
      CEntityDefinition edef = sch_data.entities[subtypes[i]];
    ...
    while (staticFields.it.next()) {
      ...
      for (int k = 0; k < count; k++) {
        ...
        if ((model.mode & SdaiModel.MODE_MODE_MASK) == SdaiModel.READ_ONLY) {
          CEntityDefinition exact_type = sch_data.entities[ref_index];
          index_to_type_local = model.find_entityRO(exact_type);
        ...
        if (model.instances_sim[index_to_type_local] == null || model.lengths[index_to_type_local] <= 0) {
          continue;
        }
        ...
        CEntity[] row_of_instances = model.instances_sim[index_to_type_local];
        for (int j = 0; j < model.lengths[index_to_type_local]; j++) {
...
//}
    ...
    if (index_to_entity >= 0) {
      ...
      if (ind >= 0) {
        ...
        System.arraycopy(owning_model.instances_sim[index_to_entity], ind + 1, owning_model.instances_sim[index_to_entity], ind,
            owning_model.instances_sim[index_to_entity].length - ind - 1);
...
//System.out.println("CEntity !!!!!   validated instance = " + strg + "  id: " + instance_identifier);
    ...
    if (rule != null) {
      ...
      SchemaData part_sch_data;                          // added for Bug #2739
      ...
      for (int j = 0; j < def.noOfPartialEntityTypes; j++) {
        ...
        if (def.complex == 2) {                             // added for Bug #2739
          part_sch_data = part_def.owning_model.schemaData; // added for Bug #2739
        }
        else {                                            // added for Bug #2739
          part_sch_data = sch_data;                         // added for Bug #2739
        }                                                   // added for Bug #2739
        int def_index = part_sch_data.findEntityExtentIndex(part_def); // added for Bug #2739
        if (def_index < 0) {
          throw new SdaiException(SdaiException.SY_ERR, "Entity not found in the table");
        }
        int br_count = part_sch_data.getBypassedRulesCount(def_index, ss); // added for Bug #2739
        if (br_count > 0) {                                                // added for Bug #2739
          bp_rules_names = part_sch_data.getBypassedRules(def_index);      // added for Bug #2739
        }                                                                  // added for Bug #2739
...
//Value.prnt=false;
    ...
    if (ret_value <= 1) {
      ...
      SdaiContext contxt = ss.sdai_context;
...
//		} // syncObject
  ...
  private boolean look_for_string(EData_type type) throws SdaiException {
    ...
    if (tp.express_type >= DataType.NUMBER && tp.express_type <= DataType.BINARY) {
      ...
      return look_for_string(((EAggregation_type) tp).getElement_type(null));
    }
...
//		}
    ...
    if (SdaiSession.debug2) {
      System.out.println("  domain member count = " + modelDomain.myLength);
    }
    if (SdaiSession.debug2) {
      System.out.println("  referent_def ident = " +
    ...
    if (SdaiSession.debug2) {
      System.out.println("  this ident = " + instance_identifier +
...
//				k++;
        if (SdaiSession.debug2) {
          System.out.println("  inst 2 ident = " + inst.instance_identifier +
              "owning model: " + inst.owning_model.name + "  inst type: " +
              ((CEntity_definition) inst.getInstanceType()).getCorrectName() + "  referent def: " +
              ((CEntity_definition) referent_def).getCorrectName());
        }
...
//				}
    ...
    if (SdaiSession.debug2) {
      System.out.println(" ******* result.myLength = " + result.myLength);
    }
...
//		} // syncObject
  ...
  private void makeUsedin2(CEntity inst, EAttribute role, AEntity result, ASdaiModel domain, boolean for_inverse)
    ...
    CExplicit_attribute[] attributes = entityDef.attributes;
    if (SdaiSession.debug2) {
      System.out.println("  entityDef ident = " + ((CEntity_definition) entityDef).instance_identifier +
...
//System.out.println(" !!!DOMAIN: " + dom.getClass().getName());
    ...
    if (SdaiSession.debug2) {
      System.out.println("  provided_type = " + provided_type);
    }
    try {
    ...
    if (SdaiSession.debug2) {
      if (aggr.myType == null) {
        System.out.println("   myType is NULL");
      }
      else {
        System.out.println("   myType is POSITIVE");
      }
    }
    return aggr;
//		} // syncObject
...
//		} // syncObject
  ...
  public AGeneric_attribute_mapping testMappedAttribute(EAttribute sourceAttribute, ASdaiModel targetDomain, ASdaiModel mappingDomain, int mode)
      throws SdaiException {
    if (Implementation.mappingSupport) {
  ...
  public boolean testMappedAttribute(EGeneric_attribute_mapping sourceAttribute, ASdaiModel targetDomain, ASdaiModel mappingDomain, int mode)
      throws SdaiException {
    if (Implementation.mappingSupport) {
  ...
  public Object getMappedAttribute(EGeneric_attribute_mapping sourceAttribute, ASdaiModel targetDomain, ASdaiModel mappingDomain, int mode)
      throws SdaiException {
    if (Implementation.mappingSupport) {
  ...
  public int findEntityMappings(EEntity_definition sourceEntity, ASdaiModel targetDomain, ASdaiModel mappingDomain, AEntity_mapping mappings, int mode)
      throws SdaiException {
    if (Implementation.mappingSupport) {
...
//		if (val.check((EData_type)attr.getDomain(null)) == null) {
    ...
    if (sel_type != null) {
      ...
      int sel_number;
      if (val.dt_count > 0) {
        sel_number = sel_type.giveSelectNumber(val.definedTypes, val.dt_count);
      }
      else {
        sel_number = val.sel_number;
      }
      try {
...
// print error message
  ...
  public Value get(EAttribute attribute, SdaiContext context) throws SdaiException {
    if (owning_model == null) {
      throw new SdaiException(SdaiException.EI_NEXS);
    }
    if (attribute == null) {
...
//			throw new SdaiException(SdaiException.AT_NVLD, saved_attr);
    ...
    if (sel_type != null) {
      ...
      val.getDefinedTypes(sel_type, sel_number);
      val.aux = 1;
      return val;
    }
...
//		} // syncObject
  ...
  /*public*/    void replaceA_double3() throws SdaiException {
    StaticFields staticFields = StaticFields.get();
    if (staticFields.entity_values == null) {
      staticFields.entity_values = new ComplexEntityValue();
    }
    ...
    getAll(staticFields.entity_values);
    for (int i = 0; i < def.noOfPartialEntityTypes; i++) {
      EntityValue ent_value = staticFields.entity_values.entityValues[i];
      for (int j = 0; j < ent_value.count; j++) {
        Value val = ent_value.values[j];
        if (val.tag != PhFileReader.EMBEDDED_LIST) {
          continue;
        }
        if (val.reference instanceof A_double3) {
          val.change_value = 1;
        }
      }
    }
...
//					val.reference = ref_mod.instances_sim[pop_index][inst_index];
  ...
  boolean validateInstanceAfterImport(StaticFields staticFields, CEntityDefinition def) throws SdaiException {
    owning_model.prepareAll(staticFields.entity_values, (CEntity_definition) def);
    getAll(staticFields.entity_values);
    boolean incosist_found = false;
    long referenced_id = 0;
    for (int i = 0; i < def.noOfPartialEntityTypes; i++) {
      EntityValue ent_val = staticFields.entity_values.entityValues[i];
      if (ent_val == null) {
        continue;
      ...
      for (int j = 0; j < p_def.noOfPartialAttributes; j++) {
        Value val = ent_val.values[j];
        if ((val.tag != PhFileReader.ENTITY_REFERENCE) && (val.tag != PhFileReader.EMBEDDED_LIST)) {
          continue;
        }
        if (val.tag == PhFileReader.ENTITY_REFERENCE) {
          referenced_id = ((CEntity) val.reference).instance_identifier;
        }
        boolean res = val.validateValueAfterImport((DataType) expl_attrs[j].getDomain(null), this, true);
        if (!res) {
          incosist_found = true;
          if (val.tag == PhFileReader.ENTITY_REFERENCE) {
            EntityValue.printWarningToLogo(owning_model.repository.session, AdditionalMessages.RD_WRRE,
                instance_identifier, expl_attrs[j], referenced_id);
          }
          else {
            EntityValue.printWarningToLogo(owning_model.repository.session, AdditionalMessages.RD_WRAG,
                instance_identifier, expl_attrs[j]);
          }
        }
      }
    }
