...
public class CMappingIntersection_constraint extends CConstraint implements MappingConstraintPath, MappingConstraintMatcher {
  ...
  public void mapUsersForward(ObjectMapping mappingContext, Set instances) throws SdaiException {
    ...
    while (constraintIter.next()) {
      ...
      ((MappingConstraintPath) constraint).mapUsersForward(mappingContext, constraintInstances);
  ...
  public MatcherInstances findForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      EIntersection_constraint selfInterface = (EIntersection_constraint) this;
      AConstraint_select constraints = selfInterface.getSubpaths(null);
      EEntity[] dependentSelfInstances = constraints.getMemberCount() > 0 ? new EEntity[constraints.getMemberCount()] : null;
      int dependentSelfInstCnt = 0;
      SdaiIterator constraintIter = constraints.createIterator();
      while (constraintIter.next()) {
        EEntity constraint = constraints.getCurrentMember(constraintIter);
        if (outInstances != null) {
          outInstances = outInstances.intersect(((MappingConstraintMatcher) constraint).findPathForward(mappingContext,
              instances.dup(null, MatcherInstances.STATUS_PATH_FORWARD), false));
        }
        else {
          outInstances = ((MappingConstraintMatcher) constraint).findPathForward(mappingContext,
              instances.dup(null, MatcherInstances.STATUS_PATH_FORWARD), false);
        }
