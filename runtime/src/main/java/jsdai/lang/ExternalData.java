...
public abstract class ExternalData {
  ...
  public void loadToStream(OutputStream stream) throws SdaiException {
    ...
    if (storeStream != null) {
      if (!storeStream.markSupported()) {
        throw new SdaiException(SdaiException.SY_ERR, "Assigned store stream does not support marking,"
      ...
      if (unset) {
        throw new SdaiException(SdaiException.VA_NSET, "External data does not exist for this instance: " + owningEntity);
      }
    }
  }
   ...
   * closed after transaction commit or abort within calls <code>SdaiTransaction#abort()</code>,
   * <code>SdaiTransaction#endTransactionAccessAbort()</code>, <code>SdaiTransaction#commit()</code>,
   * <code>SdaiTransaction#endTransactionAccessCommit()</code>, or
   * <code>SdaiTransaction#endTransactionAccessCommit(String)</code>.
   * If <code>loadToStream</code> is invoked after call to <code>storeFromStream</code> then provided stream has to
   * support mark/reset, ie. <code>stream.markSupported()</code> has to return <code>true</code> and calls to
   * <code>stream.mark(0)</code> and <code>stream.reset()</code> have to be functional.
  ...
  public String getName() throws SdaiException {
    ...
    if (name == null) {
      throw new SdaiException(SdaiException.VA_NSET, "Name not set: " + owningEntity);
    }
    return name;
  }
