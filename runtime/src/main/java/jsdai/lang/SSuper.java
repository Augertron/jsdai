...
public abstract class SSuper {
  ...
  protected abstract CEntity makeInstanceX(Class c) throws java.lang.InstantiationException, java.lang.IllegalAccessException;
  ...
  CEntity makeInstance(Class c, SdaiModel model, int typeIndex, long id) throws SdaiException {
    ...
    if (typeIndex >= 0) {
      if (typeIndex < model.underlying_schema.owning_model.schemaData.entities.length) {
        if (SdaiSession.debug2) {
          System.out.println("  inst created: " + c.getName());
        }
    ...
    public Class findXClass(Class mainClass) {
      ...
      if (XClass == null) {
        ...
        if (className.charAt(dotPos + 1) == 'C') {
          ...
          try {
            XClass = Class.forName(newClassName, true, SdaiClassLoaderProvider.getDefault().getClassLoader());
          }
