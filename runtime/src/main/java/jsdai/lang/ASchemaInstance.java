...
public class ASchemaInstance extends SessionAggregate {
  ...
  /**
   * Constructs a new object of aggregate <code>ASchemaInstance</code> without aggregation type. The aggregates of such kind are
   * used
...
//		} // syncObject
   ...
   * the first parameter is of type <code>SchemaInstance</code> and the
...
//		} // syncObject
   ...
   * [])} method - the second parameter is of type <code>SchemaInstance</code> and the third parameter is dropped.
   *
   * @param index
   * @param value
   */
...
//	}
   ...
   * <p>
   * This method can be used to get an aggregate which can be submitted to parameter with the name "domain" in several
   * methods in class <code>CEntity</code>, for example {@link EEntity#findEntityInstanceUsedin
   * findEntityInstanceUsedin} method.
   * <p>
...
//		synchronized (syncObject) {
    ASdaiModel models = new ASdaiModel(SdaiSession.setTypeSpecial, owner != null ? owner : GENERIC_OWNER);
    ...
    while (staticFields.it_sa.next()) {
      ...
      for (int j = 0; j < si.associated_models.myLength; j++) {
        models.addUnorderedRO(si.associated_models.myData[j]);
      }
    }
    return models;
//		} // syncObject
   ...
   * <B>Example:</B>
   * <p>
   * <pre>    ASchemaInstance sch_aggr = ...;
   *    System.out.println(sch_aggr);</pre> &nbsp;&nbsp;&nbsp;&nbsp;The string printed will be like this:
   *
   * <pre>
