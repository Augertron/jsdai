...
class Scanner {
  ...
  private final static int keyword_length[] = { 4, 8, 6, 6, 5, 12, 16 };
  private final int keyword_flag[] = new int[7];
  ...
  private void skip_blanks() throws java.io.IOException {
    ...
    while (s == PhFileReader.SPACE || s == (byte) '\n' || s == (byte) '\r' || s == (byte) '\t' || s == (byte) '\b' || s == (byte) '\f' || s == 0) {
      if (s == (byte) '\n') {
        line++;
        column = 0;
      }
...
//System.out.println("token in scanner="+tok.type);
  ...
  private void printWarningToLogo(SdaiSession session) {
    ...
    String base = AdditionalMessages.RD_WCHA + SdaiSession.line_separator + "   Line: " + incr_line + SdaiSession.line_separator + "   Column: " + column;
    try {
