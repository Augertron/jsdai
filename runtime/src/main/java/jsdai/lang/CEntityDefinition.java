...
/**
 * It is a supertype of the class <code>CEntity_definition</code> contained in the <code>jsdai.dictionary</code> package. This
 * class is designed primarily
 ...
 * data dictionary during opening of the session. Also, the public methods {@link EntityDefinition#isSubtypeOf isSubtypeOf} and
 ...
 * and additional public methods {@link EntityDefinition#getExplicit_attributes getExplicit_attributes} and
...
//		} // syncObject
  ...
  private boolean IsInSubtypesList(CEntityDefinition def, CEntityDefinition compType) throws SdaiException {
    ...
    if (sub_index < 0) {
      throw new SdaiException(SdaiException.SY_ERR, "Entity not found in the table: " + ((CEntity_definition) def).getName(null) + " (used in model: "
          + owning_model.name + ")");
    }
  ...
  private boolean IsInSupertypeList(CEntity_definition def, CEntity_definition compType) throws SdaiException {
    if (def == compType) {
      return true;
    }
...
//		return false;
  ...
  public boolean isDomainEquivalentWith(EEntity_definition compType) throws SdaiException {
...
//				singleEntityTypes.addByIndex(i + 1, partialEntityTypes[i]);
  ...
  static Class getEntityInterface(SchemaDefinition schemaDef, String entity) throws SdaiException {
    ...
    try {
      ...
      throw new SdaiException(SdaiException.SY_ERR, "Early binding entity-class class not found: " + s);
    }
  }
...
//else System.out.println("  CEntityDefinition sup is POS for  model: " + model.name);
  ...
  static Class getEntityAggregate(SchemaDefinition schemaDef, String entity) throws SdaiException {
    ...
    try {
      ...
      throw new SdaiException(SdaiException.SY_ERR, "Early binding entity-aggregate class not found: " + s);
    }
  }

  static Class getEntityNestedAggregate(SchemaDefinition schemaDef, String entity, int level) throws SdaiException {
    SdaiModel model = schemaDef.owning_model;
    SSuper sup = model.schemaData.super_inst;
    String str = sup.getClass().getName();
    int index = str.lastIndexOf('.');
    ...
    if (SdaiSession.debug2) {
      System.out.println(" In EntityDefinition s = " + s);
    }
    try {
      return SdaiClassLoaderProvider.getDefault().getClassLoader().loadClass(s);
    }
  ...
  Class getEntityNestedAggregate(int level) throws SdaiException {
    ...
    if (level > entityNestedAggregate.length) {
      throw new SdaiException(SdaiException.SY_ERR, "Aggregates of nesting level " + level + " are not supported");
    }
    int ind = level - 1;
  ...
  private void setEarlyBinding1(EEntity_definition entity, AEntity_definition result) throws SdaiException {
      ...
      if (((CEntityDefinition) entity).der_attr_exist) {
        if (complex == 2 /* true */) {
          ...
          if (index < 0) {
            ...
            throw new SdaiException(SdaiException.SY_ERR, "Entity not found in the table: " + entity.getName(null) + " (used in model: "
                + error_mod.name + ")");
          }
          ...
          if (index < 0) {
            ...
            throw new SdaiException(SdaiException.SY_ERR, "Entity not found in the table: " + ((CEntity_definition) this).getName(null)
                + " (used in model: " + error_mod.name + ")");
          }
        }
        else {
...
//"   partial entity: " + entity.getName(null));
      ...
      if (partial_attrs.length > 0 || ((CEntityDefinition) entity).der_attr_exist || ((CEntityDefinition) entity).inv_attr_exist) {
...
//"  entity: " + ((CEntityDefinition)entity).getName(null));
  ...
  /**
   * Returns a JSDAI entity (CXxxx class) super-class of the specified class, or <code>null</code> if no such class is
   * available.
   *
  ...
  private boolean findPartialEntity(CEntity_definition base_entity, CEntity_definition super_entity, CEntity_definition target_entity) throws SdaiException {
...
//				EAttribute attrib = partial_attrs.getByIndex(i);
      for (int i = 0; i < partial_attrs.length; i++) {
        ...
        if ((((AttributeDefinition) attrib).attr_tp == AttributeDefinition.EXPLICIT) && !((CExplicit_attribute) attrib).testRedeclaring(null)) {
          entity.noOfPartialAttributes++;
        }
      }
    }
    ...
    while (j < noOfPartialEntityTypes - 1) {
      ...
      for (i = 0; i < partial_attrs.length; i++) {
        ...
        try {
          ...
          String base = SdaiSession.line_separator + "Class: " + cl.getName() + "   schema: " + sch_data.schema.getName(null) + "   name: " + name
              + "   " + ex;
...
//						}
  ...
  private void addPartialTypes(AEntity_definition result) throws SdaiException {
...
//			result.addByIndex(((AEntity)result).myLength + 1, this);
  ...
  public final AExplicit_attribute getExplicit_attributes(EEntity_definition type) throws SdaiException {
//		synchronized (syncObject) {
    int i;
  ...
  private final void take_attributes(SdaiModel mod) throws SdaiException {
    ...
    if (mod == SdaiSession.baseDictionaryModel) {
      for (i = 0; i < mod.lengths[SdaiSession.EXPLICIT_ATTRIBUTE]; i++) {
        CExplicit_attribute ex_attr = (CExplicit_attribute) mod.instances_sim[SdaiSession.EXPLICIT_ATTRIBUTE][i];
...
//System.out.println("CEntityDefinition:  i: " + i + "  der_attrs[i]: " + der_attrs[i]);
    ...
    if (mod == SdaiSession.baseDictionaryModel) {
      for (i = 0; i < count; i++) {
        attributesExpl[i] = (CExplicit_attribute) mod.instances_sim[SdaiSession.EXPLICIT_ATTRIBUTE][staticFields.indices[i]];
    ...
    if (mod == SdaiSession.baseDictionaryModel) {
      for (i = 0; i < mod.lengths[SdaiSession.DERIVED_ATTRIBUTE]; i++) {
        CDerived_attribute der_attr = (CDerived_attribute) mod.instances_sim[SdaiSession.DERIVED_ATTRIBUTE][i];
      ...
      if (count > 0) {
        der_attr_exist = true;
        for (i = 0; i < count; i++) {
          attributesDeriv[i] = (CDerived_attribute) mod.instances_sim[SdaiSession.DERIVED_ATTRIBUTE][staticFields.indices[i]];
    ...
    if (mod == SdaiSession.baseDictionaryModel) {
      for (i = 0; i < mod.lengths[SdaiSession.INVERSE_ATTRIBUTE]; i++) {
        CInverse_attribute inv_attr = (CInverse_attribute) mod.instances_sim[SdaiSession.INVERSE_ATTRIBUTE][i];
      ...
      if (count > 0) {
        inv_attr_exist = true;
        for (i = 0; i < count; i++) {
          attributesInv[i] = (CInverse_attribute) mod.instances_sim[SdaiSession.INVERSE_ATTRIBUTE][staticFields.indices[i]];
    ...
    if (mod == SdaiSession.baseDictionaryModel) {
      ...
      if (count1 > 0) {
        for (i = 0; i < count1; i++) {
          attributesRedecl[i] = (EAttribute) mod.instances_sim[SdaiSession.EXPLICIT_ATTRIBUTE][staticFields.indices[i]];
        }
      }
      if (count2 > 0) {
        for (i = count1; i < count1 + count2; i++) {
          attributesRedecl[i] = (EAttribute) mod.instances_sim[SdaiSession.DERIVED_ATTRIBUTE][staticFields.indices[i]];
        }
      }
      if (count3 > 0) {
        for (i = count1 + count2; i < count; i++) {
          attributesRedecl[i] = (EAttribute) mod.instances_sim[SdaiSession.INVERSE_ATTRIBUTE][staticFields.indices[i]];
        }
      }
    }
   ...
   * This method is invoked in <code>getAttributeDefinition</code> in <code>CEntity</code> class.
   */
      ...
      for (i = 0; i < partial_edef.attributesRedecl.length; i++) {
        ...
        if (at.getName(null).equals(attributeName)) {
          if (count_of_found > 0) {
            ...
            if (((AttributeDefinition) at).attr_tp == AttributeDefinition.EXPLICIT && ((CExplicit_attribute) at).testRedeclaring(null)) {
  ...
  EAttribute find_attribute(String attributeName) throws SdaiException {
    ...
    if (attributes == null) {
      ...
      if (session != null && session.logWriterSession != null) {
        session.printlnSession(AdditionalMessages.DI_ATNF + SdaiSession.line_separator + AdditionalMessages.RD_ENT + ent_name
            + SdaiSession.line_separator + AdditionalMessages.RD_ATTR + attributeName);
      }
      else {
        SdaiSession.println(AdditionalMessages.DI_ATNF + SdaiSession.line_separator + AdditionalMessages.RD_ENT + ent_name + SdaiSession.line_separator
      ...
      if (indx < 0) {
        ...
        throw new SdaiException(SdaiException.SY_ERR, "Entity not found in the table: " + ent_name + " (used in model: " + error_mod.name + ")");
      }
...
//		} // syncObject
  ...
  /* private */
  static void sortWhereRules(CWhere_rule[] s_rules, CWhere_rule[] rules, int start_index, int end_index) {
    int i;
    int gap = end_index - start_index;
    if (gap < 7) {
      for (i = start_index; i < end_index; i++) {
        for (int j = i; j > start_index && ((CEntity) rules[j - 1]).instance_position > ((CEntity) rules[j]).instance_position; j--) {
    ...
    for (i = start_index, m = start_index, n = middle; i < end_index; i++) {
      if (n >= end_index || m < middle && ((CEntity) s_rules[m]).instance_position <= ((CEntity) s_rules[n]).instance_position) {
        rules[i] = s_rules[m++];
      }
      else {
        rules[i] = s_rules[n++];
