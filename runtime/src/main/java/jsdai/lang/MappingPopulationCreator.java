...
public interface MappingPopulationCreator {
   ...
   * guarantees that each entity instance passed to <code>setSourceAttributeValues</code> through <code>sourceValues</code>
   * parameter is previosly passed to <code>createSourceInstance</code> method through <code>targetInstances</code> parameter.
   *
   ...
   * @throws SdaiException if underlying JSDAI exception occurs
   ...
   * called as part of AIM2ARM conversion during <code>convertMapping</code> invocation.
   * It is invoked by the algorithm only once per <code>type</code>, <code>targetInstances</code> pair.
   *
   ...
   *                       The keys of this map are <code>generic_attribute_mappings</code> for all attributes of this source (ARM) instance.<br>
   *                       The values are either objects corresponding simple types,
   *                       entity instance as target (AIM) or <code>Collection</code> of before mentioned objects for aggregates.
   * @throws SdaiException if underlying JSDAI exception occurs
