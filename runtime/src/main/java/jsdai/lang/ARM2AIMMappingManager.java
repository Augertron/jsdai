...
class ARM2AIMMappingManager {
      ...
      try {
        ...
        if (istream != null) {
          ...
          while (prop_elems.hasMoreElements()) {
            ...
            EMappedARMEntity armEntityEntry = (EMappedARMEntity) Class.forName(armClassStr, true,
                SdaiClassLoaderProvider.getDefault().getClassLoader()).newInstance();
            Object armDefinition = armEntityEntry.getInstanceType();
            EEntity aimEntityEntry = (EEntity) Class.forName(aimClassStr, true, SdaiClassLoaderProvider.getDefault().getClassLoader())
                .newInstance();
            ...
            if (!found) {
              throw new SdaiException(SdaiException.ED_NVLD, "The mapping wasn't found for these definitions:" + armDefinition + "->"
                  + aimDefinition);
            }
          }
        }
  ...
  protected static EEntity_mapping findMostSpecificEntityMapping(AEntity_mapping mappings, boolean mostAbstract) throws SdaiException {
    ...
    for (int i = 1; i < mappingsCount; i++) {
      ...
      if ((target.isSubtypeOf(otherTarget) && (mostAbstract)) || (otherTarget.isSubtypeOf(target) && (!mostAbstract))) {
        rv = otherMapping;
        target = otherTarget;
      }
    }
    ...
    for (int i = 0; i < mappingsCount; i++) {
      ...
      EEntity otherTargetEntity = otherMapping.getTarget(null);
      EEntity_definition otherTarget = (EEntity_definition) otherTargetEntity;
      if ((!otherTarget.isSubtypeOf(target)) && (!otherTarget.isKindOf(target))) {
        throw new SdaiException(SdaiException.ED_NVLD, "Entity mappings targets aren't subtypes of each other: " + rv + ", " + otherMapping);
      }
    }
      ...
      try {
        ...
        if (istream != null) {
          ...
          while (prop_elems.hasMoreElements()) {
            ...
            EEntity aimEntity = (EEntity) Class.forName(aimClassStr, true, SdaiClassLoaderProvider.getDefault().getClassLoader()).newInstance();
            ...
            if (reuseFlag.compareTo(REUSE_GLOBALLY_STR) == 0) {
              ...
              throw new SdaiException(SdaiException.VA_NVLD, "This type of reuse flag is not supported: \"" + reuseFlag + "\", Class: "
                  + aimEntity.getClass());
            }
