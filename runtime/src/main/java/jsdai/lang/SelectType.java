...
public abstract class SelectType extends DataType {
  ...
  void enumerateSelectPaths(SdaiContext context) throws SdaiException {
    if (SdaiSession.debug2) {
      System.out.println(" IN ENUM SEL " + instance_identifier +
...
//		ANamed_type sels = ((ESelect_type)this).getSelections(null, context);
    ...
    for (int i = 1; i <= ((AEntity) sels).myLength; i++) {
      ...
      if (SdaiSession.debug2) {
        System.out.println(" alternative " + ((CEntity) alternative).instance_identifier + " name: " + alternative.getClass().getName());
      }
    ...
    if (SdaiSession.debug2) {
      System.out.println(" AFTER ENUM SEL  " + count);
    }
    shrinkSelectPaths();
...
//System.out.println(" New path No. " + count + "   of length = " + ttt + "  sel type" + this);
      for (int i = 0; i < path_length; i++) {
        if (SdaiSession.debug2) {
          System.out.println(" route 1: " + route[i].getName(null));
        }
      ...
      if (SdaiSession.debug2) {
        System.out.println(" route 2: " + def_type.getName(null)
      ...
      count++;
      if (SdaiSession.debug2) {
        System.out.println("  SIMPLE type  " + count);
      }
      count_other_leaves++;
    }
...
//}
  ...
  int getSelectSize(int select_number) throws SdaiException {
    if (select_number <= 1 || select_number > count + 1) {
      return -1;
    }
    CDefined_type[] row = paths[select_number - 2];
...
//System.out.println("## in getSelectArray - paths lenght: " + paths.length + ", select_number: " + select_number);
  ...
  protected ANamed_type getSelectionsExtensible(CExplicit_attribute attr, SdaiContext context) throws SdaiException {
    ...
    SdaiModel own_mod; // added when considering Bug #5667
    ...
    if ((express_type >= DataType.EXTENDED_SELECT && express_type <= DataType.ENT_EXT_NON_EXT_SELECT) ||
      ...
      for (i = 0; i < selExt.length; i++) {
        own_mod = ((CEntity) selExt[i]).owning_model; // added when considering Bug #5667
        own_mod.optimized = true; // added when considering Bug #5667
        aggr.addUnordered(selExt[i]);
        own_mod.optimized = false; // added when considering Bug #5667
      }
    }
    else {
      ...
      for (i = 0; i < ((AEntity) self_selections).myLength; i++) {
        CEntity inst = (CEntity) self_selections.getByIndex(i + 1); // added when considering Bug #5667
        own_mod = inst.owning_model; // added when considering Bug #5667
        own_mod.optimized = true; // added when considering Bug #5667
//				aggr.addUnordered(self_selections.getByIndex(i + 1));  // commented when considering Bug #5667
        aggr.addUnordered(inst); // added when considering Bug #5667
        own_mod.optimized = false; // added when considering Bug #5667
      }
    }
    if (express_type >= DataType.EXTENSIBLE_SELECT && express_type <= DataType.ENT_EXT_EXT_SELECT) {
    ...
    if (!first) {
      ...
      for (i = 1; i <= ((AEntity) self_selections).myLength; i++) {
        ...
        if (isDifferent(aggr, selection, count)) {
          own_mod = ((CEntity) selection).owning_model; // added when considering Bug #5667
          own_mod.optimized = true; // added when considering Bug #5667
