...
abstract class SdaiLoggingForwardIterableEvent extends SdaiLoggingIterableEvent {
  ...
  /**
   * Constructs a new object of <code>SdaiLoggingForwardIterableEvent</code> with methods which allow to iterate over records in
   * the undo/redo log file.
   *
   ...
   */
  SdaiLoggingForwardIterableEvent(SdaiSession source, int id, int item, Object argument) {
      ...
      if (!finished) {
        ...
        switch (token) {
          ...
          case 'e': {
            ...
            operationRepository = session.findRepositoryByIdentity(session.undo_redo_file.readInt());
            operationInstanceType = null;
            operationModel = null;
            break;
          }
