...
public abstract class CMappingExplicit_attribute extends AttributeDefinition implements MappingConstraintPath, MappingConstraintMatcher {
  ...
  public MatcherInstances findForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    MatcherInstances outInstances = mappingContext.getCacheInstances(this, instances, decCacheUseCnt);
    if (outInstances != null) {
      return outInstances;
    }
    else {
      return generalFindForward(this, mappingContext, instances.dup(), false, decCacheUseCnt);
    }
  }
  ...
  public MatcherInstances findBackward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      return generalFindBackward(this, mappingContext, instances.dup(), decCacheUseCnt);
    }
  }
  ...
  public MatcherInstances findPathForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      return generalFindForward(this, mappingContext, instances.dup(), true, decCacheUseCnt);
    }
  }
  ...
  static MatcherInstances generalFindForward(EAttribute attribute, MappingContext mappingContext, MatcherInstances instances, boolean inPath,
      boolean decCacheUseCnt) throws SdaiException {
    MatcherInstances outInstances;
    boolean dontTestPath = instances.attributePathTop < 0
        || (instances.attributePathTop == 0 && instances.isAttributeMemberAt(0) && instances.getAttributeMemberAt(0) < 0);
    ...
    if (instanceMap.equals(workingModels.getInstances())) {
      ...
      outInstances = new MatcherInstances(new AEntityAttrMap(attribute, dontTestPath ? null : instances.dup(), instanceAgg, inPath), instances.status);
      if (!inPath) {
        Map valueMap = new HashMap();
        MatcherInstances constraintUserInstances = instances.dup(false);
        findConstraintUsers(mappingContext, constraintUserInstances, attribute, constraintUserInstances, attribute, valueMap);
        if (!valueMap.isEmpty()) {
      ...
      if (dontTestPath) {
        ...
        while (instanceIter.hasNext()) {
          ...
          while (aggValueIter.next()) {
            ...
            Object pathValue = instances.followAttributePath(aggValue.get_object(attribute), instances.attributePathTop, mappingContext.selectPath);
      ...
      outInstances = new MatcherInstances(new ImmutableArrayMap(outInstanceMap), instances.status);
    }
  ...
  static MatcherInstances generalFindBackward(EAttribute attribute, MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt)
      throws SdaiException {
    Map instanceMap = instances.extractType(mappingContext, attribute.getParent_entity(null)).getInstanceMap();
    Map outInstanceMap = new HashMap();
    Iterator instanceIter = instanceMap.entrySet().iterator();
    while (instanceIter.hasNext()) {
      Map.Entry entry = (Map.Entry) instanceIter.next();
      ...
      if (((CEntity) instance).testAttributeFast(attribute, mappingContext.selectPath) > 0) {
        ...
        if (instances.attributePathTop >= 0) {
          value = instances.followAttributePath(value, instances.attributePathTop, mappingContext.selectPath);
          if (value == null) {
            continue;
          }
        ...
        if (value instanceof EEntity) {
          ...
          throw new SdaiException(SdaiException.SY_ERR, "Unexpected value: " + value + " class: " + value.getClass());
        }
      }
    }
    convertValueSets(outInstanceMap);
    MatcherInstances outInstances = new MatcherInstances(new ImmutableArrayMap(outInstanceMap), instances.status);
    mappingContext.setCacheInstances(attribute, instances, outInstances, decCacheUseCnt);
    return outInstances;
  }

  private static void findConstraintUsers(MappingContext mappingContext, MatcherInstances instances, EAttribute attribute,
      MatcherInstances constraintInstances, EEntity parentConstraint, Map valueMap) throws SdaiException {
    ...
    while (constraintUserIter.next()) {
      ...
      if (constraintUser instanceof EString_constraint) {
        ...
        value = new Integer(LangUtils.convertEnumerationStringToInt(stringValue, definedType));
      }
        ...
        amcInstances.attributePathReversePush(constraint.testMember(null) ? constraint.getMember(null) : -1);
        findConstraintUsers(mappingContext, instances, attribute, amcInstances, constraint, valueMap);
      }
        ...
        findConstraintUsers(mappingContext, instances, attribute, scInstances, constraint, valueMap);
      }
      if (value != null) {
        ...
        if (attribute instanceof EDerived_attribute) {
          ...
          mappingContext.setUserCacheInstances(attribute, constraintUser, instances,
              constraintInstances.dup(new AEntityMap(instanceAgg, false), constraintInstances.status));
        }
        else {
          MatcherInstances cuInstances = constraintInstances.dup(new HashMap(), constraintInstances.status);
  ...
  private static void testValueInMap(MappingContext mappingContext, EEntity instance, Object value, Object attrValue, Map valueMap) throws SdaiException {
    if (value instanceof Aggregate) {
      ...
      while (valueAggIter.next()) {
        if (valueAggregate.testCurrentMember(valueAggIter, mappingContext.selectPath) >= 0) {
          testValueInMap(mappingContext, instance, valueAggregate.getCurrentMemberObject(valueAggIter), attrValue, valueMap);
        }
      }
    }
    else {
      ...
      if (valueInstances instanceof List) {
        ...
        while (valueInstIter.hasNext()) {
          addMatcherInstances(mappingContext, instance, attrValue, (MatcherInstances) valueInstIter.next());
        }
      }
      else if (valueInstances != null) {
        addMatcherInstances(mappingContext, instance, attrValue, (MatcherInstances) valueInstances);
      }
    }
  }

  private static void addMatcherInstances(MappingContext mappingContext, EEntity instance, Object value, MatcherInstances constraintInstances)
      throws SdaiException {
    if (constraintInstances.attributePathTop >= 0
        && (constraintInstances.attributePathTop != 0 || !constraintInstances.isAttributeMemberAt(0) || constraintInstances.getAttributeMemberAt(0) > 0)) {
      if (constraintInstances.followAttributePath(value, constraintInstances.attributePathTop, mappingContext.selectPath) != null) {
  ...
  static final class AEntityAttrSet extends AEntitySet {
    ...
    private final MatcherInstances matcherInstances;
    private int size;
    private final EDefined_type[] selectPath;
    ...
    public boolean equals(Object obj) {
      if (obj instanceof AEntityAttrSet) {
        ...
        return instances.equals(other.instances) && attribute == other.attribute && matcherInstances.equalsAttributePath(other.matcherInstances);
      }
      else {
        return false;
      }
    ...
    private Object isMatchingInstance(EEntity instance) throws SdaiException {
      if (((CEntity) instance).testAttributeFast(attribute, selectPath) > 0) {
        ...
        if (matcherInstances != null) {
          if (matcherInstances.followAttributePath(pathValue, matcherInstances.attributePathTop, selectPath) != null) {
            return pathValue;
          }
        }
        return pathValue;
  ...
  static final class AEntityAttrMap extends AEntityMap {
    ...
    AEntityAttrMap(EAttribute attribute, MatcherInstances matcherInstances, AEntity instances, boolean inPath) {
    ...
    public boolean equals(Object obj) {
      if (obj instanceof AEntityAttrMap) {
        ...
        return instances.equals(other.instances)
            ...
            && ((matcherInstances == null && other.matcherInstances == null) || (matcherInstances != null && other.matcherInstances != null && matcherInstances
            .equalsAttributePath(other.matcherInstances)));
      }
      else {
        return false;
      }
      ...
      public boolean hasNext() {
        try {
          if (hasNextResult < 0) {
            ...
            hasNextResult = isNext ? 1 : 0;
            if (isNext) {
              currEntry = new AEntityAttrEntry(currInstance, inPath ? pathValue : NON_PATH_VALUE);
            }
            else {
              currEntry = null;
            }
    ...
    protected class AEntityAttrEntry extends AbstractCollection implements Entry {
      ...
      public boolean equals(Object obj) {
        ...
        if (obj instanceof Entry) {
          ...
          return keyInstance.equals(other.getKey()) && (getValue() == null ? other.getValue() == null : getValue().equals(other.getValue()));
        }
      ...
      public int hashCode() {
        if (value instanceof Aggregate) {
          ...
          return (keyInstance == null ? 0 : keyInstance.hashCode()) ^ (value == null ? 0 : value.hashCode());
        }
      }
            ...
            while (hasNextResult < 0) {
              ...
              try {
                ...
                while (sdaiIterators[sdaiIterIdx].next() && hasNextResult < 0) {
                  if (aggregates[sdaiIterIdx].testCurrentMember(sdaiIterators[sdaiIterIdx], selectPath) >= 0) {
                    currValue = aggregates[sdaiIterIdx].getCurrentMemberObject(sdaiIterators[sdaiIterIdx]);
                    hasNextResult = 1;
                  }
                }
              }
