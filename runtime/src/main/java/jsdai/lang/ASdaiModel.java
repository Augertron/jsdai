...
public class ASdaiModel extends SessionAggregate {
  ...
  /**
   * Constructs a new object of aggregate <code>ASdaiModel</code> without aggregation type. The aggregates of such kind are used
   ...
   * the first parameter is of type <code>SdaiModel</code> and the
...
//		} // syncObject
   ...
   * [])} method - the second parameter is of type <code>SdaiModel</code> and the third parameter is dropped.
   *
   * @param index
   * @param value
   */
...
//		} // syncObject
  ...
  /**
   * Returns a read-only aggregate containing all instances of all started <code>SdaiModel</code>s in this
...
//		} // syncObject
   ...
   * collected going through all started <code>SdaiModel</code>s in this <code>ASdaiModel</code>.
   ...
   * <B>Example:</B>
   * <p>
   * <pre>    ASdaiModel maggr = ...;
   *    EEntity_definition cp_type = ...;  // cartesian point
   *    ACartesian_point aggr = (ACartesian_point)maggr.getInstances(cp_type);</pre>
   *
...
//		} // syncObject
   ...
   * <p>
   * <pre>    ASdaiModel maggr = ...;
   *    ACartesian_point aggr = (ACartesian_point)maggr.getInstances(ECartesian_point.class);</pre>
   *
...
//		ListElement element = (ListElement)mod_list.myData[0];
    ...
    if (schema_data == null) {
      ...
      String dict_name = name.substring(SdaiSession.SCHEMA_PREFIX_LENGTH, last_dot).toUpperCase() + SdaiSession.DICTIONARY_NAME_SUFIX;
...
//		} // syncObject
  ...
  private AEntity getInstancesInternal(SchemaData sch_data, int index, Object type, CSet_type set_type) throws SdaiException {
...
// Temporary solution
   ...
   * <pre>    ASdaiModel maggr = ...;
   *    EEntity_definition cp_type = ...;  // cartesian point
   *    ACartesian_point aggr = (ACartesian_point)maggr.getExactInstances(cp_type);</pre>
   *
   * @param type definition of the entity of which instances are required.
   * @return aggregate containing all instances of the specified
   * entity data type taken from all started <code>SdaiModel</code>s
...
//		} // syncObject
   ...
   * <p>
   * <pre>    ASdaiModel maggr = ...;
   *    ACartesian_point aggr = (ACartesian_point)maggr.getExactInstances(CCartesian_point.class);</pre>
   *
   * @param type Java class for the entity of which instances are required.
   * @return aggregate containing all instances of the specified
   * entity data type taken from all started <code>SdaiModel</code>s
    ...
    if (myType == null || myType.express_type == DataType.LIST) {
      ...
      while (element != null) {
        ...
        if (model.underlying_schema != null && (model.mode & SdaiModel.MODE_MODE_MASK) > 0) {
          index_to_entity = model.underlying_schema.owning_model.schemaData.findEntityExtentIndex(type);
          if ((model.mode & SdaiModel.MODE_MODE_MASK) == SdaiModel.READ_ONLY && model.repository != SdaiSession.systemRepository) {
            index_to_entity = model.find_entityRO(model.underlying_schema.owning_model.schemaData.entities[index_to_entity]);
          }
          if (index_to_entity >= 0) {
            count += model.lengths[index_to_entity];
          }
...
//		}
   ...
   * <B>Example:</B>
   * <p>
   * <pre>    ASdaiModel mod_aggr = ...;
   *    System.out.println(mod_aggr);</pre> &nbsp;&nbsp;&nbsp;&nbsp;The string printed will be like this:
   *
   * <pre>
   * SdaiModels: mod, model2, model3, mod_special
   * </pre>
   *
...
//		} // syncObject
   ...
   * Instances that satisfies mapping constraints must be in this aggregate of models, but other instances required by mapping
   * constraints may be in other models within given data domain.
   ...
   *                      MOST_SPECIFC_ENTITY} - returned mappings are restricted to most specific, {@link EEntity#MANDATORY_ATTRIBUTES_SET
   ...
   */
  public AEntity findMappingInstances(EEntity_definition sourceEntity, ASdaiModel dataDomain, ASdaiModel mappingDomain, AEntity_mapping instanceMappings,
   ...
   * Instances that satisfy mapping constraints must be in supplied aggregate of model, but other instances required by mapping
   * constraints may be in other models within data domain.
   * <p>
   * <B>Example:</B>
   * <p>
   * <pre>    ASdaiModel m = ...;
  ...
  public AEntity findMappingInstances(EEntity_mapping entityMapping, ASdaiModel dataDomain, ASdaiModel mappingDomain, int mode) throws SdaiException {
    if (Implementation.mappingSupport) {
      return Mapping.findMappingInstances(this, entityMapping, dataDomain, mappingDomain, null, mode);
    }
    else {
   ...
   * @see <a href="package-summary.html#MappingExtension">JSDAI Mapping Extension</a>
   */
  public AEntity findMappingInstances(EEntity_mapping entityMapping, ASdaiModel dataDomain, ASdaiModel mappingDomain, AEntity_mapping instanceMappings,
      int mode) throws SdaiException {
    if (Implementation.mappingSupport) {
      return Mapping.findMappingInstances(this, entityMapping, dataDomain, mappingDomain, instanceMappings, mode);
    }
    else {
      throw new SdaiException(SdaiException.FN_NAVL, AdditionalMessages.MP_NAVL);
    }
    ...
    for (SdaiIterator i = createIterator(); i.next(); ) {
      ...
      if (model.getMode() == SdaiModel.NO_ACCESS) {
        if (bridgeSession == null) {
          ...
          throw new SdaiException(SdaiException.SS_NAVL, "Model " + model + " belongs to a different bridge session");
        }
        if (bridgeSession != null && bridgeSession.isStartMultiModelAccessAvailable() && model.getModRemote() != null) {
  ...
  /**
   * Terminates any access, either read-only or read-write, for each <code>SdaiModel</code> in this aggregate. As a result, the
   * contents of all
