...
class Print_instance {
  ...
  private void write_byte(byte sym, boolean first, DataOutputStream output_stream) throws java.io.IOException {
    int next_row;
    if (!first) {
  ...
  private void write_byte_array(byte[] string, boolean first, DataOutputStream output_stream) throws java.io.IOException {
    int next_row;
    if (!first) {
      output_stream.write(PhFileReader.COMMA_b);
      row_length++;
  ...
  private void write_byte_array(boolean first, DataOutputStream output_stream) throws java.io.IOException {
    int next_row;
    if (!first) {
      output_stream.write(PhFileReader.COMMA_b);
      row_length++;
  ...
  private void write_byte_array(boolean first, byte sym, DataOutputStream output_stream) throws java.io.IOException {
    int i;
    int next_row;
  ...
  void write_byte_array_simple(boolean first, byte[] source, int source_ln, byte sym, boolean next_row, DataOutputStream output_stream)
  ...
  int write_byte_array(boolean first, DataOutputStream output_stream, String str, int row_ln) throws java.io.IOException {
    row_length = row_ln;
    fromString(str);
  ...
  void write_byte_array_simple(boolean first, byte[] source, byte sym, boolean next_row, DataOutputStream output_stream) throws java.io.IOException {
    write_byte_array_simple(first, source, source.length, sym, next_row, output_stream);
  }

  void write_byte_array_simple(boolean first, String str, byte sym, boolean next_row, DataOutputStream output_stream) throws java.io.IOException {
    fromString(str);
    StaticFields staticFields = StaticFields.get();
    write_byte_array_simple(first, staticFields.string, staticFields.string_length, sym, next_row, output_stream);
  }

  int write_byte_array(boolean first, DataOutputStream output_stream, byte[] str, int row_ln) throws java.io.IOException {
    row_length = row_ln;
    write_byte_array(str, first, output_stream);
    return row_length;
  }

  void print_instance(ComplexEntityValue entity_values, byte[] name, int name_length, DataOutputStream output_stream, int row_length_initial)
  ...
  private void print_instance(EntityValue pval, boolean short_names, SchemaData sch_data, DataOutputStream output_stream) throws java.io.IOException,
      SdaiException {
    boolean first;
    ...
    if (index < 0 || index >= sch_data.bNames.length) {
      String base = SdaiSession.line_separator + AdditionalMessages.WR_OUT + SdaiSession.line_separator + "  " + pval.def.name + "  "
  ...
  void print_instance(ComplexEntityValue entity_values, boolean short_names, SchemaData sch_data, DataOutputStream output_stream, int row_length_initial)
      throws java.io.IOException, SdaiException {
    output_stream.write(PhFileReader.LEFT_PARENTHESIS);
  ...
  void print_instance(EntityValue pval, DataOutputStream output_stream, int row_length_initial, boolean in_header) throws java.io.IOException, SdaiException {
    output_stream.write(PhFileReader.LEFT_PARENTHESIS);
    row_length = row_length_initial + 1;
    boolean first = true;
    for (int i = 0; i < pval.count; i++) {
  ...
  private boolean explore_value(boolean first, Value val, DataOutputStream output_stream, Object aggr) throws java.io.IOException, SdaiException {
    Value value_next;
    boolean first_next;
...
//System.out.println(" Print_instance   tag: " + val.tag);
      ...
      case PhFileReader.ENTITY_REFERENCE:
        ...
        if (val.reference instanceof SdaiModel.Connector) {
          ...
          if (ref_inst.owning_model == null) {
            throw new SdaiException(SdaiException.EI_NVLD, ref_inst, "Instance #" + ref_inst.instance_identifier + " of type "
                + ref_inst.getInstanceType().getName(null));
          }
  ...
  private void explore_value_for_header(boolean first, Value val, DataOutputStream output_stream, boolean in_header) throws java.io.IOException,
      SdaiException {
    Value value_next;
    boolean first_next;
    ...
    switch (val.tag) {
      ...
      case PhFileReader.REDEFINE:
        if (in_header) {
          String base = SdaiSession.line_separator + AdditionalMessages.WR_OUT + SdaiSession.line_separator + "     "
  ...
  private void take_integer_value(int integ) throws SdaiException {
    ...
    while (number != 0) {
      ...
      digit_index++;
      staticFields.string[digit_index] = PhFileWriter.DIGITS[number - next_number * 10];
      number = next_number;
    }
    if (neg) {
...
//					if (non_basic && string.length < count + 2) {
        ...
        case 2:
          ...
          staticFields.string[count + 2] = PhFileReader.BACKSLASH;
          staticFields.string[count + 3] = (byte) (sym - 128);
          count += 4;
          non_basic = true;
          break;
        case 3:
