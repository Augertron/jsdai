...
public class CMappingNegation_constraint extends CConstraint implements MappingConstraintMatcher {
  ...
  public MatcherInstances findForward(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      outInstances = ((MappingConstraintMatcher) selfInterface.getConstraints(null)).findForward(mappingContext, instances, false);
      outInstances = new NegationInstances(mappingContext, outInstances);

      mappingContext.setCacheInstances(this, instances, outInstances, decCacheUseCnt, new EEntity[] { selfInterface.getConstraints(null) });
      return outInstances;
    }
  }
  ...
  static class NegationInstances extends MatcherInstances {
    ...
    protected MatcherInstances dup(Map instances, int status, boolean dupAttributePath) {
      ...
      NegationInstances newMatcherInstances = new NegationInstances(mappingContext, this.instances, status);
    ...
    protected MatcherInstances doIntersect(MatcherInstances other) throws SdaiException {
      ...
      if (!instances.equals(workingModels.getInstances()) && !(otherNegation && other.instances.equals(workingModels.getInstances()))) {
      ...
      return new MatcherInstances(new ImmutableArrayMap(newInstances), (status & STATUS_PATH_MASK) != 0 ? status : other.status);
    }
    ...
    protected MatcherInstances extractType(MappingContext mappingContext, EEntity_definition type) throws SdaiException {
      ASdaiModel workingModels = mappingContext.context.working_modelAggr;
      MatcherInstances typeInstances = new MatcherInstances(workingModels.getInstances(), status).extractType(mappingContext, type);
    ...
    protected MatcherInstances extractExactType(MappingContext mappingContext, EEntity_definition type) throws SdaiException {
      ASdaiModel workingModels = mappingContext.context.working_modelAggr;
      MatcherInstances typeInstances = new MatcherInstances(new AEntityMap(workingModels.getExactInstances(type), (status & STATUS_PATH_MASK) != 0),
          status);
      return doIntersect(typeInstances);
    }
        ...
        if (instances.isEmpty()) {
          ...
          if (!instances.equals(allInstances)) {
            ...
            while (instanceIter.next()) {
              EEntity instance = allInstances.getCurrentMemberEntity(instanceIter);
