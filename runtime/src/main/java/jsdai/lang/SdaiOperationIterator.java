...
/**
 * <p>
 * Implicit supertype for all classes aiding to run through entity instances for which some changes (due to their modification,
 * creation, deletion or substitution) were recorded in the log file for subsequent undo/redo operations. The records are divided
 * into segments called groups. The classes that implement this interface are designed to iterate over instances described by
...
public interface SdaiOperationIterator {
   ...
   * <ul>
   * <li>iterating records for undo operation; in this case, the records of a group are searched in the backward direction, and
   * the first instance in the above defined sequence corresponds to the last record in the group;
   * <li>iterating records for redo operation; in this case, the records of a group are searched in the forward direction.
   * </ul>
   *
   * @return <code>true</code> if iterator has a new current member (instance), <code>false</code> otherwise.
   ...
   * on which an operation was performed.
   * The repository is available only for <code>CREATE_EXTERNAL_DATA_OPERATION</code> and
   * <code>REMOVE_EXTERNAL_DATA_OPERATION</code>.
   *
   * @return the owning <code>SdaiRepository</code> of the entity instance or <code>null</code> if repository is not known.
   * @since 4.2.0
   */
