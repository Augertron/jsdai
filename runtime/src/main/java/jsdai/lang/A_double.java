...
//		synchronized (syncObject) {
    ...
    if (myType.express_type == DataType.LIST && !(myData instanceof double[])) {
...
//		synchronized (syncObject) {
    ...
    else if (myType.express_type == DataType.LIST) {
      index--;
      if (index < 0 || index >= myLength) {
        throw new SdaiException(SdaiException.IX_NVLD, this);
      }
...
//		} // syncObject
  ...
  void setValueSimple(Value val) throws SdaiException {
    if (myType.express_type == DataType.ARRAY) {
    ...
    for (int i = 0; i < val.length; i++) {
      myDataArray[i] = val.nested_values[i].getDouble(null);
    }
  }
...
//			index -= lower_index;
  ...
  boolean checkUniquenessViolation(AggregationType aggr_type) throws SdaiException {
    ...
    int j;
    if (aggr_type.express_type == DataType.LIST && !(myData instanceof double[])) {
