...
public class CMappingEntity_constraint extends CConstraint_attribute implements MappingConstraintPath, MappingConstraintMatcher {
  ...
  private MatcherInstances generalFind(MappingContext mappingContext, MatcherInstances instances, boolean decCacheUseCnt, int direction) throws SdaiException {
    ...
    if (outInstances != null) {
      ...
      EEntity_definition domain = selfInterface.getDomain(null);
      outInstances = this instanceof EExact_entity_constraint ? instances.extractExactType(mappingContext, domain) : instances.extractType(
          mappingContext, domain);
      MappingConstraintMatcher childMatcher = (MappingConstraintMatcher) selfInterface.getAttribute(null);
      switch (direction) {
        case 0:
          outInstances = childMatcher.findForward(mappingContext, outInstances, false);
          break;
        case 1:
          outInstances = childMatcher.findBackward(mappingContext, outInstances, false);
          break;
        case 2:
          outInstances = childMatcher.findPathForward(mappingContext, outInstances, false);
          break;
        case 3:
          outInstances = childMatcher.findPathBackward(mappingContext, outInstances, false);
          break;
      }
      mappingContext.setCacheInstances(this, instances, outInstances, decCacheUseCnt, new EEntity[] { (EEntity) childMatcher });
      return outInstances;
    }
  }
