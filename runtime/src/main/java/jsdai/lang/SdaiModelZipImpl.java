...
class SdaiModelZipImpl extends SdaiModelLocalImpl {
  ...
  long loadLocal(int mode_to_start) throws SdaiException {
    ...
    try {
      ...
      String base = SdaiSession.line_separator + AdditionalMessages.BF_MNF1 + fileSource.getAbsolutePath() + AdditionalMessages.BF_MNF2;
  ...
  public URL getLocationURL() throws SdaiException {
    ...
    try {
      ...
      JarFileURLStreamHandler handler = (JarFileURLStreamHandler) repository.session.jarFileURLStreamHandlers.get(repositoryUrl);
      if (handler == null) {
