...
import jsdai.dictionary.CImplicit_declaration$rule_declaration;
import jsdai.dictionary.CLocal_declaration$rule_declaration;
...
import jsdai.dictionary.ERule_declaration;
...
/**
 * The SchemaInstance class represents a logical collection of <code>SdaiModel</code>s.
 *
 * <p>
 * Conceptionally, the <code>SchemaInstance</code> can be represented by the following EXPRESS entity:
 *
 * <p>
 * <pre>ENTITY SchemaInstance;
 *  {@link #getName name} : String;
 *  {@link #getAssociatedModels associatedModels} : {@link ASdaiModel SET} [0:?] OF {@link SdaiModel SdaiModel};
 *  {@link #getNativeSchema nativeSchema} : {@link ESchema_definition ESchema_definition};
 ...
 * <p>
 * <p>
 * All attributes are read-only, and for each of them a corresponding <code>get</code> method (or methods sometimes) is defined.
 * Both the <code>changeDate</code> and <code>validationDate</code> can be retrieved either as <code>String</code> or as
 * <code>long</code>. Similarly, <code>nativeSchema</code> can be retrieved either as <code>ESchema_definition</code> or as
 * <code>String</code>. For <code>preprocessorVersion</code>, <code>originatingSystem</code>, <code>authorization</code>, and
 * <code>defaultLanguage</code> the corresponding <code>set</code> methods are given.
 *
 * <p>
 * A <code>SchemaInstance</code> belongs to an <code>SdaiRepository</code> and is available only when the
 * <code>SdaiRepository</code> is open. Closing an <code>SdaiRepository</code> makes its <code>SchemaInstance</code>s invalid. The
 * associated <code>SdaiModel</code>s can belong to different <code>SdaiRepositories</code>. The underlying EXPRESS schema of each
 * <code>SdaiModel</code> associated with some <code>SchemaInstance</code> needs to be domain equivalent with the native schema of
 * this <code>SchemaInstance</code>. The <code>SchemaInstance</code>s define the domain over which to perform validation
...
//	}
   ...
   * JSDAI library has its 'own' schema instance created during <code>openSession</code>.
   */
   ...
   *
   * @param given_name
   * @param given_repository
   * @param schema
   */
  protected SchemaInstance(String given_name, CSchema_definition schema, SdaiRepository given_repository) throws SdaiException {
   ...
   * @param given_repository
   */
  protected SchemaInstance(String given_name, SdaiRepository given_repository, SdaiModel given_dictionary) throws SdaiException {
    repository = given_repository;
   ...
   *
   * @param given_name
   * @param given_schema_model
   * @param given_repository
   * @param given_dictionary
   */
  protected SchemaInstance(String given_name, SdaiRepository given_repository, SdaiModel given_dictionary, SdaiModel given_schema_model) throws SdaiException {
    repository = given_repository;
    initLocal(given_name, given_dictionary, given_schema_model);
  }

  protected void initLocal(String given_name, SdaiModel given_dictionary, SdaiModel given_schema_model) throws SdaiException {
    name = given_name;
    dictionary = given_dictionary;
...
//		associated_models = new ASdaiModel(SdaiSession.setType0toN, this);
   ...
   * <p>
   * The schema instances within a <code>SdaiRepository</code> must have unique names.
   *
   ...
   * <p>
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this aggregate within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated
   * contains strings taken from the field <code>description</code> of the entity <code>file_description</code> in the header of
   * the exchange structure (see ISO 10303-21::8.2.1 file_description). During a read-write transaction the aggregate can be
   ...
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this aggregate within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated
   * contains strings taken from the field <code>author</code> of the entity <code>file_name</code> in the header of the
   * exchange structure (see ISO 10303-21::8.2.2 file_name). During a read-write transaction the aggregate can be modified by
   * adding/modifying/removing its members. It is a responsibility of the application to fill this aggregate with useful
   * information.
   *
   ...
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this aggregate within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated
   * contains strings taken from the field <code>organization</code> of the entity <code>file_name</code> in the header of the
   * exchange structure (see ISO 10303-21::8.2.2 file_name). During a read-write transaction the aggregate can be modified by
   * adding/modifying/removing its members. It is a responsibility of the application to fill this aggregate with useful
   * information.
   *
   ...
   * <p>
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this string within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated has
   * value taken from the field <code>preprocessor_version</code> of the entity <code>file_name</code> in the header of the
   * exchange structure (see ISO 10303-21::8.2.2 file_name).
   *
   * @return string describing the system used to build <code>SdaiModel</code>s associated with this schema instance.
   * @throws SdaiException SI_NEXS, schema instance does not exist.
   * @throws SdaiException RP_NOPN, repository is not open.
   ...
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this string within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated has
   * value taken from the field <code>originating_system</code> of the entity <code>file_name</code> in the header of the
   * exchange structure (see ISO 10303-21::8.2.2 file_name).
   *
   ...
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this string within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated has
   * value taken from the field <code>authorization</code> of the entity <code>file_name</code> in the header of the exchange
   ...
   * <p>
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this string within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated can
   * take value extracted from the header of the exchange structure (see ISO 10303-21::8.2.4 section_language) provided an
   * appropriate value is given there (otherwise it is set to null).
   *
   ...
   * describing the contexts within which the instances in the <code>SdaiModel</code>s associated with this schema instance are
   * applicable.
   ...
   * <p>
   * After invocation of {@link SdaiSession#importClearTextEncoding importClearTextEncoding} this aggregate within each
   * <code>SchemaInstance</code> with which at least one <code>SdaiModel</code> in the resulting repository is associated will
   * contain information extracted from the header of the exchange structure (see ISO 10303-21::8.2.5 section_context) provided
   * an appropriate information is given there. During a read-write transaction the aggregate can be modified by
   * adding/modifying/removing its members.
   *
...
//		} // syncObject
  ...
  /**
   * Returns the date of the most recent invocation of the <code>validateSchemaInstance</code> method for this schema instance.
   *
   * @return a <code>String</code> representing the date and time.
   * @throws SdaiException RP_NOPN, repository is not open.
   * @throws SdaiException SI_NEXS, schema instance does not exist.
...
//		} // syncObject
   ...
   * most recent invocation of the <code>validateSchemaInstance</code> method for this schema instance.
   *
   * @return a <code>long</code> representing the date and time.
   * @throws SdaiException RP_NOPN, repository is not open.
   * @throws SdaiException SI_NEXS, schema instance does not exist.
  ...
  /**
   * Gives the result returned by the most recent invocation of the <code>validateSchemaInstance</code> method for this schema
   * instance.
   *
...
//		synchronized (syncObject) {
    ...
    return defining_schema_model.described_schema;
//		} // syncObject
  }

  /**
   * Returns the name of the user that has placed exclusive lock
   * on remote schema instance. This method always returns <code>null</code> for local schema instance.
   *
   ...
   * <p>
   * Deleting of a <code>SchemaInstance</code> from "SystemRepository" is forbidden. In this case, SdaiException VT_NVLD is
   * thrown.
   *
...
//		repository.schemas.removeUnorderedRO(this);
   ...
   * <p>
   * Renaming of a <code>SchemaInstance</code> from "SystemRepository" is forbidden. In the case of such an attempt,
   * SdaiException VT_NVLD is thrown.
   *
...
//		} // syncObject
   ...
   * <p>
   * Adding a model to a <code>SchemaInstance</code> in "SystemRepository" is forbidden. In the case of such an attempt,
   * SdaiException VT_NVLD is thrown.
   *
   ...
   * then SdaiException VA_NSET is thrown.
   * <p>
   * Removing a model from a <code>SchemaInstance</code> in "SystemRepository" is forbidden. In the case of such an attempt,
   * SdaiException VT_NVLD is thrown.
   *
...
//			CWhere_rule w_rule = (CWhere_rule)element.object;
      ...
      if (br_count > 0 && (bp_rules_names[0].equals("") || ((WhereRule) w_rule).isRuleBypassed(bp_rules_names, br_count))) { // added for Bug #2739
...
//		} // syncObject
  ...
  public int validateGlobalRule(String rule, AWhere_rule nonConf) throws SdaiException {
    ...
    for (i = 0; i < model_dict.lengths[SdaiSession.IMPLICIT_DECL_RULE_DECL]; i++) {
      CImplicit_declaration$rule_declaration dec_impl = (CImplicit_declaration$rule_declaration) instances[i];
    ...
    if (rule_found == null) {
      ...
      for (i = 0; i < model_dict.lengths[SdaiSession.LOCAL_DECL_RULE_DECL]; i++) {
        CLocal_declaration$rule_declaration dec_loc = (CLocal_declaration$rule_declaration) instances[i];
...
//		throw new SdaiException(SdaiException.FN_NAVL);
  ...
  private int validateInstances(AEntity instances, AAttribute attrs_aggr, SdaiIterator it_insts, SdaiIterator it_rules, ASdaiModel models_dom)
...
//	}
  ...
  protected final void removeModelAssociatedWith(SdaiModel model, boolean initialize) throws SdaiException {
   ...
   * <ul>
   * <li>"2;1" if the aggregate <code>associated_models</code> contains at most one model;
   * <li>"3;1" otherwise; in this case for the schema instance an instance of the entity <code>file_population</code> in the
   * header of the exchange structure (see ISO 10303-21::8.2.4 file_population) is created;
   * </ul>
   * In this <code>file_population</code> instance the value standing for <code>associated_models</code> is missing, that is,
   * dollar sign is used.
   * This happens because all the data sections in the
   * resulting exchange structure file correspond to models in the <code>associated_models</code> set (see ISO 10303-21::8.2.4
   * file_population).
   * <p>
   * After execution of this method the schema instance itself remains unchanged. The method is disabled for the special schema
   * instance <code>data_dictionary</code> defined in <code>SdaiSession</code> class. In this case SdaiException FN_NAVL is
   * thrown.
   *
  ...
  public void exportClearTextEncoding(String location) throws SdaiException {
    exportClearTextEncodingGeneric(location, null, null, FileImplementationLevel.LEVEL_21);
  }

  /**
   * Creates a STEP file in a form of the exchange structure containing the data of this schema instance, that is, the
   * header information and all its associated models. This operation is allowed only if this schema instance was not
   * modified and no associated model was created, deleted or modified since the most recent commit or abort operation
   * was performed. If this condition is not satisfied, then SdaiException TR_RW is thrown. At the moment of invocation
   * of the method the repository owning this schema instance must be opened. If parameter <code>location</code> gets
   * some <code>String</code> value, then this value is used as a name for the resulting file. If, however,
   * <code>location</code> is null, then the file name is constructed by taking the name of this schema instance and
   * combining it with the extension "stp". The contents of each associated model is written down to the file as a
   * separate data section. The header of the file includes time-stamp which specifies the date and time when the schema
   * instance was created or most recently modified. The attribute <code>name</code> of the entity
   * <code>file_name</code> in the header of the exchange structure (see ISO 10303-21::8.2.2 file_name) is set to the
   * name of this schema instance.
   * <p>
   * In this <code>file_population</code> instance the value standing for <code>associated_models</code> is missing,
   * that is, dollar sign is used. This happens because all the data sections in the resulting exchange structure file
   * correspond to models in the <code>associated_models</code> set (see ISO 10303-21::8.2.4 file_population).
   * <p>
   * After execution of this method the schema instance itself remains unchanged. The method is disabled for the special
   * schema instance <code>data_dictionary</code> defined in <code>SdaiSession</code> class. In this case SdaiException
   * FN_NAVL is thrown.
   *
   * @param location            the name of the file created by the method.
   * @param implementationLevel the implementation level.
   * @throws SdaiException SI_NEXS, schema instance does not exist.
   * @throws SdaiException RP_NOPN, repository is not open.
   * @throws SdaiException TR_NEXS, transaction does not exist.
   * @throws SdaiException TR_NAVL, transaction currently not available.
   ...
   * @see #exportClearTextEncoding(String location, String file_name)
   * @see #exportClearTextEncoding(OutputStream location)
   * @see #exportClearTextEncoding(OutputStream location, String file_name) see "ISO 10303-21"
   * @since 4.0.0
   */
  public void exportClearTextEncoding(String location, FileImplementationLevel implementationLevel) throws SdaiException {
    exportClearTextEncodingGeneric(location, null, null, implementationLevel);
  }
   ...
   * this value is nonnull. If this value is <code>null</code>, then the attribute <code>name</code> is set to the name of this
   * schema instance.
   * The value of the attribute <code>implementation_level</code> of the
   * entity <code>file_description</code> (see ISO 10303-21::8.2.1 file_description)
   * can take one of the following values:
  ...
  public void exportClearTextEncoding(String location, String file_name) throws SdaiException {
    exportClearTextEncodingGeneric(location, null, file_name, FileImplementationLevel.LEVEL_21);
  }
   ...
   * instance was created or most recently modified. The attribute <code>name</code> of the entity
   * <code>file_name</code> in the header of the exchange structure (see ISO 10303-21::8.2.2 file_name) is set to the
   * string value submitted through the second method's parameter provided this value is non null. If this value is
   * <code>null</code>, then the attribute <code>name</code> is set to the name of this schema instance. The value of
   * the attribute <code>implementation_level</code> of the entity <code>file_description</code> (see ISO
   * 10303-21::8.2.1 file_description) can take one of the following values:
   * <ul>
   * <li>"2;1" if the aggregate <code>associated_models</code> contains at most one model;
   * <li>"3;1" otherwise; in this case for the schema instance an instance of the entity <code>file_population</code> in
   * the header of the exchange structure (see ISO 10303-21::8.2.4 file_population) is created;
   * </ul>
   * In this <code>file_population</code> instance the value standing for <code>associated_models</code> is missing,
   * that is, dollar sign is used. This happens because all the data sections in the resulting exchange structure file
   * correspond to models in the <code>associated_models</code> set (see ISO 10303-21::8.2.4 file_population).
  ...
  public void exportClearTextEncoding(String location, String file_name, FileImplementationLevel implementationLevel) throws
      SdaiException {
    exportClearTextEncodingGeneric(location, null, file_name, implementationLevel);
  }
   ...
   * header of the exchange structure (see ISO 10303-21::8.2.2 file_name) is set
   * to the name of this schema instance.
   * For more details about the exchange format see {@link #exportClearTextEncoding(String location)}.
   * Invoking this method is equivalent to making the following call: <code>exportClearTextEncoding(location, null);</code>
   *
  ...
  public void exportClearTextEncoding(OutputStream location) throws SdaiException {
    exportClearTextEncodingGeneric(null, location, null, FileImplementationLevel.LEVEL_21);
  }

  /**
   * Creates a STEP file in a form of the exchange structure containing the data of this schema instance, that is, the
   * header information and all its associated models. At the moment of invocation of the method the repository owning
   * this schema instance must be opened. The exchange structure is written to the supplied stream. The attribute
   * <code>name</code> of the entity <code>file_name</code> in the header of the exchange structure (see ISO
   * 10303-21::8.2.2 file_name) is set to the name of this schema instance. For more details about the exchange format
   * see {@link #exportClearTextEncoding(String location)}. Invoking this method is equivalent to making the following
   * call: <code>exportClearTextEncoding(location, null);</code>
   *
   * @param location            an <code>OutputStream</code> the exchange structure is directed to.
   * @param implementationLevel the file implementation level (see see ISO 10303-21::8.2.1 file_description).
   * @throws SdaiException SI_NEXS, schema instance does not exist.
   ...
   * @since 4.0.0
   */
  public void exportClearTextEncoding(OutputStream location, FileImplementationLevel implementationLevel) throws
      SdaiException {
    exportClearTextEncodingGeneric(null, location, null, implementationLevel);
  }
  ...
  public void exportClearTextEncoding(OutputStream location, String file_name) throws SdaiException {
    exportClearTextEncodingGeneric(null, location, file_name, FileImplementationLevel.LEVEL_21);
  }
   ...
   * this schema instance must be opened. The exchange structure is written to the supplied stream. The attribute
   * <code>name</code> of the entity <code>file_name</code> in the header of the exchange structure (see ISO
   * 10303-21::8.2.2 file_name) is set to the string value submitted through the second method's parameter provided this
   * value is nonnull. If this value is <code>null</code>, then the attribute <code>name</code> is set to the name of
   * this schema instance. For more details about the exchange format see
  ...
  public void exportClearTextEncoding(OutputStream location, String file_name,
      FileImplementationLevel implementationLevel) throws SdaiException {
    exportClearTextEncodingGeneric(null, location, file_name, implementationLevel);
  }
...
// Exporting.
    ...
    try {
      ...
      if (location_stream != null) {
        rTemp.exportClearTextEncoding(location_stream, file_name, implementationLevel);
      }
      else {
        rTemp.exportClearTextEncoding(location, file_name, implementationLevel);
      }
...
//"   ind: " + ind + "  mod: " + mod.name + "   all_inst_count: " + mod.all_inst_count);
  ...
  private void removeDuplicateModNames(StaticFields staticFields, SdaiModel model, SdaiModel[] other_repo_mods, int mods_count) throws SdaiException {
    int i;
    int count = 0;
    boolean first = true;
   ...
   * <p>
   * Applying inclusion operation to a <code>SchemaInstance</code> in "SystemRepository" is forbidden. In the case of such an
   * attempt, SdaiException VT_NVLD is thrown.
   *
   ...
   * then SdaiException VA_NSET is thrown.
   * <p>
   * Applying removal operation to a <code>SchemaInstance</code> in "SystemRepository" is forbidden. In the case of such an
   * attempt, SdaiException VT_NVLD is thrown.
   *
