...
//public static CEntity eee;
  ...
  /**
   * The field to store a sequence of defined types specifying select data type.
   */
  EDefined_type[] definedTypes;
...
//
  ...
  private Object getMixedInternal(StaticFields staticFields, CExplicit_attribute attr, CDefined_type def, CEntity inst,
    ...
    if (SdaiSession.debug2) {
      System.out.println("  IN getMixedInternal   tag = " + tag);
    }
    if (SdaiSession.debug2) {
      System.out.println("   DEFINED TYPE: " + def.getName(null));
    }
    switch (tag) {
      case PhFileReader.ENTITY_REFERENCE:
        sel_number = 1;
...
//						}
      ...
      case PhFileReader.TYPED_PARAMETER:
        if (SdaiSession.debug2) {
          System.out.println("  BEFORE findTyped");
        }
        value_returned = findTyped(staticFields, attr, def, inst, sel, root);
        if (SdaiSession.debug2) {
          System.out.println("  IN TYPED_PARAMETER sel_number = " + sel_number);
        }
        break;
      case PhFileReader.EMBEDDED_LIST:
    ...
    if (first && sel_number < 0) {
      if (staticFields.def_count > 0) {
        ...
        if (SdaiSession.debug2) {
          System.out.println(" GET MIXED  " + sel_number + "   value_returned = " + value_returned.getClass().getName());
        }
        if (SdaiSession.debug2) {
        ...
        if (SdaiSession.debug2) {
          if (inst.instance_identifier == 2281 || inst.instance_identifier == 2284) {
            for (int i = 0; i < staticFields.def_count; i++)
              System.out.print("  " + staticFields.route[i].getName(null));
            System.out.println();
          }
        }
      }
...
//"tag = " + tag);
  ...
  private CDefined_type findDefinedType(String def_name, CDefined_type def, SdaiContext context) throws SdaiException {
    if (SdaiSession.debug2) {
      System.out.println("  IN findDefinedType  def_name = " + def_name + "   defined type name = " + def.getName(null));
    }
    ...
    if (domain.express_type >= DataType.SELECT && domain.express_type <= DataType.ENT_EXT_EXT_SELECT) {
      ...
      if (SdaiSession.debug2) {
        System.out.println("  IN findDefinedType domain is SELECT type   " + ((AEntity) sels).myLength);
      }
      ...
      if (SdaiSession.debug2) {
        System.out.println("  IN findDefinedType domain is defined type" + ((CDefined_type) domain).getName(null));
      }
      return findDefinedType(def_name, (CDefined_type) domain, context);
    }
    if (SdaiSession.debug2) {
      System.out.println("  IN findDefinedType domain is of type: " + domain.getClass().getName());
    }
    return null;
  }
  ...
  private Aggregate findTypedAggregate(StaticFields staticFields, CDefined_type def_type, SelectType sel, Value root, CEntity inst)
    ...
    if (SdaiSession.debug2) {
      System.out.println("  Value  domain type: " + domain.getClass().getName() +
          "   inst: #" + inst.instance_identifier);
    }
...
//			}
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && int_value == Integer.MIN_VALUE) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(int_value, count, Integer.MIN_VALUE, inst);
            break;
          case 1:
            aggr.addAtTheEnd(int_value);
            break;
        }
        count++;
      }
    ...
    if (change_value > 0) {
      ...
      for (int i = 0; i < length; i++) {
        ...
        if ((aggr_type.express_type != DataType.ARRAY) && Double.isNaN(double_value)) {
          ...
          switch (type) {
            case 0:
              aggr.setForNonList(double_value, count, inst);
              break;
            case 1:
              aggr.addAtTheEnd(double_value);
              break;
          }
          count++;
        }
...
//			}
    ...
    if (SdaiSession.debug2) {
      System.out.println("  inst ident = " + inst.instance_identifier);
    }
    if (SdaiSession.debug2) {
      System.out.println("  attr inst ident = " + attr.instance_identifier);
    }
    if (SdaiSession.debug2) {
      System.out.println("  domain inst ident = " +
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && str == null) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(str, count, null, inst);
            break;
          case 1:
            aggr.addAtTheEnd(str, null);
            break;
        }
        count++;
      }
...
//			}
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && log_value == 0) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(log_value, count, 0, inst);
            break;
          case 1:
            aggr.addAtTheEnd(log_value);
            break;
        }
        count++;
      }
...
//			}
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && res == 0) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(res, count, 0, inst);
            break;
          case 1:
            aggr.addAtTheEnd(res);
            break;
        }
        count++;
      }
...
//			}
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && enum_val == 0) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(enum_val, count, 0, inst);
            break;
          case 1:
            aggr.addAtTheEnd(enum_val);
            break;
        }
        count++;
      }
...
//			}
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && binar == null) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(binar, count, null, inst);
            break;
          case 1:
            aggr.addAtTheEnd(binar, null);
            break;
        }
        count++;
      }
...
//System.out.println(" !!  domain = " + domain.getClass().getName());
    ...
    if (SdaiSession.debug2) {
      System.out.println("  aggr_type inst ident = " + aggr_type.instance_identifier);
    }

    if (SdaiSession.debug2) {
      if (aggr_type == null) {
        System.out.println("  returned NULL aggr_type");
      }
      else {
        System.out.println("  returned POSITIVE aggr_type");
      }
    }
    if (SdaiSession.debug2) {
      if (aggr_type.getAggregateClass() == null) {
        System.out.println("  returned NULL aggr_type.aggregateClass");
      }
      else {
        System.out.println("  returned POSITIVE aggr_type.aggregateClass");
      }
    }
    CAggregate aggr;
    try {
...
//		AggregationType a_type = (AggregationType)d_type;
    AggregationType a_type = (AggregationType) v_type;
//Class agr_class = a_type.getAggregateClass();
//System.out.println("Value  agr_class: " + agr_class.getName());
//System.out.println("Value  d_type class: " + d_type.getClass().getName());
    if (agg_owner instanceof CEntity) {
      inst = (CEntity) agg_owner;
    }
    else if (a_type != null) {
...
*/
  Aggregate getMixedAggregate(CExplicit_attribute attr, CEntity inst) throws SdaiException {
    ...
    CAggregate aggr = getInstanceAggregateInternal(attr, inst);
    if (SdaiSession.debug2) {
      System.out.println("  OWNING inst ident = " + inst.instance_identifier +
    ...
    if (SdaiSession.debug2) {
      System.out.println("  attr name = " + attr.getName(null));
    }
    if (aggr != null) {
      if (SdaiSession.debug2) {
        System.out.println(" In getMixedAggregate aggregation_type inst ident = " + ((CEntity) d_type).instance_identifier);
      }
...
//			}
    ...
    for (int i = 0; i < length; i++) {
      ...
      if ((aggr_type.express_type != DataType.ARRAY) && for_adding == null) {
        ...
        switch (type) {
          case 0:
            aggr.setForNonList(for_adding, count, null, inst);
            break;
          case 1:
            aggr.addAtTheEnd(for_adding, null);
            break;
        }
        count++;
      }
...
//			}
  ...
  /**
   * Returns a double aggregate of entity instances that is wrapped in an object of this class.
   * In the case of unset, null is returned.
   * <p> This method is used when dealing with Express expressions.
   * It is invoked in Express compiler generated classes.
   *
   * @param owner the entity instance owning the returned aggregate.
   * @param attr  the attribute whose value is the aggregate.
   * @return double aggregate of entity instances described by this class.
   * @throws SdaiException SY_ERR, underlying system error.
   * @since 3.6.0
   */
  public Aggregate getInstance2Aggregate(EEntity owner, EAttribute attr) throws SdaiException {
    if (owner != null) {
      agg_owner = (CEntity) owner;
      AggregationType a_type = (AggregationType) v_type;
      if (a_type != null) {
    ...
    return getInstance2Aggregate(attr);
  }

  /**
   * Returns a double aggregate of entity instances wrapped in an object of this class.
   * In the case of unset, null is returned.
   *
   * @param attr the attribute whose value is the aggregate.
   * @return double aggregate of entity instances described by this class.
   */
  public Aggregate getInstance2Aggregate(EAttribute attr) throws SdaiException {
    if (tag == INDETERMINATE) {
      return null;
    }
    if (tag != PhFileReader.EMBEDDED_LIST) {
    ...
    CEntity inst = null;
    AggregationType a_type = (AggregationType) v_type;
    /*System.out.println("Value  d_type: " + d_type + "   a_type: " + a_type + "   tag: " + tag);*/
    if (agg_owner instanceof CEntity) {
      inst = (CEntity) agg_owner;
    }
    else if (a_type != null) {
    ...
    try {
      if (a_type != null) {
        if (a_type == ExpressTypes.LIST_GENERIC_TYPE || a_type == ExpressTypes.SET_GENERIC_TYPE ||
            a_type == ExpressTypes.BAG_GENERIC_TYPE || a_type == ExpressTypes.AGGREGATE_GENERIC_TYPE) {
          if (remedy_aggr2_type()) {
            a_type = (AggregationType) d_type;
            aggr = (CAggregate) a_type.getAggregateClass().newInstance();
            aggr.myType = a_type;
          }
          else {
            Object memb_tp = nested_values[0].nested_values[0].reference;
            if (memb_tp instanceof CEntity) {
              CEntity entt = (CEntity) memb_tp;
              CEntityDefinition base = (CEntityDefinition) entt.getInstanceType();
              Class aggr_class = base.getEntityNestedAggregate(2);
//System.out.println("Value  created aggr: " + aggr_class.getName());
//System.out.println("Value  attribute: " + attr);
//System.out.println("Value  assigned a_type: " + a_type);
              DataType type = (DataType) ((CDerived_attribute) attr).getDomain(null);
//System.out.println("Value  domain of attr: " + type);
              aggr = (CAggregate) aggr_class.newInstance();
              aggr.myType = (AggregationType) type;
              spec_case = true;
            }
            else {
              aggr = new CAggregate();
...
//			aggr.setValue(a_type, inst, this, false, false);
      aggr.setValue(aggr_tp, inst, this, false, false);
    }
//System.out.println("Value  +++++++++  aggr.myLength: " + aggr.myLength);
//System.out.println("Value  +++++++++  aggr: " + aggr);
    return aggr;
  }

  private boolean remedy_aggr2_type() throws SdaiException {
    AggregationType ag_type = (AggregationType) d_type;
    if (ag_type == null || ag_type == ExpressTypes.LIST_GENERIC_TYPE || ag_type == ExpressTypes.SET_GENERIC_TYPE ||
        ag_type == ExpressTypes.BAG_GENERIC_TYPE || ag_type == ExpressTypes.ARRAY_GENERIC_TYPE ||
        ag_type == ExpressTypes.AGGREGATE_GENERIC_TYPE) {
      return false;
    }
    DataType member_tp = (DataType) ((EAggregation_type) d_type).getElement_type(null);
    if (member_tp.express_type < DataType.LIST || member_tp.express_type > DataType.AGGREGATE) {
      return false;
    }
    EData_type el_type = ((EAggregation_type) member_tp).getElement_type(null);
    if (el_type == ExpressTypes.GENERIC_TYPE) {
      return false;
    }
    for (int i = 0; i < length; i++) {
      Value memb = nested_values[i];
      for (int j = 0; j < memb.length; j++) {
        boolean res = memb.nested_values[j].isConforming(el_type);
        if (!res) {
          return res;
        }
      }
  ...
  /**
   * Returns a triple-nested aggregate of entity instances that is wrapped in an object of this class.
   * In the case of unset, null is returned.
   * <p> This method is used when dealing with Express expressions.
   * It is invoked in Express compiler generated classes.
   *
   * @param owner the entity instance owning the returned aggregate.
   * @param attr  the attribute whose value is the aggregate.
   * @return triple-nested aggregate of entity instances described by this class.
   * @throws SdaiException SY_ERR, underlying system error.
   * @since 3.6.0
   */
  public Aggregate getInstance3Aggregate(EEntity owner, EAttribute attr) throws SdaiException {
    if (owner != null) {
      agg_owner = (CEntity) owner;
      AggregationType a_type = (AggregationType) v_type;
      if (a_type != null) {
    ...
    return getInstance3Aggregate(attr);
  }

  /**
   * Returns a triple-nested aggregate of entity instances wrapped in an object of this class.
   * In the case of unset, null is returned.
   *
   * @param attr the attribute whose value is the aggregate.
   * @return triple-nested aggregate of entity instances described by this class.
   */
  public Aggregate getInstance3Aggregate(EAttribute attr) throws SdaiException {
    if (tag == INDETERMINATE) {
      return null;
    }
    if (tag != PhFileReader.EMBEDDED_LIST) {
...
//System.out.println("Value  <<<<<<<<<<BEFORE PRINT>>>>>>>>>>> ");
//inst.owning_model.print_value(this);
//System.out.println("Value  <<<<<<<<<<AFTER PRINT>>>>>>>>>>> ");
    boolean spec_case = false;
    try {
      if (a_type != null) {
        if (a_type == ExpressTypes.LIST_GENERIC_TYPE || a_type == ExpressTypes.SET_GENERIC_TYPE ||
            a_type == ExpressTypes.BAG_GENERIC_TYPE || a_type == ExpressTypes.AGGREGATE_GENERIC_TYPE) {
          if (remedy_aggr3_type()) {
            a_type = (AggregationType) d_type;
            aggr = (CAggregate) a_type.getAggregateClass().newInstance();
            aggr.myType = a_type;
          }
          else {
            Object memb_tp = nested_values[0].nested_values[0].nested_values[0].reference;
            if (memb_tp instanceof CEntity) {
              CEntity entt = (CEntity) memb_tp;
              CEntityDefinition base = (CEntityDefinition) entt.getInstanceType();
              Class aggr_class = base.getEntityNestedAggregate(3);
//System.out.println("Value  created aggr: " + aggr_class.getName());
//System.out.println("Value  attribute: " + attr);
//System.out.println("Value  assigned a_type: " + a_type);
              DataType type = (DataType) ((CDerived_attribute) attr).getDomain(null);
...
//System.out.println("Value  +++++++++  aggr: " + aggr);
  ...
  private boolean remedy_aggr3_type() throws SdaiException {
    AggregationType ag_type = (AggregationType) d_type;
    if (ag_type == null || ag_type == ExpressTypes.LIST_GENERIC_TYPE || ag_type == ExpressTypes.SET_GENERIC_TYPE ||
        ag_type == ExpressTypes.BAG_GENERIC_TYPE || ag_type == ExpressTypes.ARRAY_GENERIC_TYPE ||
        ag_type == ExpressTypes.AGGREGATE_GENERIC_TYPE) {
    ...
    DataType member2_tp = (DataType) ((EAggregation_type) member_tp).getElement_type(null);
    if (member2_tp.express_type < DataType.LIST || member2_tp.express_type > DataType.AGGREGATE) {
      return false;
    }
    EData_type el_type = ((EAggregation_type) member2_tp).getElement_type(null);
    if (el_type == ExpressTypes.GENERIC_TYPE) {
      return false;
    }
    for (int i = 0; i < length; i++) {
      Value memb = nested_values[i];
      for (int j = 0; j < memb.length; j++) {
        Value memb2 = memb.nested_values[j];
        for (int k = 0; k < memb2.length; k++) {
          boolean res = memb2.nested_values[k].isConforming(el_type);
          if (!res) {
            return res;
          }
        }
...
*/
  ...
  void setEnumeration(int value, CExplicit_attribute attr) throws SdaiException {
    ...
    if ((value > ee.myLength) || (value < 1)) {
      ...
      string = ee.getByIndex(value).toUpperCase();
    }
//		v_type = et;
  }
  ...
  void setAggrEnumeration(A_enumeration agg, A_string elements) throws SdaiException {
    ...
    if (agg.myType.express_type == DataType.LIST) {
      ...
      while (element != null) {
        ...
        str_value = elements.getByIndex(member);
...
//				String base = SdaiSession.line_separator + AdditionalMessages.SE_ERDD;
  ...
  private boolean check_array_compatibility(EArray_type left_type, EArray_type right_type,
    ...
    int i;
    EBound upper_bound = left_type.getUpper_index(null);
    int left_size;
    if (upper_bound instanceof CPopulation_dependent_bound) {
      left_size = ((CPopulation_dependent_bound) upper_bound).getBound_value(null, (CEntity) agg_owner, context);
    }
    else {
      left_size = upper_bound.getBound_value(null);
    }
    EBound lower_bound = left_type.getLower_index(null);
    if (lower_bound instanceof CPopulation_dependent_bound) {
      left_size -= ((CPopulation_dependent_bound) lower_bound).getBound_value(null, (CEntity) agg_owner, context);
    }
    else {
      left_size -= lower_bound.getBound_value(null);
    }
    upper_bound = right_type.getUpper_index(null);
    int right_size;
    if (upper_bound instanceof CPopulation_dependent_bound) {
      right_size = ((CPopulation_dependent_bound) upper_bound).getBound_value(null, (CEntity) agg_owner, context);
    }
    else {
      right_size = upper_bound.getBound_value(null);
    }
    lower_bound = right_type.getLower_index(null);
    if (lower_bound instanceof CPopulation_dependent_bound) {
      right_size -= ((CPopulation_dependent_bound) lower_bound).getBound_value(null, (CEntity) agg_owner, context);
    }
    else {
      right_size -= lower_bound.getBound_value(null);
    }
...
//				return false;
    ...
    if (!oneCompatibleToAnother(el_type, v_tp_el_type, context, false)) {
...
//System.out.println(" SchemaData   el_type: " + el_type);
  ...
  /**
   * Checks if this object of <code>Value</code> class has a value attached and if so,
   * then returns an indicator showing the type of the value.
   * More precisely, this method fulfills the following three purposes:
   * <OL>
   * <LI>checks whether this object has a value or no value is attached.
   * In the latter case, the return value is 0.</LI>
   * <LI>retrieves the type of the value (if attached). The numbering of the types is as follows:
   * <ul><li> 1 for object (EEntity, Aggregate, String, Binary);
   * <li> 2 for integer;
   * <li> 3 for double;
   * <li> 4 for boolean.</ul>
   * The return of 2 encompasses the cases where the value is of
   * EXPRESS type INTEGER, ENUMERATION and LOGICAL.
   * </LI>
   * <LI> if the value wrapped in this object is of the select type, then
   * this method retrieves the select path corresponding to this value.
   ...
   *               corresponding to the actual value wrapped in this object.
   * @return type of the value (0 if no value is stored).
   * @throws SdaiException SY_ERR, underlying system error.
   */
  public int test(EDefined_type[] select) throws SdaiException {
    if (dt_count > 0) {
      if (select == null || select.length < dt_count) {
        String base = SdaiSession.line_separator + AdditionalMessages.SE_ARTS;
        throw new SdaiException(SdaiException.SY_ERR, base);
      }
      for (int i = 0; i < dt_count; i++) {
        select[i] = definedTypes[i];
      }
      if (select.length > dt_count) {
        select[dt_count] = null;
      }
    }
    if (tag == PhFileReader.TYPED_PARAMETER) {
      return nested_values[0].testTyped();
    }
    else {
      return testTyped();
    }
  ...
  private int testTyped() throws SdaiException {
    switch (tag) {
      case INDETERMINATE:
        return 0;
      case PhFileReader.INTEGER:
    ...
    String base = SdaiSession.line_separator + AdditionalMessages.VA_FATV;
    throw new SdaiException(SdaiException.SY_ERR, base);
  }
...
/*	public void set(EEntity inst, EExplicit_attribute attr, Object obj_val, EDefined_type select[]) throws SdaiException {
		SelectType sel_type = ((CEntity)inst).take_select(attr);
		int sel_number = 0;
		if (sel_type != null) {
...
// not finished
		d_type = (EData_type)attr.getDomain(null);
		setMixed(obj_val, sel_type, sel_number);
		aux = 1;
	}*/

  /**
   * Writes the array of defined types submitted as a method's parameter to the
   * current object of this class. This array specifies the select path
   * which tells about the type of values that may be wrapped in the current
   * object. Select path is a sequence of defined data types needed to identify a
   * value of a select data type (as described in ISO 10303-21:11.1.8).
   ...
   * If the method's parameter is null, then the current object is not initialized
   * as representing an array of defined types (the length of this array is set to zero).
   *
   * @param select select path.
   */
  public void setDefinedTypes(EDefined_type[] select) throws SdaiException {
    dt_count = 0;
    if (select == null) {
      return;
    }
    if (definedTypes == null) {
      definedTypes = new EDefined_type[select.length];
    }
    else if (select.length > definedTypes.length) {
      int new_length = definedTypes.length * 2;
      if (new_length < select.length) {
        new_length = select.length;
      }
      definedTypes = new EDefined_type[new_length];
    }
    for (int i = 0; i < select.length; i++) {
      if (select[i] == null) {
        break;
      }
      definedTypes[i] = select[i];
      dt_count++;
    }
  }

  void getDefinedTypes(SelectType sel_type, int sel_number) throws SdaiException {
    if (sel_number <= 1) {
      dt_count = 0;
      return;
    }
    int size = sel_type.getSelectSize(sel_number);
    if (size < 0) {
      String base = SdaiSession.line_separator + AdditionalMessages.SE_INSI;
      throw new SdaiException(SdaiException.SY_ERR, base);
    }
    if (definedTypes == null) {
      definedTypes = new EDefined_type[2 * size];
    }
    else if (size > definedTypes.length) {
      int new_length = definedTypes.length * 2;
      if (new_length < size) {
        new_length = size;
      }
    ...
    dt_count = size;
    sel_type.giveDefinedTypes(sel_number, definedTypes);
  }
  ...
  public Value set(SdaiContext context, Object val) throws SdaiException {
    ...
    else if (val instanceof Aggregate) {
...
//		}
  ...
  public Value setLB(SdaiContext context, int val) throws SdaiException {
    ...
    if (val == 0) {
      ...
      if (((DataType) v_type).express_type == DataType.BOOLEAN) {
        tag = PhFileReader.BOOLEAN;
      }
      else {
        tag = PhFileReader.LOGICAL;
...
//		}
   ...
   * @see #indexing(SdaiContext, int, int)
   */
  public Value indexing(SdaiContext context, Value index1, Value index2) throws SdaiException {
    if (index1 == null) {
    ...
    return indexing(context, index1.integer, ind);
  }
//	public Value indexing(Value index1, Value index2) throws SdaiException {
//		return indexing(null, index1, index2);
//	}
   ...
   * @see "ISO 10303-11::12.5.1 String indexing"
   * @see "ISO 10303-11::12.6.1 Aggregate indexing"
   * @see #indexing(SdaiContext, Value, Value)
   */
  public Value indexing(SdaiContext context, int index1, int index2) throws SdaiException {
    Value val;
    if (tag == PhFileReader.TYPED_PARAMETER) {
      return nested_values[0].indexing(context, index1, index2);
    }
    if (tag == PhFileReader.EMBEDDED_LIST) {
      ...
      if (v_tp == DataType.ARRAY) {
        ...
        EBound upper_bound = arr_tp.getUpper_index(null);
        int ub;
        if (upper_bound instanceof CPopulation_dependent_bound) {
          ub = ((CPopulation_dependent_bound) upper_bound).getBound_value(null, (CEntity) agg_owner, context);
        }
        else {
          ub = upper_bound.getBound_value(null);
        }
        EBound lower_bound = arr_tp.getLower_index(null);
        int lb;
        if (lower_bound instanceof CPopulation_dependent_bound) {
          lb = ((CPopulation_dependent_bound) lower_bound).getBound_value(null, (CEntity) agg_owner, context);
        }
        else {
          lb = lower_bound.getBound_value(null);
        }
...
//				int ub = arr_tp.getUpper_index(null).getBound_value(null);
   ...
   * makes a call to {@link #indexing(SdaiContext, Value, Value)}.
   *
   * @param context context in which the function is considered.
   ...
   * @see #replaceRange(SdaiContext, int, int, Value)
   * @since 4.1.2
   */
  public Value replaceRange(SdaiContext context, Value index1, Value index2, Value repl) throws SdaiException {
    if (tag == INDETERMINATE) {
      throw new SdaiException(SdaiException.VA_NEXS);
    }
    if (tag != PhFileReader.STRING && tag != PhFileReader.BINARY && tag != PhFileReader.TYPED_PARAMETER) {
      return indexing(context, index1, index2);
    }
    if (tag == PhFileReader.TYPED_PARAMETER) {
      return nested_values[0].replaceRange(context, index1, index2, repl);
    }
    ...
    return replaceRange(context, index1.integer, index2.integer, repl);
  }
   ...
   * If the type of the data represented by this object differs from
   * STRING and BINARY, then the method ignores the last parameter and
   * makes a call to {@link #indexing(SdaiContext, int, int)}.
   *
   * @param context context in which the function is considered.
   * @param index1  index of the first element of STRING or BINARY to be replaced.
   * @param index2  index of the last element of STRING or BINARY to be replaced.
   ...
   * @throws SdaiException SY_ERR, underlying system error.
   * @see "ISO 10303-11::13.3.2 Assignment compatibility"
   * @see #replaceRange(SdaiContext, Value, Value, Value)
   * @since 4.1.2
   */
  public Value replaceRange(SdaiContext context, int index1, int index2, Value repl) throws SdaiException {
    if (tag == INDETERMINATE) {
      throw new SdaiException(SdaiException.VA_NEXS);
    }
    if (tag != PhFileReader.STRING && tag != PhFileReader.BINARY && tag != PhFileReader.TYPED_PARAMETER) {
...
//				}
  ...
  private boolean isCompatibleAggregation(DataType tp1, DataType tp2, SdaiContext context, boolean overrid) throws SdaiException {
    ...
    EData_type tp2_next;
    if (tp2 instanceof EAggregation_type) {
      tp2_next = ((EAggregation_type) tp2).getElement_type(null);
    }
    else {
      tp2_next = ExpressTypes.GENERIC_TYPE;
    }
    return oneCompatibleToAnother(((EAggregation_type) tp1).getElement_type(null),
        tp2_next, context, overrid);
  }
...
//			throw new SdaiException(SdaiException.SY_ERR);
  ...
  /**
   * Creates an object of this class and attaches to it the class wrapping an array the
   * elements of which represent values assigned to attributes of a simple entity data type.
   * These values are submitted through the parameter of the constructor.
   * This constructor is related to an implementation of complex entity instance
   * construction operator of Express language.
   *
   * @param eval an object of type <code>EntityValue</code> containing attribute
   *             values of the simple entity data type.
   * @see "ISO 10303-11::12.10 Complex entity instance construction operator"
   */
...
//					throw new SdaiException(SdaiException.SY_ERR);
   ...
   */
  public Value hiBound(SdaiContext context, Value val) throws SdaiException { // 15.10
    if (val == null) {
      printWarningToLogoValidate(AdditionalMessages.EE_NAVA);
      throw new SdaiException(SdaiException.VA_NEXS);
    }
    ...
    if (val.tag == PhFileReader.EMBEDDED_LIST) {
      if (((DataType) val.v_type).express_type == DataType.ARRAY) {
        ...
        if (upper_bound instanceof CPopulation_dependent_bound) {
          integer = ((CPopulation_dependent_bound) upper_bound).getBound_value(null, (CEntity) val.agg_owner, context);
        }
        else {
          integer = upper_bound.getBound_value(null);
        }
//				integer = arr_type.getUpper_index(null).getBound_value(null);
   ...
   */
  public Value hiIndex(SdaiContext context, Value val) throws SdaiException { // 15.11
    if (val == null) {
      printWarningToLogoValidate(AdditionalMessages.EE_NAVA);
      throw new SdaiException(SdaiException.VA_NEXS);
    }
...
//	public Value hiIndex(Value val) throws SdaiException {
//		return hiIndex(null, val);
//	}
   ...
   */
  public Value sizeOf(SdaiContext context, Value val) throws SdaiException { // 15.22
    if (val == null) {
      printWarningToLogoValidate(AdditionalMessages.EE_NAVA);
      throw new SdaiException(SdaiException.VA_NEXS);
    }
...
//			String base = SdaiSession.line_separator + AdditionalMessages.SE_ERDD;
    ...
    if (((DataType) val.v_type).express_type == DataType.ARRAY) {
      ...
      EBound upper_bound = arr_type.getUpper_index(null);
      if (upper_bound instanceof CPopulation_dependent_bound) {
        integer = ((CPopulation_dependent_bound) upper_bound).getBound_value(null, (CEntity) val.agg_owner, context) + 1;
      }
      else {
        integer = upper_bound.getBound_value(null) + 1;
      }
      EBound lower_bound = arr_type.getLower_index(null);
      if (lower_bound instanceof CPopulation_dependent_bound) {
        integer -= ((CPopulation_dependent_bound) lower_bound).getBound_value(null, (CEntity) val.agg_owner, context);
      }
      else {
        integer -= lower_bound.getBound_value(null);
      }
//			integer = arr_type.getUpper_index(null).getBound_value(null) -
...
//	public Value sizeOf(Value val) throws SdaiException {
//		return sizeOf(null, val);
//	}
...
//		String dict_name = qual_name.substring(0, index1) + SdaiSession.DICTIONARY_NAME_SUFIX;
  ...
  boolean validateValueAfterImport(DataType type, CEntity owning_instance, boolean ent_value) throws SdaiException {
//System.out.println("Value  tag = " + tag + "   INIT type.express_type: " + type.express_type +
//"  owning_instance: #" + owning_instance.instance_identifier);
    ...
    if (tag == PhFileReader.ENTITY_REFERENCE) {
      ...
      if (type.express_type == DataType.ENTITY) {
        CEntity ref_ent = (CEntity) reference;
        check_res = ref_ent.getInstanceType().isSubtypeOf((CEntity_definition) type);
//System.out.println("Value  ref_ent = #" + ref_ent.instance_identifier + "   check_res: " + check_res);
        if (!check_res && ent_value) {
          reference = null;
        }
        return check_res;
      }
      else if (type.express_type >= DataType.SELECT && type.express_type <= DataType.ENT_EXT_EXT_SELECT) {
        check_res = validateSelectValueAfterImport((ESelect_type) type, owning_instance);
        if (!check_res && ent_value) {
          reference = null;
        }
        return check_res;
    ...
    return validateAggrValueAfterImport(type, owning_instance);
  }

  private boolean validateAggrValueAfterImport(DataType type, CEntity owning_instance) throws SdaiException {
    type = (DataType) ((EAggregation_type) type).getElement_type(null);
//		if ((type.express_type != DataType.ENTITY) &&
//					(type.express_type < DataType.LIST || type.express_type > DataType.AGGREGATE)) {
//			return true;
//		}
    int i;
    ...
    for (i = 0; i < length; i++) {
      if ((nested_values[i].tag != PhFileReader.ENTITY_REFERENCE) && (nested_values[i].tag != PhFileReader.EMBEDDED_LIST)) {
        nested_values[i].aux = 0;
        continue;
      }
      boolean res = nested_values[i].validateValueAfterImport(type, owning_instance, false);
      if (!res) {
        ret_val = false;
        if (nested_values[i].tag == PhFileReader.ENTITY_REFERENCE) {
          nested_values[i].aux = 1;
          contracted = true;
        }
        else {
          nested_values[i].aux = 0;
    ...
    if (contracted) {
      int k = -1;
      for (i = 0; i < length; i++) {
        if (nested_values[i].aux == 0) {
          k++;
          if (k < i) {
            nested_values[k].copyValue(nested_values[i], owning_instance);
          }
        }
      }
      length = k + 1;
  ...
  boolean validateSelectValueAfterImport(ESelect_type sel_type, CEntity owning_instance) throws SdaiException {
    ANamed_type selections;
    if (((DataType) sel_type).express_type >= DataType.EXTENSIBLE_SELECT) {
      SdaiSession ss = owning_instance.owning_model.repository.session;
      if (!owning_instance.owning_model.export) {
        if (ss.sdai_context == null) {
          StaticFields staticFields = StaticFields.get();
          staticFields.context_schema = owning_instance.owning_model.underlying_schema;
          if (!ss.sdai_context_missing) {
            ss.sdai_context_missing = true;
            ss.printWarningToLogoSdaiContext(AdditionalMessages.SS_SCMI);
          }
        }
        owning_instance.owning_model.export = true;
      }
      selections = ((EExtensible_select_type) sel_type).getSelections(null, ss.sdai_context);
    }
    else {
      selections = sel_type.getSelections(null);
    }
    ...
    for (int i = 0; i < sels.myLength; i++) {
      ...
      if (element.express_type == DataType.DEFINED_TYPE) {
        while (element.express_type == DataType.DEFINED_TYPE) {
          element = (DataType) ((CDefined_type) element).getDomain(null);
        }
      }
      boolean check_res;
      if (element.express_type == DataType.ENTITY) {
        CEntity ref_ent = (CEntity) reference;
        check_res = ref_ent.getInstanceType().isSubtypeOf((CEntity_definition) element);
//System.out.println("Value  ref_ent = #" + ref_ent.instance_identifier + "   check_res: " + check_res);
        if (check_res) {
          return check_res;
        }
        ...
        check_res = validateSelectValueAfterImport((ESelect_type) element, owning_instance);
        if (check_res) {
          return check_res;
        }
      }
...
//		} // syncObject
  ...
  private boolean elementsCompatibleToAggr(EAggregation_type aggr_tp, SdaiContext context, boolean overrid) throws SdaiException {
    ...
    if (tp == ExpressTypes.LIST_GENERIC_TYPE || tp == ExpressTypes.SET_GENERIC_TYPE ||
        tp == ExpressTypes.BAG_GENERIC_TYPE || tp == ExpressTypes.ARRAY_GENERIC_TYPE ||
        tp == ExpressTypes.AGGREGATE_GENERIC_TYPE) {
      return true;
    }
