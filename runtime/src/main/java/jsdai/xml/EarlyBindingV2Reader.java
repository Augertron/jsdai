...
//		String schema_version =  //no versio attribute at this moment
    attributes.addAttribute(null, "schemaLocation", "schemaLocation", "CDATA", "URI:www.lksoft.com/Schema/" + schema_name + ".xml");
  ...
  protected void parseSchInst(SchemaInstance currSchInst, Map mIds, Map sIds) throws IOException, SAXException, SdaiException {
    ...
    attributes.addAttribute(null, "determination_methods", "determination_methods", "CDATA", "section_boundary");
  ...
  protected void parseModelInstances(SdaiModel currModel) throws SdaiException, SAXException {
    ...
    while (instanceIter.next()) {
      ...
      for (int i = entityAttr.length - 1; i >= 0; i--) {
        if (entityAttrFields[i] != null) {
          boolean ambiguousName = ((Boolean) entAttrNameAmbiguity.get(entityAttr[i].getName(null))).booleanValue();
  ...
  private void parseAttribute(EEntity instance, EExplicit_attribute attr, boolean ambiguousName) throws SdaiException, SAXException {
    ...
    if (attrType != 0) {
      ...
      if (ambiguousName) {
        attributeName = formatEntityName(attr.getParent_entity(null).getName(null)) + "." + attributeName;
      }
  ...
  private void parseAttributeValue(Object value, EEntity domain, EDefined_type[] select) throws SdaiException, SAXException {
      ...
      for (SdaiIterator it = aggregate.createIterator(); it.next(); ) {
        ...
        if (aggregate.testCurrentMember(it, memberSelect) != 0) {
          ...
          if (aggregateMember instanceof EEntity) {
            ...
            EEntity aggregateElementType = ((Aggregate) aggregateMember).getAggregationType().getElement_type(null); // should work
...
//                        System.out.println("got type:" + stringMemeberType + " className:" + memberType.getClass().getName());
  ...
  static private String getSimpleTypeSerializedName(EData_type type) throws SdaiException {
