...
public class InstanceWriterSelector extends JsdaiLangAccessor implements ContentHandler {
  ...
  private final InstanceWriter[] instanceWriters;
  ...
  public InstanceWriterSelector(SdaiRepository repository) {
    this.instanceWriters = new InstanceWriter[] { new LateBindingWriter(repository), new EarlyBindingV2Writer(repository) };
  }
   ...
   */
  static public void parse(InputStream location, ContentHandler handler) throws SAXException, IOException, ParserConfigurationException {
  ...
  public void startElement(String namespaceURI, String localName, String qName, Attributes attr) throws SAXException {
    if (currentWriter == null) {
      for (int i = 0; i < instanceWriters.length; i++) {
        ...
        if (instanceWriters[i].startRootElement(namespaceURI, localName, qName, attr, prefixes, uris)) {
