...
class MappingFwd extends Fwd {
  ...
  protected void setParameters(String entityName, String attributeName, String targetName, Node containerNode, Context context) throws SdaiException {
...
//		SdaiModel owningModel = localContext.getLocalQuery().owningModel;
    ...
    // Getting target
    if (targetName != null) {
      EEntity_definition targetEntity = JsdaiLangAccessorLocal.findSchemaEntityDefinition(schema, targetName);
      ...
      while (ctsIter.hasNext()) {
        ...
        while (targetEntMappingIter.next()) {
          EEntity_mapping targetEntMapping = targetEntMappings.getCurrentMember(targetEntMappingIter);
      ...
      while (ctsIter.hasNext()) {
        ...
        if (cType.isEmpty()) {
          throw new SdaiException(SdaiException.AT_NVLD, SdaiQueryEngine.formatMessage(containerNode, "Attribute is not a reference to the instance",
              attribute.toString()));
        }
      }
    }
  ...
  public void execute(Context context) throws SdaiException {
    ...
    while (cisIter.hasNext()) {
      ...
      while (currInstIter.hasNext()) {
        ...
        Object[] fwdVals = currInst.getMappedAttribute(attribute, localContext.getLocalQuery().domain, mappingDomain, EEntity.NO_RESTRICTIONS);
        if (fwdVals != null) {
