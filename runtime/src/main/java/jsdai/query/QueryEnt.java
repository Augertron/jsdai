...
class QueryEnt extends ConstraintContainer {
  ...
  private final Map queryFwdMap;
  private final Map queryValMap;
  ...
  protected void parse(Node containerNode, ConstraintContainer parent, Context context) throws SdaiException {
    ...
    if (xmlAttribute == null) {
      throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(containerNode, "Attribute name is required", null));
    }
    name = xmlAttribute.getNodeValue();
    boolean isQueryEntEmpty = true;
    Context queryTypeContext = null;
    for (Node queryChild = containerNode.getFirstChild(); queryChild != null; queryChild = queryChild.getNextSibling()) {
      if (queryChild.getNodeType() != Node.ELEMENT_NODE) {
        continue;
      }
      if (!context.query.isNamespaceHandler(queryChild.getNamespaceURI())) {
        throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(queryChild, "Unknown namespace: ", queryChild.getNamespaceURI()));
      }
      if (isQueryEntEmpty && queryChild.getLocalName().equals(QUERY_TYPE_ELEM)) {
        ...
        throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(queryChild, "Unrecognized element", null));
      }
  ...
  public void execute(Context context) throws SdaiException {
    throw new SdaiException(SdaiException.SY_ERR, "Method QueryEnt.execute CAN NEVER BE CALLED");
  }
  ...
  final QueryFwd getQueryFwd(String attributeName, String targetName, Node containerNode) throws SdaiException {
    QueryFwd queryFwd;
    ...
    if (queryFwd == null) {
      throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(containerNode, "Query-fwd with the attribute " + attributeName
          + " and target name " + targetName + " not found", null));
    }
    return queryFwd;
  }

  final QueryVal getQueryVal(String attributeName, String selectName, Node containerNode) throws SdaiException {
    QueryVal queryVal;
