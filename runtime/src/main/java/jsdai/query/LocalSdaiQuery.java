...
public class LocalSdaiQuery extends SdaiQueryEngine implements SdaiQuery {
  ...
  protected static final Object allTypesToken = new Object() {
  ...
  private static final SCreator schemaConstrCreators[] = new SCreator[] {
      /* and */new SCreator() {
    public Constraint create(ESchema_definition schema) {
      return new And();
    }
  },
  ...
  private static final String limitedConstraints[] = new String[] { "and", "intersect", "items", "not", "or", "union", };
  ...
  private static final SdaiQueryEngine.Creator valConstraintCreators[] = new SdaiQueryEngine.Creator[] {
      /* and */new SdaiQueryEngine.Creator() {
    public Constraint create() {
      return new SchemaValAnd();
    }
  },
      /* eq */new SdaiQueryEngine.Creator() {
    public Constraint create() {
      return new SchemaEq();
    }
  },
      /* neq */new SdaiQueryEngine.Creator() {
    public Constraint create() {
      return new SchemaNeq();
    }
  },
      /* nregex */new SdaiQueryEngine.Creator() {
    public Constraint create() {
      return new SchemaNregex();
    }
  },
      /* or */new SdaiQueryEngine.Creator() {
    public Constraint create() {
      return new SchemaValOr();
    }
  },
      /* regex */new SdaiQueryEngine.Creator() {
    public Constraint create() {
...
// 	}
  ...
  protected Object createNamespaceHandler(String namespaceURI, Element queryElem) throws SdaiException {
    if (namespaceURI.equals(QUERY_V1_URL)) {
      ...
      if (namespaceURI.startsWith(SCHEMA_URI, JSDAI_URI_LEN)) {
        ...
        ESchema_definition schema = findMappingDomAndSrcSchema(namespaceURI.substring(MAPPING_URI_LEN), mappingDomain);
    ...
    throw new SdaiException(SdaiException.FN_NAVL, "Unsupported namespace URI in query element: " + namespaceURI);
  }

  static protected String displayName(Object type) throws SdaiException {
    return (type instanceof EEntity_definition) ? ((EEntity_definition) type).getName(null) : type.toString();
  }

  static protected EAttribute getAttribute(ESchema_definition schema, String entityName, String attributeName, Node containerNode, LocalContext localContext,
    ...
    if (entityName != null) {
      ...
      while (ctsIter.hasNext()) {
        ...
        while (currTypeIter.hasNext()) {
          ...
          if (currTypeObject == allTypesToken
              || ((currType = LocalContext.getEntityType(currTypeObject, meta)) != entity && !((CEntityDefinition) currType).isSubtypeOf(entity))) {
            throw new SdaiException(SdaiException.FN_NAVL, formatMessage(containerNode, "Incompatible types",
                displayName(currType != null ? currType : currTypeObject) + " and " + entity.getName(null)));
          }
        }
      }
      ...
      while (ctsIter.hasNext()) {
        ...
        while (currTypeIter.hasNext()) {
          Object currTypeObject = currTypeIter.next();
          if (currTypeObject == allTypesToken) {
            throw new SdaiException(SdaiException.FN_NAVL, formatMessage(containerNode, "No instances selected", null));
          }
          EAttribute currAttr = JsdaiLangAccessorLocal.findAttribute(LocalContext.getEntityType(currTypeObject, meta), attributeName);
          if (attribute != null && currAttr != attribute) {
            throw new SdaiException(SdaiException.FN_NAVL, formatMessage(containerNode, "Ambiguous attributes", attribute.toString() + " and "
                + currAttr.toString()));
          }
          attribute = currAttr;
        }
  ...
  private ESchema_definition findSchemaDefinition(String schemaName) throws SdaiException {
    SdaiModel schemaModel = session.getSystemRepository().findSdaiModel(schemaName.toUpperCase() + SdaiSession.DICTIONARY_NAME_SUFIX);
    if (schemaModel == null) {
      throw new SdaiException(SdaiException.FN_NAVL, "Unknown schema name in query element: " + schemaName);
    }
  ...
  protected QueryLibProvider getQueryLibProvider(int scope) throws SdaiException {
    ...
    switch (scope) {
      ...
      default:
        throw new SdaiException(SdaiException.SY_ERR, "Unrecognized query lib provider scope: " + scope);
    }
  ...
  private QueryLib findQueryLib(String queryLibId) throws SdaiException {
    ...
    throw new SdaiException(SdaiException.FN_NAVL, "Query library was not found: " + queryLibId);
  }

  private ESchema_definition findMappingDomAndSrcSchema(String mappingName, ASdaiModel mappingDomain) throws SdaiException {
    SdaiModel mappingModel = session.getSystemRepository().findSdaiModel(mappingName.toUpperCase() + SdaiSession.MAPPING_NAME_SUFIX);
    if (mappingModel == null) {
      throw new SdaiException(SdaiException.FN_NAVL, "Unknown mapping name in query element: " + mappingName);
    }
  ...
  public void setParameter(String name, Object value) throws SdaiException {
    ...
    if (allowedParams == null || !(allowedParams.isEmpty() || allowedParams.contains(name))) {
    ...
    protected void execute(Context context) throws SdaiException {
      ...
      while (valueListIter.hasNext()) {
        ...
        if (value instanceof Object[]) {
          Object[] values = (Object[]) value;
          for (int i = 0; i < values.length; i++) {
            if (values[i] instanceof CharSequence && realExpPattern.matcher((CharSequence) values[i]).find(0)) {
              matches = true;
              break;
            }
          }
          ...
          matches = value instanceof CharSequence && realExpPattern.matcher((CharSequence) value).find(0);
        }
        if (!matches) {
          valueListIter.set(null);
        }
