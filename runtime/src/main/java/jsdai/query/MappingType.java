...
class MappingType extends Type {
  ...
  protected void setParameters(String entityName, Node containerNode, Context context) throws SdaiException {
    ...
    while (ctsIter.hasNext()) {
      ...
      while (currTypeIter.hasNext()) {
        ...
        EEntity_definition currType = currTypeObject == LocalSdaiQuery.allTypesToken ? null : LocalContext.getSchemaEntityType(currTypeObject);
        ...
        while (entMappingIter.next()) {
          ...
          if (currType == null || currType == targetType
              || (targetType instanceof EEntity_definition && ((CEntityDefinition) targetType).isSubtypeOf(currType))) {
  ...
  public void execute(Context context) throws SdaiException {
    ...
    while (cisIter.hasNext()) {
      ...
      while (currInstIter.hasNext()) {
        ...
        AEntity_mapping instMappings = currInst.testMappedEntity(entity, localContext.getLocalQuery().domain, mappingDomain, EEntity.NO_RESTRICTIONS);
        if (instMappings == null) {
          currInstIter.remove();
        }
        else if (exact) {
          boolean matches = false;
          AEntity_mapping specificMappings = currInst.findMostSpecificMappings(localContext.getLocalQuery().domain, mappingDomain, instMappings,
