...
class QueryLib {
  ...
  QueryEnt getQueryEnt(String name) throws SdaiException {
    ...
    if (queryEnt == null) {
      throw new SdaiException(SdaiException.FN_NAVL, "Query-ent with the name " + name + "not found");
    }
    return queryEnt;
  }
  ...
  QueryEnt getQueryEnt(String name, Node containerNode) throws SdaiException {
    QueryEnt queryEnt = (QueryEnt) queryEntMap.get(name);
    if (queryEnt == null) {
      throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(containerNode, "Query-ent with the name " + name + "not found", null));
    }
    return queryEnt;
  }
