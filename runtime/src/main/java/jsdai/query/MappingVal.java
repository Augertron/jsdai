...
class MappingVal extends Val {
  ...
  protected void setParameters(String entityName, String attributeName, String selectNames, Node containerNode, Context context) throws SdaiException {
    LocalContext localContext = (LocalContext) context;

    attribute = LocalSdaiQuery.getAttribute(schema, entityName, attributeName, containerNode, localContext, true);

    // Getting select path
    if (selectNames != null) {
      throw new SdaiException(SdaiException.AT_NVLD, SdaiQueryEngine.formatMessage(containerNode,
          "Attribute select path is not supported in mapping val constraint", selectNames));
    }
    ...
    while (ctsIter.hasNext()) {
      ...
      while (genAttrMappingIter.next()) {
        EGeneric_attribute_mapping genAttrMapping = genAttrMappings.getCurrentMember(genAttrMappingIter);
      ...
      if (!foundValues) {
        throw new SdaiException(SdaiException.AT_NVLD, SdaiQueryEngine.formatMessage(containerNode, "Attribute is a reference to the instance",
            attribute.toString()));
      }
    }
  }
  ...
  public void execute(Context context) throws SdaiException {
    ...
    while (cisIter.hasNext()) {
      ...
      while (currInstIter.hasNext()) {
        ...
        Object[] instVals = currInst.getMappedAttribute(attribute, localContext.getLocalQuery().domain, mappingDomain, EEntity.NO_RESTRICTIONS);
    ...
    if ((containsValConstraints() || context.contextConstraint.containsValConstraints() || executeValueResult) && !localContext.values.isEmpty()) {
