  ...
  protected void setParameters(String entityName, Node containerNode, Context context) throws SdaiException {
    ...
    while (ctsIter.hasNext()) {
      ...
      while (currTypeIter.hasNext()) {
        ...
        if (currTypeObject == LocalSdaiQuery.allTypesToken || (currType = LocalContext.getSchemaEntityType(currTypeObject)) == entity
  ...
  protected void execute(Context context) throws SdaiException {
    ...
    while (cisIter.hasNext()) {
      ...
      while (currInstIter.hasNext()) {
        ...
        if (!(!exact && currInst.isKindOf(entity)) && !(exact && currInst.isInstanceOf(entity))) {
          currInstIter.remove();
        }
      }
    }
