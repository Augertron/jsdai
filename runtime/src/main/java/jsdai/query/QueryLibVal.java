...
class QueryLibVal extends Val {
  ...
  protected void setParameters(String entityName, String attributeName, String selectNames, Node containerNode, Context context) throws SdaiException {
    if (entityName == null) {
      throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(containerNode, "Attribute ent is required for query lib fwd", null));
    }
    ...
    if (!context.isNarrowedBy(queryVal.getInValContext())) {
      throw new SdaiException(SdaiException.FN_NAVL, SdaiQueryEngine.formatMessage(containerNode, "Query-fwd is not compatible with the current context",
          null));
    }
  ...
  public boolean isValueResult() {
    return contextConstraint == this ? valueResult : valueResult && contextConstraint.isValueResult();
  }

  public boolean containsValConstraints() {
    return contextConstraint == this ? constraints != null : constraints != null || contextConstraint.containsValConstraints();
  }
