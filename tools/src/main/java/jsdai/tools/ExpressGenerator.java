...
import jsdai.SExtended_dictionary_schema.AExpress_code;
...
import jsdai.SExtended_dictionary_schema.ASchema_definition;
...
import jsdai.SExtended_dictionary_schema.CConstant_declaration;
import jsdai.SExtended_dictionary_schema.CDeclaration;
import jsdai.SExtended_dictionary_schema.CEntity_declaration;
import jsdai.SExtended_dictionary_schema.CFunction_declaration;
import jsdai.SExtended_dictionary_schema.CInterface_specification;
import jsdai.SExtended_dictionary_schema.CProcedure_declaration;
import jsdai.SExtended_dictionary_schema.CRule_declaration;
import jsdai.SExtended_dictionary_schema.CSub_supertype_constraint;
import jsdai.SExtended_dictionary_schema.CSubtype_constraint_declaration;
import jsdai.SExtended_dictionary_schema.CType_declaration;
...
//	static Vector exclude_models = new Vector();
  ...
  public static final void main(String args[]) throws SdaiException, java.io.IOException {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if ((args[i].equalsIgnoreCase("-long_form")) || (args[i].equalsIgnoreCase("-long"))) {
        flag_long_form = true;
      }
      ...
      if (args[i].equalsIgnoreCase("-end_comment")) {
        flag_end_comment = true;
      }
      if (args[i].equalsIgnoreCase("-format_expressions")) {
        flag_format_expressions = true;
      }
      if (args[i].equalsIgnoreCase("-format")) {
        flag_format_expressions = true;
      }
      if (args[i].equalsIgnoreCase("-dont_allign_local")) {
        flag_do_not_allign_LOCAL = true;
      }
      if (args[i].equalsIgnoreCase("-include_arm")) {
        flag_include_arm = true;
      }
      if (args[i].equalsIgnoreCase("-exclude_arm")) {
        flag_exclude_arm = true;
      }
      if (args[i].equalsIgnoreCase("-skip_conflicts")) {
        flag_skip_conflicts = true;
      }
...
//		repo.exportClearTextEncoding("_KUKU3Jurga.pf");
    ...
    while (iter_assoc.next()) {
      SdaiModel mod = domain.getCurrentMember(iter_assoc);
    ...
    for (int i = 0; i < constant_count; i++) {
      EConstant_definition constant_definition = constant_definitions[i];
    ...
    while (iter_sub.next()) {
      ESubtype_constraint_declaration subtype_constraint_declaration = (ESubtype_constraint_declaration) subtype_constraint_declarations
          .getCurrentMemberObject(iter_sub);
    ...
    for (int i = 0; i < subtype_constraint_count; i++) {
      ESub_supertype_constraint subtype_constraint = subtype_constraints[i];
    ...
    for (int i = 0; i < rule_count; i++) {
      EGlobal_rule global_rule = global_rules[i];
    ...
    for (int i = 0; i < function_count; i++) {
      EFunction_definition function_definition = function_definitions[i];
    ...
    for (int i = 0; i < procedure_count; i++) {
      EProcedure_definition procedure_definition = procedure_definitions[i];
  ...
  void run(SdaiRepository repo) throws SdaiException, java.io.IOException {
    ...
    while (iter.next()) {
      ...
      else if (name.endsWith("_DICTIONARY_DATA")) {
        if (flag_include_arm && (!name.endsWith("_ARM_DICTIONARY_DATA"))) {
          continue;
        }
        if (flag_exclude_arm && name.endsWith("_ARM_DICTIONARY_DATA")) {
          continue;
        }
        if (model.getMode() == SdaiModel.NO_ACCESS) {
          model.startReadOnlyAccess();
...
// 	    System.out.println("Schema start D: " + schemaName);
     ...
     * partIndexTmp += printHRefandTarget(getComplexName(entity.getName(null)),
     * getUpper(getComplexName(entity.getName(null)))+".html", "classFrame");
      ...
      while (aconstraintsIter.hasNext()) {
        ...
        if (flag_long_form) {
          ...
          if (!(hm_all_names.containsKey(name.toLowerCase()))) {
            ...
            String other_schema = (String) hm_all_names.get(name.toLowerCase());
            System.out.println("duplicate name " + name + " in schema " + schema.getName(null) + " (subtype_constraint), and in schema: "
                + other_schema);
            if (flag_skip_conflicts) {
              continue;
            }
...
//        result += "\t" + printBold(constant.getName(null).toLowerCase());
    ...
    while (iter_code.next()) {
      ...
      if (target == constant) {
        ...
        while (iter_ecv.next()) {
          String expr = expressions.getCurrentMember(iter_ecv);
          if (flag_long_form) {
...
//        result += println("END_CONSTANT; -- " + constant.getName(null));
  ...
  private void printDefinedTypeLF(PrintWriter pw, EDefined_type type, SdaiRepository repo, EEntity_definition[] entity_definitions, int entity_count,
      EDefined_type[] defined_types, int type_count) throws SdaiException {
  ...
  private void printDefinedTypeLF(PrintWriter pw, EDefined_type type, boolean haveAggregate, EEntity_definition[] entity_definitions, int entity_count,
      EDefined_type[] defined_types, int type_count) throws SdaiException {
    String schemaName = findSchemaForEntity(type).getName(null);
    String partExpress = "", partComments = "", partInterface = "", partAggregate = "", partBasedOnSelects = "", partUsers = "", partSubtypes = "", partRules = "";
...
//		pw.print(partRules);
  ...
  private void printEntity(PrintWriter pw, EEntity_definition entity, SdaiRepository repo, EEntity_definition[] entity_definitions, int entity_count)
  ...
  private void printEntityLF(PrintWriter pw, EEntity_definition entity, ESchema_definition schema, EEntity_definition[] entity_definitions, int entity_count)
      throws SdaiException {
...
//express fro attribute
        String temp_inverse = ((testRedeclaring(attribute)) ? "SELF\\" + getRedeclaring(attribute).getParent(null).getName(null) + "." : "")
//					+attribute.getName(null).toLowerCase()+": ";
            + attribute.getName(null) + ": ";
        if (attribute.testMin_cardinality(null)) {
          temp_inverse += ((attribute.getDuplicates(null)) ? "BAG [" : "SET [") + printBound(attribute.getMin_cardinality(null)) + ":"
...
//	        	System.out.println("in-entity subtype_constraint: " + subtypes.getCurrentMember(it_subtypes));
            String partSuperX = printSubtypeConstraintLF(subtypes.getCurrentMember(it_subtypes).getConstraint(null), entity_definitions,
                entity_count, definition);
...
//	private String printType(EEntity type) throws SdaiException {
  private String printTypeLF(EEntity type, EEntity_definition[] entity_definitions, int entity_count, EDefined_type[] defined_types, int type_count)
      throws SdaiException {
    String partType = "";
...
//			partType += println("SELECT (");
      ...
      else if (((EData_type) type).getName(null).equals("_GENERIC")) {
...
//							break;
  ...
  private void printSubtypeConstraintLF(PrintWriter pw, ESub_supertype_constraint constraint, ESchema_definition schema,
      EEntity_definition[] entity_definitions, int entity_count) throws SdaiException {
    String partHeader = "", partExpress = "", partClass = "", partComments = "";
...
//		pw.print(partComments);
  ...
  private String printExpressForSubtypeConstraintLF(ESub_supertype_constraint constraint, EEntity_definition[] entity_definitions, int entity_count)
      throws SdaiException {
    String result = "";
    ...
    if (!entityPresent(ed, entity_definitions, entity_count, null)) {
      System.out.println("WARNING: stand-alone subtype_constraint " + constraint.getName(null)
          + " not generated, because its entity is not in this long form schema: " + ed.getName(null));
      return "";
    }
...
//    result += printBreak();
  ...
  private void printGlobalRuleLF(PrintWriter pw, EGlobal_rule rule, SdaiRepository repo, EEntity_definition[] entity_definitions, int entity_count)
      throws SdaiException {
    asmExpressDomain.clear();
  ...
  private void printGlobalRuleLF(PrintWriter pw, EGlobal_rule rule, ESchema_definition schema, EEntity_definition[] entity_definitions, int entity_count)
      throws SdaiException {
    String partHeader = "", partExpress = "", partClass = "", partComments = "";
...
// some formatting, but better formating would be nice, perhaps to implement parsing of the string
           ...
           * sEC = replaceAll( sEC, ad.getName(null)+" (", printHRef(ad.getName(null).toLowerCase(),
           * getSchemaNameIfDiffer(ad, schema)+getUpper(ad.getName(null))+".html")+" (");
           * }
           * algorithm_body += sEC;
           */
          result += println(sEC);
...
//System.out.println("MULTI - current label: " + a_label);
       ...
       * if a_label is "" then it might be either AGGREGATE, or ARRAY, BAG, LISI, SET, but all of them have OF and the label
       * is not present
       */
...
//						System.out.println("\"GENERIC\".length(): " + "GENERIC".length());
          ...
          if (((result.indexOf("GENERIC_ENTITY", current_index) == result.length() - "GENERIC_ENTITY".length()) || (result
              .lastIndexOf("GENERIC_ENTITY") == result.indexOf("GENERIC_ENTITY", current_index)))
              || ((result.indexOf("GENERIC", current_index) == result.length() - "GENERIC".length()) && (result.lastIndexOf("GENERIC") == result
              .indexOf("GENERIC", current_index)))) {
            ...
            if (!iter_labels.next()) {
              ...
              System.out.println("ExpressGenerator INTERNAL ERROR: the label does not match an AGGREGATE even though it is not the last one: "
                  + a_label);
            }
          }
          else {
            ...
            System.out
                .println("ExpressGenerator INTERNAL ERROR: wrong inner-most member of a nested aggregate, must be GENERIC or GENERIC ENTITY: "
                    + result);
          }
    ...
    if (index1 >= 0) {
      ...
      if (index2 >= 0) {
        ...
        if (index3 >= 0) {
          ...
          System.out.println("ExpressGenerator INTERNAL ERROR - more than two aggregates with two labels: " + a_parameter + ", labels: " + aggr_label
              + ", " + element_label);
        }
        else {
          ...
          if (label_1_present && label_2_present) {
            ...
            if (index >= 0) { // it must be
              ...
              if (index5 >= 0) {
                ...
                // INTERNAL ERROR
                System.out.println("ExpressGenerator INTERNAL ERROR - inserting 2 labels 03: " + a_parameter + ", " + aggr_label + ", "
                    + element_label);
              }
            }
            else {
              // INTERNAL ERROR
              System.out.println("ExpressGenerator INTERNAL ERROR - inserting 2 labels 04: " + a_parameter + ", " + aggr_label + ", "
                  + element_label);
            }
          }
          else if (!label_1_present && label_2_present) {
            // cases 4),4b),4c)
            // 4) AGGREGATE OF AGGREGATE:label2 OF GENERIC
            // 4b) SET OF AGGREGATE:label2 OF GENERIC
            // 4c) SET[0:?] OF AGGREGATE:label2 OF GENERIC
        ...
        if (label_1_present && label_2_present) {
          ...
          if ((substring_1 != null) && (substring_2 != null)) {
            ...
            // INTERNAL ERROR
            System.out
                .println("ExpressGenerator INTERNAL ERROR - inserting 2 labels 01: " + a_parameter + ", " + aggr_label + ", " + element_label);
          }
        }
        else if (!label_1_present && label_2_present) {
          // cases 3),3b),3c)
          ...
          if ((substring_1 != null) && (substring_2 != null)) {
            ...
            // INTERNAL ERROR
            System.out
                .println("ExpressGenerator INTERNAL ERROR - inserting 2 labels 02: " + a_parameter + ", " + aggr_label + ", " + element_label);
          }
        }
      ...
      System.out.println("ExpressGenerator INTERNAL ERROR - no aggregates with two labels: " + a_parameter + ", labels: " + aggr_label + ", "
          + element_label);
    }
...
//                        parameter_list += " : "+printType(p.getParameter_type(null));
            ...
            if (p.testType_labels(null)) {
              ...
              else if (number_of_labels == 1) { // should be a non-aggregate type
...
//                              	a_parameter += ": " + stl;
              ...
              else if (number_of_labels == 2) { // a simple aggregate - correction, also a nested aggregate with the inner element without label
...
//                            	}
              ...
              else if (number_of_labels > 2) { // a nested aggregate
...
//System.out.println("function: " + function.getName(null) + ", labels: " + atl);
          ...
          else if (number_of_labels == 2) { // a simple aggregate
            String st_aggregate_label = atl.getByIndex(1);
            String st_element_label = atl.getByIndex(2);
//                    if ((st_aggregate_label != null) && (!st_aggregate_label.equals(""))){
            // have to insert into a_parameter
...
//                        String sEC = format(asECValues.getCurrentMember(iter_ecv));
  ...
  private String printSubtypeConstraintLF(ESubtype_expression expression, EEntity_definition[] entity_definitions, int entity_count,
      EEntity_definition current) throws SdaiException {
    String result = "";
    if (expression.getGeneric_operands(null).getMemberCount() > 1) {
      if (expression instanceof EOneof_subtype_expression) {
        ...
        else if (result3.indexOf("ANDOR") < 0) {
  ...
  private String printOperandsLF(ESubtype_expression expression, String separator, EEntity_definition[] entity_definitions, int entity_count,
      EEntity_definition current) throws SdaiException {
    String result = "";
  ...
  boolean namedTypePresent(ENamed_type entity, EEntity_definition[] entities, int entity_count, EDefined_type[] types, int type_count, ENamed_type current)
      throws SdaiException {
    // at is an interesting situation - pruning is not needed, but such a case should not happen,
    // and if happens - pruning could correct it.
    // but perhaps just print error message and not prune
...
//			System.out.println("ERROR - an entity has itself as its subtype: " + entity.getName(null));
   ...
   *
   * entity1 ANDOR entity2 ANDOR entity3 - a simpler case - nothing or entity1 is done by itself, still we would like to have a
   * count returned somehow
   *
...
//System.out.println("IN TOP - model found, starting recursion: " + model);
  ...
  static private void printIfAsked(String str) {
    if (flag_print_schemas) {
      ...
      flag_printed_schema++;
      String nothing = "";
      if (flag_printed_schema > 9999) {
        nothing = "";
      }
      else if (flag_printed_schema > 999) {
        nothing = " ";
      }
      else if (flag_printed_schema > 99) {
        nothing = "  ";
      }
      else if (flag_printed_schema > 9) {
        nothing = "   ";
      }
      else {
        nothing = "    ";
