...
import jsdai.SMapping_schema.EConstraint;
  ...
  public static final void main(String args[]) throws SdaiException, IOException {
    ...
    for (int ihi = 0; ihi < args.length; ihi++) {
      ...
      if (args[ihi].equalsIgnoreCase("-verbose")) {
        flag_print_attributes = true;
      }
      if (args[ihi].equalsIgnoreCase("-debug")) {
        flag_print_debug = true;
      }
      if (args[ihi].equalsIgnoreCase("-comments")) {
        flag_print_comments = true;
      }
      if (args[ihi].equalsIgnoreCase("-swap")) {
        flag_swap_supertype_constraints = true;
      }
      if (args[ihi].equalsIgnoreCase("-no_errors")) {
        flag_print_errors = false;
      }
      if (args[ihi].equalsIgnoreCase("-cardinality")) {
        flag_display_cardinality = true;
      }
      if (args[ihi].equalsIgnoreCase("-result")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-AIM")) {
        flag_many_on_demand_active = false;
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-map")) {
        flag_many_on_demand_active = false;
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-entity")) {
        flag_many_on_demand_active = false;
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-root_entity")) {
        flag_many_on_demand_active = false;
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-entities")) {
        flag_many_on_demand_active = false;
        ihi++;
        int entity_count = 0;
...
//		pw.println("END_SCHEMA; -- x");
  ...
  public void printSchemas(ASdaiModel models) throws SdaiException {
    ...
    while (iter.next()) {
      ...
      else if (model.getName().equalsIgnoreCase(AIM_name)) {
        aim_model = model;
        if (aim_model.getMode() == SdaiModel.NO_ACCESS) {
          aim_model.startReadOnlyAccess();
        }
      }
      else if (model.getName().equalsIgnoreCase(MAP_name)) {
        map_model = model;
...
// System.out.println("generate constraint function for constraint_attribute: " + constraint);
      ...
      else if (constraint instanceof EConstraint_relationship) { // ABSTRACT
...
// System.out.println("ENDING, more from inside: " + next_constraint);
  ...
  void generateFunctionsInSequence(SdaiModel arm_model, SdaiModel aim_model, SdaiModel map_model) throws SdaiException {
    ...
    while (iter.next()) {
      ...
      if (mark_used_in_path_constraint_as_element1 != null) {
        continue;
      }
...
//				pw.println("END_FUNCTION;");
  ...
  Vector handleConstraint_attribute(EConstraint constraint) throws SdaiException {
    ...
    else if (constraint instanceof EAggregate_size_constraint) {
...
// System.out.println("2 generate entity_constraint function: " + constraint);
  ...
  Vector handleAggregate_member_constraint(EConstraint constraint) throws SdaiException {
    ...
    else if (attribute instanceof EEntity_constraint) {
      if (member == Integer.MIN_VALUE) {
        // does not occur in the current ap210
...
//					}
          ...
          while (itdt.next()) {
            EDefined_type dt = data_type.getCurrentMember(itdt);
...
//							pw.println("\tIF (\'" + AIM_schema_name + "." + dt.getName(null).toUpperCase() + "\' IN TYPEOF (e." + attribute2_name + "))");
              pw.println("\tIF (\'" + AIM_schema_name + "." + dt.getName(null).toUpperCase() + "\' IN TYPEOF (e\\" + parent_entity2_name + "."
                  + attribute2_name + "))");
            }
            else {
//							pw.println("\tAND (\'" + AIM_schema_name + "." + dt.getName(null).toUpperCase() + "\' IN TYPEOF (e." + attribute2_name + "))");
              pw.println("\tAND (\'" + AIM_schema_name + "." + dt.getName(null).toUpperCase() + "\' IN TYPEOF (e\\" + parent_entity2_name + "."
                  + attribute2_name + "))");
            }
          }
          if (!first_time) {
...
//			printDebug("\t -- attribute - select_constraint: " + attribute);
    ...
    else if (constraint instanceof ENon_optional_constraint) {
      // does not occur in the current ap210
...
// printDebug("-- attribute (aggregate_member_constraint) domain (aggregate): " + xa_domain );
            ...
            else if (xa_element_domain instanceof EDefined_type) {
          ...
          else if (xa_domain instanceof EEntity_definition) {
...
//				pw.println("\t\taggr : BAG OF " + bag_member_type + " := USEDIN(e, \'" + AIM_schema_name + "." + inverted_attribute_parent_entity_name.toUpperCase() + "." + inverted_attribute_name.toUpperCase() + "\');");
        ...
        if (inverted_attribute instanceof EAttribute) {
          ...
          String inverted_attribute_parent_entity_model_name = ((EAttribute) inverted_attribute).getParent(null).findEntityInstanceSdaiModel()
              .getName();
          inverted_attribute_parent_entity_schema_name = inverted_attribute_parent_entity_model_name.substring(0,
              inverted_attribute_parent_entity_model_name.length() - 16).toLowerCase();
          ...
          pw.println("\taggr := USEDIN(e, \'" + AIM_schema_name + "." + inverted_attribute_parent_entity_name.toUpperCase() + "."
              + inverted_attribute_name.toUpperCase() + "\');");
        ...
        else if (inverted_attribute instanceof EEntity_constraint) {
          // does not occur in the current ap210
          printDebug("\n-- implementing entity_constraint, attribute = inverse_attribute_constraint, inverted: entity_constraint");
        }
        else if (inverted_attribute instanceof EAggregate_member_constraint) {
          // must be implemented !!!
...
//--22222222222222222222222222222 end
            ...
            printDebug("-- entity_constraint, attribute = inverse_attribute, inverted_attribute = aggregate_member_constraint, its attribute not explicit: "
                + attribute3);
          }
...
//4444444444444444444444444444444444444444444444444444 end
        ...
        else if (inverted_attribute instanceof ESelect_constraint) {
          // does not occur in the current ap210
  ...
  Vector handleSelect_constraint(EConstraint constraint) throws SdaiException {
    ...
    if (select_constraint.testData_type(null)) {
      ...
      while (itdt.next()) {
        ...
        if (first_time) {
          first_time = false;
          pw.println("\tIF (\'" + AIM_schema_name + "." + dt.getName(null).toUpperCase() + "\' IN TYPEOF (e\\" + parent_entity_name + "."
              + attribute_name + "))");
        }
        else {
          pw.println("\tAND (\'" + AIM_schema_name + "." + dt.getName(null).toUpperCase() + "\' IN TYPEOF (e\\" + parent_entity_name + "."
              + attribute_name + "))");
        }
      }
      if (!first_time) {
...
//		  pw.println("\t\tRETURN (" + function2_name + "(e));");
    ...
    else if (constraint instanceof EInstance_equal) {
...
//								pw.println("\t\tr := aggr[i]." + ((EAttribute)element2).getName(null) + ";");
                pw.println("\t\tr := aggr[i]\\" + ((EAttribute) element2).getParent(null).getName(null).toLowerCase() + "."
                    + ((EAttribute) element2).getName(null) + ";");
              }
              else {
            ...
            if (!(attribute_name.equals("_UNKNOWN__attribute_name"))) {
              ...
              if (element2 instanceof EAttribute) {
                pw.println("\t\tRETURN (aggr[" + member + "]\\" + ((EAttribute) element2).getParent(null).getName(null).toLowerCase() + "."
                    + ((EAttribute) element2).getName(null) + ");");
              }
              else {
...
//			  			attribute_name = attribute2_name;
              if (attribute2 instanceof EExplicit_attribute) {
                ...
                else if (attribute2_domain instanceof ENamed_type) {
...
//											pw.println("\t\t\tr := aggr[i]." + ((EAttribute)element2).getName(null) + ";");
                    ...
                    if (element2 instanceof EAttribute) {
                      pw.println("\t\tRETURN (aggr[" + member + "]\\"
                          + ((EAttribute) element2).getParent(null).getName(null).toLowerCase() + "."
                          + ((EAttribute) element2).getName(null) + ");");
                    }
                    else {
                      pw.println("\t\tRETURN (" + element2_function_name + "(aggr[" + member + "]));");
...
//							printDebug("\t-- inverted_attribute = aggregate_member_constraint, its attribute domain: " + attribute_domain);
            ...
            if (attribute instanceof EInverse_attribute_constraint) {
              printDebug(
                  "\t-- To Be Done: path_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute = inverse_attribute_constraint");
            }
            else if (attribute instanceof EAggregate_member_constraint) {
              printDebug(
                  "\t-- To Be Done: path_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute = aggregate_member_constraint");
            }
            else if (attribute instanceof EEntity_constraint) {
              printDebug(
                  "\t-- To Be Done: path_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute = entity_constraint");
            }
            else if (attribute instanceof ESelect_constraint) {
              printDebug(
                  "\t-- To Be Done: path_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute =  select_constraint");
            }
            else {
              // must be one of the above
              pw.println(
                  "\t-- ERROR: incorrect type - path_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute: "
                      + attribute);
            }
        }
        else {
...
//			pw.println("\t\taggr : BAG OF " + bag_member_type + " := USEDIN(e, \'" + inverted_attribute_parent_entity_schema_name.toUpperCase() + "." + inverted_attribute_parent_entity_name.toUpperCase() + "." + inverted_attribute_name.toUpperCase() + "\');");
      pw.println("\t\taggr : BAG OF " + bag_member_type + " := USEDIN(e, \'" + AIM_schema_name + "."
          + inverted_attribute_parent_entity_name.toUpperCase() + "." + inverted_attribute_name.toUpperCase() + "\');");
      pw.println("\tEND_LOCAL;");
      pw.println("\tREPEAT i:=1 TO SIZEOF(aggr);");
...
//					ee_name = "e." + ((EAttribute)attribute).getName(null).toLowerCase();
          ee_name = "e\\" + ((EAttribute) attribute).getParent(null).getName(null).toLowerCase() + "."
              + ((EAttribute) attribute).getName(null).toLowerCase();
        }
        else if (attribute instanceof EAggregate_member_constraint) {
...
//						ee_name = "e." + ((EAttribute)attribute2).getName(null).toLowerCase() + "[1]";
            ee_name = "e\\" + ((EAttribute) attribute2).getParent(null).getName(null).toLowerCase() + "."
                + ((EAttribute) attribute2).getName(null).toLowerCase() + "[1]";
            EEntity attr_domain = null;
            if (attribute2 instanceof EExplicit_attribute) {
              attr_domain = ((EExplicit_attribute) attribute2).getDomain(null);
            }
            else if (attribute2 instanceof EDerived_attribute) {
...
// ###################################
          ...
          if (inverted_attribute instanceof EAttribute) {
            ...
            pw.println(
                "-- ERROR: path_constraint + entity_constraint + inverse_attribute_constraint - inverted_attribute - entity_constraint - not yet implemented");
          }
          else if (inverted_attribute instanceof EAggregate_member_constraint) {
//				pw.println("-- ERROR: path_constraint + inverse_attribute_constraint - inverted_attribute = aggregate_member_constraint - not yet implemented");
...
//							printDebug("\t-- inverted_attribute = aggregate_member_constraint, its attribute domain: " + attribute_domain);
                ...
                if (attribute instanceof EInverse_attribute_constraint) {
                  printDebug(
                      "\t-- To Be Done: path_constraint+entity_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute = inverse_attribute_constraint");
                }
                else if (attribute instanceof EAggregate_member_constraint) {
                  printDebug(
                      "\t-- To Be Done: path_constraint+entity_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute = aggregate_member_constraint");
                }
                else if (attribute instanceof EEntity_constraint) {
                  printDebug(
                      "\t-- To Be Done: path_constraint+entity_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute = entity_constraint");
                }
                else if (attribute instanceof ESelect_constraint) {
                  printDebug(
                      "\t-- To Be Done: path_constraint+entity_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute =  select_constraint");
                }
                else {
                  // must be one of the above
                  pw.println(
                      "\t-- ERROR: incorrect type - path_constraint+entity_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint attribute: "
                          + attribute);
                }
            }
            else {
              // error, non-optional
              pw.println(
                  "\t-- ERROR: attribute unset !!! - path_constraint+entity_constraint+inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint");
            }
...
//					pw.println("\tr := e." +  ((EAttribute)subpath_constraint).getName(null) + ";");
          pw.println("\tr := e\\" + ((EAttribute) subpath_constraint).getParent(null).getName(null).toLowerCase() + "."
              + ((EAttribute) subpath_constraint).getName(null) + ";");
        }
        else {
...
//				pw.println("\t\tr := intersection[i]." + ((EAttribute)element2).getName(null) + ";");
        pw.println("\t\tr := intersection[i]\\" + ((EAttribute) element2).getParent(null).getName(null).toLowerCase() + "."
            + ((EAttribute) element2).getName(null) + ";");
      }
      else {
...
//			pw.println("\tIF EXISTS(e." + ((EAttribute)constraints).getName(null).toLowerCase() + ") THEN");
      pw.println("\tIF EXISTS(e\\" + ((EAttribute) constraints).getParent(null).getName(null).toLowerCase() + "."
          + ((EAttribute) constraints).getName(null).toLowerCase() + ") THEN");
...
//		return null;
  ...
  Vector handleInverse_attribute_constraint(EConstraint constraint) throws SdaiException {
    ...
    if (inverted_attribute instanceof EAttribute) {
      ...
      pw.println("\tLOCAL");
      pw.println("\t\tresult : BAG OF GENERIC := USEDIN(e, \'" + AIM_schema_name + "." + inverted_attribute_parent_entity_name.toUpperCase() + "."
          + inverted_attribute_name.toUpperCase() + "\');");
      pw.println("\tEND_LOCAL;");
      pw.println("\tIF EXISTS(result) THEN");
      pw.println("\t\tIF SIZEOF(result) > 0 THEN");
  ...
  Vector handleNegation_constraint(EConstraint constraint) throws SdaiException {
    ...
    if (constraints instanceof EConstraint_attribute) {
      ...
      if (constraints instanceof EAttribute_value_constraint) {
        ...
        else if (constraints instanceof EInteger_constraint) {
          // does not occur in the current ap210
          printDebug("\n-- implementing negation_constraint, constraints: integer_constraint");
        }
        else if (constraints instanceof ENon_optional_constraint) {
          // does not occur in the current ap210
        ...
        else if (constraints instanceof EEnumeration_constraint) {
          // does not occur in the current ap210
    ...
    else if (constraints instanceof EConstraint_relationship) {
      // must be implemented !!!
      ...
      else if (constraints instanceof EInstance_constraint) {
        // does not occur in the current ap210
    ...
    else if (constraints instanceof EAttribute) {
      // does not occur in the current ap210
...
// set -> bag          type_name += ("OF SET");
        ...
        if (element_type_set) {
          ...
          System.out.println("RG: constructing type names, nested aggregate element type value unset:" + an_ss);
        }
      }
    ...
    if (rt == null) {
      type_name += "_NULL";
      System.out.println("RG: constructing type names: base type of aggregation type is null: " + rt1);
    }
...
//      type_name += constructNameWithAggregatePackage(aggr_prefix, rt);
        ...
        System.out.println("RG: constructing type names, not yet supported element type in aggregate: " + rt);
  ...
  String constructNameWithAggregatePackage(String aggr_prefix, EEntity ee) throws SdaiException {
    String a_package = "";
    String a_name = "";
...
//						printDebug("-- attribute of aggregate_member_constraint not explicit: " + attribute2 );
        ...
        else if (element1 instanceof EAggregate_member_constraint) {
          attribute = ((EAggregate_member_constraint) element1).getAttribute(null);
          if (attribute instanceof EAttribute) {
            parent_entity = (EEntity_definition) ((EAttribute) attribute).getParent(null);
            result = parent_entity.getName(null).toLowerCase();
...
//						printDebug("-- attribute - select_constraint - not yet supported in parameter");
    ...
    else if (constraint instanceof EInstance_constraint) {
      EInstance_constraint instance_constraint = (EInstance_constraint) constraint;
      EEntity element1 = null;
      if (instance_constraint.testElement1(null)) {
        element1 = instance_constraint.getElement1(null);
...
//		pw.println("END_SCHEMA; -- x");
  ...
  void printFunction2NoFunction(EEntity_definition arm_entity, EEntity_mapping the_entity_mapping, int i, int count, SdaiModel arm_model,
      SdaiModel aim_model, SdaiModel map_model) throws SdaiException {
...
//				a_line += "(\'" + rule_schema_name + ".\' + \'" + a_name + "\' IN TYPEOF(each))";
  ...
  void printBatchForRules(EEntity_definition arm_entity, EEntity aim_target, int i, int count) throws SdaiException {
    ...
    String to_print = "java  -D\"jsdai.properties=.\" jsdai.tools.RuleAndFunctionValidator -verbose -rule rule__" + arm_entity_name
        + " -p21 design5.stp  >> _LOG_Multiple_Launch 2>>_LOG_Multtiple_Lautch_EXEPTIONS";
...
//							pw.println("\nFUNCTION get__" + arm_entity_name + "__" + the_attribute_name + "_" + alternative_count + "(e : " + aim_entity_name + ") : BAG OF GENERIC;");
            pw.println("\nFUNCTION get__" + arm_entity_name + "__" + the_attribute_name + "_" + alternative_count + "(e : " + aim_entity_name
...
//							pw.println("\tIF 'BAG' IN TYPEOF(result) THEN");
          ...
          pw.println(
              "\tIF ('BAG' IN TYPEOF(result)) OR ('SET' IN TYPEOF(result)) OR ('LIST' IN TYPEOF(result)) OR ('ARRAY' IN TYPEOF(result)) OR ('AGGREGATE' IN TYPEOF(result)) THEN");
  ...
  boolean generateArmStrongAttributeChecks(RuleGeneratorArmAttribute arm_attribute, int alternative_nr, String arm_entity_name) throws SdaiException {
    ...
    if (arm_attribute.arm_attribute instanceof EExplicit_attribute) {
      ...
      if (arm_attribute.is_aggregate) {
        if (arm_attribute.min_cardinality == Integer.MIN_VALUE) {
        ...
        else if (arm_attribute.max_cardinality == Integer.MIN_VALUE) {
          if (!xa.getOptional_flag(null)) {
          ...
          if (!xa.getOptional_flag(null)) {
            ...
            pw.println(indentString(alternative_nr) + "IF (size < " + arm_attribute.min_cardinality + ") OR (SIZE > "
                + arm_attribute.max_cardinality + ") THEN");
          }
          else {
...
//				EAttribute xa = (EAttribute)arm_attribute.arm_attribute;
        if (arm_attribute.is_aggregate) {
          if (arm_attribute.min_cardinality == Integer.MIN_VALUE) {
            ...
            // both defined
            printDebug("\t\t-- aggregate [" + arm_attribute.min_cardinality + ":" + arm_attribute.max_cardinality + "]");
            pw.println("indentString(alternative_nr) + IF (size < " + arm_attribute.min_cardinality + ") OR (SIZE > " + arm_attribute.max_cardinality
                + ") THEN");
            if (go_deeper) {
              int_empty_if = generateArmStrongAttributeChecks(arm_attribute, alternative_nr, arm_entity_name);
            }
      ...
      if (arm_attribute.arm_attribute instanceof EExplicit_attribute) {
        ...
        if (arm_attribute.is_aggregate) {
          if (arm_attribute.min_cardinality == Integer.MIN_VALUE) {
            ...
            if (!xa.getOptional_flag(null)) {
              printDebug("\t\t-- aggregate [" + arm_attribute.min_cardinality + ":" + arm_attribute.max_cardinality + "]");
              pw.println(indentString(alternative_nr) + "IF (size >= " + arm_attribute.min_cardinality + ") AND (SIZE <= "
                  + arm_attribute.max_cardinality + ") THEN");
            }
            else {
              printDebug("\t\t-- OPTIONAL aggregate [" + arm_attribute.min_cardinality + ":" + arm_attribute.max_cardinality + "]");
...
//		String size_expression;
     ...
     * OPTIONAL [min : max], min != 0 - size must be min <= size <= max or 0 -> IF ((size < min) AND (size <> 0)) OR (size >
     * max) THEN RETURN (FALSE); END_IF;
     *
     ...
     * OPTIONAL [min : ?], min != 0, must be size >= min or size = 0 -> IF (size < min) AND (size <> 0) THEN RETURN (FALSE);
     * END_IF;
     *
     ...
     * OPTIONAL [max : min] - size must be 0 or max - min + 1 -> IF (size <> 0) AND (size <> max-min+1) THEN RETURN (FALSE);
     * END_IF;
     */
    ...
    if (arm_attribute.is_array) {
      ...
      if (arm_attribute.arm_attribute instanceof EExplicit_attribute) {
        ...
        if (xa.getOptional_flag(null)) {
          ...
          pw.println(indentString(alternative_nr) + "IF (size <> 0) AND (size <> "
              + (arm_attribute.max_cardinality - arm_attribute.min_cardinality + 1) + ") THEN");
          pw.println(indentString(alternative_nr) + "\tRETURN (TRUE);");
          pw.println(indentString(alternative_nr) + "END_IF;");
      ...
      if (arm_attribute.arm_attribute instanceof EExplicit_attribute) {
        ...
        if (arm_attribute.min_cardinality == Integer.MIN_VALUE) {
          ...
          else if (!xa.getOptional_flag(null)) {
            empty_if = false;
          ...
          if (arm_attribute.min_cardinality == 0) {
            ...
            pw.println(indentString(alternative_nr) + "IF ((size < " + arm_attribute.min_cardinality + ") AND (size <> 0)) OR (size > "
                + arm_attribute.max_cardinality + ") THEN");
            pw.println(indentString(alternative_nr) + "\tRETURN (TRUE);");
            pw.println(indentString(alternative_nr) + "END_IF;");
...
//			pw.println(indentString(alternative_nr) + "IF NOT VALUE_UNIQUE(results) THEN");
  ...
  boolean generateAggregateARMAttributeChecksSingleFunction(RuleGeneratorArmAttribute arm_attribute, String arm_entity_name, boolean is_supertype)
      ...
      if (arm_attribute.arm_attribute instanceof EExplicit_attribute) {
        ...
        if (xa.getOptional_flag(null)) {
          ...
          pw.println(indentString(alternative_nr) + "IF (size <> 0) AND (size <> "
              + (arm_attribute.max_cardinality - arm_attribute.min_cardinality + 1) + ") THEN");
          if (is_supertype) {
            pw.println(indentString(alternative_nr) + "\tprint(\'" + supertype_message + " \');");
          }
          ...
          if (flag_display_result || flag_display_cardinality) {
            ...
            else if (flag_display_cardinality) {
              pw.println(indentString(alternative_nr) + "\tprintln(size);");
            }
            else if (flag_display_result) {
              pw.println(indentString(alternative_nr) + "\tprintln(results);");
            }
            else {
              // internal error
...
// non-strong, non-aggregate, attribute alternatives in single function
  ...
  boolean generateNonAggregateARMAttributeChecksSingleFunction(RuleGeneratorArmAttribute arm_attribute, String arm_entity_name, boolean is_supertype)
      throws SdaiException {
...
//			pw.println(indentString(alternative_nr) + "IF NOT VALUE_UNIQUE(results) THEN");
  ...
  Vector collectArmAttributes(EEntity_mapping the_entity_mapping, EEntity_definition arm_entity, SdaiModel arm_model, SdaiModel map_model)
      throws SdaiException {
...
// non-strong, non-aggregate, attribute alternatives for multiple compatible entity_mapping alternatives in supertypes
  ...
  boolean generateNonAggregateARMAttributeChecksMultipleCompatibleAlternatives(RuleGeneratorArmAttribute arm_attribute, String arm_entity_name)
      throws SdaiException {
...
//					pw.println(indentString(alternative_nr) + "\tprintln(e);");
  ...
  boolean generateAggregateARMAttributeChecksMultipleCompatibleAlternatives(RuleGeneratorArmAttribute arm_attribute, String arm_entity_name)
      throws SdaiException {
...
// strong, non-aggregate, attribute alternatives in single function for multiple compatible entity_mapping alternatives
  ...
  boolean generateNonAggregateStrongARMAttributeChecksMultipleCompatibleAlternatives(RuleGeneratorArmAttribute arm_attribute, String arm_entity_name)
      throws SdaiException {
...
// strong, aggregate, single function attribute alternatives for multiple compatible entity_mapping alternatives
  ...
  boolean generateAggregateStrongARMAttributeChecksMultipleCompatibleAlternatives(RuleGeneratorArmAttribute arm_attribute, String arm_entity_name)
      throws SdaiException {
...
// EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE end
  ...
  boolean recurseSupertypesForSkipping(EEntity_definition arm_entity, EEntity_definition start_entity, SdaiModel arm_model, SdaiModel map_model,
      EEntity_mapping the_entity_mapping, int number_entity_alternatives, int index_entity_alternatives, HashSet processed) throws SdaiException {
    boolean result = true;
    boolean empty_if = true;
...
//			pw.println("\t\t-- recursing supertypes: from " + arm_entity.getName(null) + " to " + supertype.getName(null));
      result = recurseSupertypesForSkipping(supertype, start_entity, arm_model, map_model, the_entity_mapping, number_entity_alternatives,
          index_entity_alternatives, processed);
...
//						pw.println(indentString(1) + "\t\tIF NOT EXISTS(e." + ((EAttribute)constraints).getName(null).toLowerCase() + ") THEN");
              pw.println(indentString(1) + "\t\tIF NOT EXISTS(e\\" + ((EAttribute) constraints).getParent(null).getName(null).toLowerCase() + "."
                  + ((EAttribute) constraints).getName(null).toLowerCase() + ") THEN");
            }
            else {
        ...
        if (flag_swap_supertype_constraints) { // supertype entity_mapping constraints, if true, skips the instance
          ...
          for (int ii = 0; ii < compatible_entity_mappings.size(); ii++) {
            ...
            if (supertype_entity_mapping.testConstraints(null)) {
              ...
              printComments("\t-- constraints: " + constraints);
              if (constraints instanceof EAttribute) {
                pw.println(indentString(ii) + "\t\tIF NOT EXISTS(e\\" + ((EAttribute) constraints).getParent(null).getName(null).toLowerCase()
                    + "." + ((EAttribute) constraints).getName(null).toLowerCase() + ") THEN");
              }
              else {
...
// check__altered_package_3 - no loops only return false - should not do this if constraints not present
      ...
      else if (compatible_entity_mappings.size() == 1) {
...
//					pw.println("-- one target-compatible supertype alternative found");
          ...
          if (flag_swap_supertype_constraints) { // supertype entity_mapping constraints, if true, skips the instance
            ...
            if (supertype_entity_mapping.testConstraints(null)) {
              ...
              printComments("\t-- constraints: " + constraints);
              if (constraints instanceof EAttribute) {
                pw.println("\t\tIF NOT EXISTS(e\\" + ((EAttribute) constraints).getParent(null).getName(null).toLowerCase() + "."
                    + ((EAttribute) constraints).getName(null).toLowerCase() + ") THEN");
              }
              else {
...
// non-strong
  ...
  boolean recurseSupertypesForFailure(EEntity_definition arm_entity, EEntity_definition start_entity, SdaiModel arm_model, SdaiModel map_model,
      EEntity_mapping the_entity_mapping, int number_entity_alternatives, int index_entity_alternatives, HashSet processed) throws SdaiException {
    boolean result = true;
    boolean empty_if = true;
    int multiple_status = 0;
...
//			pw.println("\t\t-- recursing supertypes: from " + arm_entity.getName(null) + " to " + supertype.getName(null));
      ...
      result = recurseSupertypesForFailure(supertype, start_entity, arm_model, map_model, the_entity_mapping, number_entity_alternatives,
          index_entity_alternatives, processed);
      empty_if = empty_if && result;
    }
    // here we add our actions
...
//		pw.println("-- checking compatibility between AIM " + current_target_entity.getName(null) + " and supertype " + supertype_target_entity.getName(null));
  ...
  boolean recurseSupertypes(EEntity_definition current_entity, EEntity_definition supertype_entity, EEntity_definition start_entity, HashSet processed)
      throws SdaiException {
...
//		}
  ...
  // version 2
  void printFunction2(EEntity_definition arm_entity, EEntity_mapping the_entity_mapping, int i, int count, SdaiModel arm_model, SdaiModel aim_model,
      SdaiModel map_model) throws SdaiException {
    Vector result = new Vector();
    String arm_entity_name = arm_entity.getName(null).toLowerCase();
    indentation = 0;
    EEntity aim_target = the_entity_mapping.getTarget(null);
...
//				EExplicit_attribute xa = (EExplicit_attribute)arm_attribute.arm_attribute;
          EAttribute xa = arm_attribute.arm_attribute;
          if (arm_attribute.is_aggregate) {
            if (arm_attribute.min_cardinality == Integer.MIN_VALUE) {
              // both must be unset, nothing to check
              printDebug("\t\t-- [0:?] any size ok - not generated: ");
...
//					pw.println("FUNCTION get__" + arm_entity_name + "__" + xa.getName(null).toLowerCase() + "(e : " + aim_entity_name + ") : GENERIC;");
            pw.println("FUNCTION get__" + arm_entity_name + "__" + xa.getName(null).toLowerCase() + "(e : " + aim_entity_name
                + ") : BAG OF GENERIC;");
...
// set->bag						pw.println("FUNCTION get__" + arm_entity_name + "__" + xa.getName(null).toLowerCase() + "(e : " + aim_entity_name + ") : SET OF " + aim_entity_name + ";");
            pw.println("FUNCTION get__" + arm_entity_name + "__" + xa.getName(null).toLowerCase() + "(e : " + aim_entity_name + ") : BAG OF "
                + aim_entity_name + ";");
            ...
            for (i = 0; i < attribute_mapping_alternatives.size(); i++) {
               ...
               * // RRObjectMapping om = new RRObjectMapping(ASdaiModel data, ASdaiModel mapping,
               * touched_instances);
               * RRObjectMapping om = new RRObjectMapping(null, null, touched_instances);
               * int mode = 0;
               * // public boolean rrTestMappedAttribute(jsdai.lang.EEntity targetInstance,
               * EGeneric_attribute_mapping sourceAttribute, int mode) throws jsdai.lang.SdaiException {
               * // boolean tested_attribute_mapping = om.rrTestMappedAttribute((EEntity)aim_target,
               * (EGeneric_attribute_mapping)an_attribute_mapping, (int)0);
               *
               * boolean tested_attribute_mapping = om.rrTestMappedAttribute((jsdai.lang.EEntity) null,
               * (EGeneric_attribute_mapping) null, (int) mode);
               *
                ...
                if (last instanceof EPath_constraint) {
                  ...
                  if (last_path_element1 instanceof EEntity_constraint) {
                    ...
                    if (target_attribute instanceof EExplicit_attribute) {
                      EEntity_definition target_attribute_parent_entity = (EEntity_definition) ((EExplicit_attribute) target_attribute)
                          .getParent(null);
...
//	              	pw.println("\t\t\tee : " + target_attribute_value_domain.getName(null).toLowerCase() + " := e." + ((EExplicit_attribute)target_attribute).getName(null).toLowerCase() + ";");
                  pw.println("\t\t\tee : " + target_attribute_value_domain.getName(null).toLowerCase() + " := e\\"
                      + ((EAttribute) target_attribute).getParent(null).getName(null).toLowerCase() + "."
                      + ((EExplicit_attribute) target_attribute).getName(null).toLowerCase() + ";");
                }
                else {
...
//	              	pw.println("\t\t\tee : GENERIC_ENTITY;");
                ...
                /*
                 * // I think here we need to invoke the function that tests the ARM entity which is the domain of
                 * the attribute
...
// printDebug("--REMOVE - generate function: " + next_constraint);
  ...
  void processAttribute_value_constraint(EAttribute_value_constraint attribute_value_constraint) throws SdaiException {
    ...
    if (attribute instanceof EAttribute) {
      ...
      else if (attribute_value_constraint instanceof ENon_optional_constraint) {
...
//			EAttribute_mapping an_alternative = (EAttribute_mapping)alternative_iterator.next();
      ...
      else if (an_alternative instanceof EAttribute_mapping_value) {
    ...
    else if (arm_attribute instanceof EDerived_attribute) {
      domain = ((EDerived_attribute) arm_attribute).getDomain(null);
    }
    else if (arm_attribute instanceof EInverse_attribute) {
