...
import jsdai.mapping.EAggregate_size_constraint;
...
import jsdai.mapping.EConstraint;
...
import jsdai.mapping.EInstance_equal;
...
import jsdai.mapping.ENon_optional_constraint;
...
public class Aim2ArmGenerator {
  ...
  public static final void main(String args[]) throws SdaiException, IOException {
    ...
    for (int ihi = 0; ihi < args.length; ihi++) {
      ...
      if (args[ihi].equalsIgnoreCase("-no_binary")) {
        flag_commit = false;
      }
      ...
      else if (args[ihi].equalsIgnoreCase("-schema")) {
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-p21_in")) {
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
      ...
      else if (args[ihi].equalsIgnoreCase("-p21_out")) {
        ihi++;
        if (ihi < args.length) {
          if (args[ihi].substring(0, 1).equals("-")) {
...
// System.out.println("AIM name: " + ge.AIM_name);
    ...
    if (ge.ARM_name == null) {
      ...
      else if (ge.AIM_name.equalsIgnoreCase("AUTOMOTIVE_DESIGN")) {
        ge.ARM_name = "AP214_ARM";
      }
      else if (ge.AIM_name.equalsIgnoreCase("ELECTROTECHNICAL_DESIGN")) {
...
// printDebug("ARM model found:" + arm_model.getName());
      ...
      else if (model_name.equalsIgnoreCase(AIM_name)) {
        aim_model = model;
...
// printDebug("AIM model found:" + aim_model.getName());
      ...
      else if (model_name.equalsIgnoreCase(MAP_name)) {
        map_model = model;
...
// printDebug("MAP model found:" + map_model.getName());
     ...
     * inverse_attribute_constraint, (* there must be at least one inverse related instance. Could be non_optional_constraint
     * instead *)
     ...
     * It seems that there is no difference because for the longest path spreading the wave multiple times may be needed in
     * both cases.
     * But with the front approach more loops are needed through all the instances or through the front instances in a
     * separate aggregate.
     */
      ...
      for (int i = 0; i < own_entity_mapping.size(); i++) {
        ...
        while (iter.next()) {
          ...
          if (flag_one_attribute_only) {
            ...
            else if (attribute_count > NR_ATTRIBUTE) {
              break;
            }
          }
          ...
          for (int j = 0; j < attribute_mapping_alternatives.size(); j++) {
            if (flag_one_attribute_mapping_alternative_only) {
              ...
              else if (j > NR_ATTRIBUTE_MAPPING_ALTERNATIVE) {
                break;
              }
            }
...
//					constraint.setTemp(entity_array);
          ...
          printDebug(">>>>> constraint level: " + constraint_level + ", count: " + processed_constraint_instances_count + ", constraint: "
              + constraint, 3);
    ...
    while (arm_iter.next()) {
      ...
      for (int i = 0; i < entity_mapping_alternatives.size(); i++) {
        ...
        if (the_entity_mapping.testConstraints(null)) {
          ...
          if (mapping_constraints instanceof EConstraint) {
          ...
          else if (mapping_constraints instanceof EExplicit_attribute) {
            arm_instances = getEntityInstancesExt(mapping_constraints);
            ...
            // filtering done later
            System.out.println("XXX mapping constraints = explicit_attribute: " + mapping_constraints + ", source: "
                + the_entity_mapping.getSource(null));
          }
          else {
        ...
        printInstanceArray(
            "before attributes - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                + the_entity_mapping.getSource(null).getName(null), arm_instances, null, 1);
...
// }
            ...
            printInstanceArray("union op1 - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                    + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                    + the_entity_mapping.getSource(null).getName(null) + ", attribute: " + an_attribute.getName(null), attribute_arm_instances,
                null, 1);
            printInstanceArray("union op2 - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                    + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                    + the_entity_mapping.getSource(null).getName(null) + ", attribute: " + an_attribute.getName(null),
                attribute_alternative_instances, null, 1);
            attribute_arm_instances = performUnion(attribute_arm_instances, attribute_alternative_instances);
            printInstanceArray("union result - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                    + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                    + the_entity_mapping.getSource(null).getName(null) + ", attribute: " + an_attribute.getName(null), attribute_arm_instances,
                null, 1);
          } // for
          if (!no_constraints) {
            ...
            printInstanceArray("op1 - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                + the_entity_mapping.getSource(null).getName(null) + ", attribute: " + an_attribute.getName(null), arm_instances, null, 1);
            printInstanceArray("op2 - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                    + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                    + the_entity_mapping.getSource(null).getName(null) + ", attribute: " + an_attribute.getName(null), attribute_arm_instances,
                null, 1);

            arm_instances = performAnd(arm_instances, attribute_arm_instances);
            printInstanceArray("result - arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                + the_entity_mapping.getSource(null).getName(null) + ", attribute: " + an_attribute.getName(null), arm_instances, null, 1);
          }
        } // while arm attributes
...
// }
        ...
        printInstanceArray(
            "arm instances for entity: " + arm_entity.getName(null) + ", alternative target: "
                + ((EEntity_definition) the_entity_mapping.getTarget(null)).getName(null) + ", source: "
                + the_entity_mapping.getSource(null).getName(null), arm_instances, null, 1);
    ...
    System.out.println("Duration of processing of " + processed_constraint_instances_count + " constraint instances: " + processing_duration
        + "  milliseconds");
...
//#################################### INTERNALLY USED METHODS ##################################################
   ...
   * An additional advantage is in the implementation - in both cases when a constraint points to an attribute or to another
   * constraint
...
//					return getEntityInstancesExtNR(ee);
    ...
    else if (ee instanceof EInverse_attribute_constraint) {
...
//					return getEntityInstancesExtNR(ee);
  ...
  EEntity[] getEntityInstancesExtNR(EEntity ee) throws SdaiException {
    ...
    else if (ee instanceof EAttribute) {
      temp = ee.getTemp();
      if (temp instanceof EEntity[]) {
        return (EEntity[]) temp;
      }
...
//				printInstanceArray("after sorting: ", temp_array, ee, 0);
    ...
    else if (ee instanceof EEntity_definition) {
      temp = ee.getTemp();
      if (temp instanceof EEntity[]) {
        return (EEntity[]) temp;
      }
    ...
    else if (ee instanceof EDefined_type) {
      temp = ee.getTemp();
      if (temp instanceof EEntity[]) {
        return (EEntity[]) temp;
      }
...
//					all_instances_count = addArray(all_instances, all_instances_count, temp_array);
    ...
    else if (ee instanceof EAggregation_type) {
      temp = ee.getTemp();
      if (temp instanceof EEntity[]) {
        return (EEntity[]) temp;
      }
...
//					all_instances_count = addArray(all_instances, all_instances_count, temp_array);
  ...
  void exploreAggregate(EAggregation_type at, HashSet entities) throws SdaiException {
    ...
    else if (element instanceof EEntity_definition) {
      // add instances
    ...
    else if (element instanceof ESimple_type) {
  ...
  void add2InstanceArray(EEntity inst) {
    ...
    instance_array[instance_array_count++] = inst;
  }
...
//		if (attribute instanceof EExplicit_attribute) {
      ...
      if (attribute2 instanceof EAttribute) {
        for (int i = 0; i < instances.length; i++) {
          ...
          if (instance.testAttribute((EAttribute) attribute2, select_path) == 1) {
            ...
            printWarning("in real_constraint, in aggregate_member_constraint, instance attribute unset or of wrong type, instance: " + instance
    ...
    if (constraint instanceof EExact_entity_constraint) {
      ...
      if (attribute instanceof EExplicit_attribute) {
        ...
        if (((EInverse_attribute_constraint) attribute).testInverted_attribute(null)) {
          ...
          printError("mandatory attribute inverted_attribute unset in exact_entity_constraint, where attribute is inverse_attribute_constraint: "
              + attribute);
        }
        ...
        if (inverted_attribute instanceof EExplicit_attribute) {
          ...
          if (((EAggregate_member_constraint) inverted_attribute).testAttribute(null)) {
            ...
            printError("mandatory attribute attribute unset in aggregate_member_constraint in inverse_attribute_constraint in exact_entity_constraint: "
                + inverted_attribute);
          }
          ...
          for (int i = 0; i < instances.length; i++) {
            ...
            if (instance.isInstanceOf(domain)) {
               ...
               * } else {
               * printWarning(
               * "in exact_entity_constraint, in attribute = inverse_attribute_constraint, inverse_attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
               * + instance + ", attribute: " + attribute2);
               * }
               */
            }
          } // for
          ...
          printError("NOT YET SUPPORTED OR WRONG inverted_attribute_type in exact_entity_constraint where attribute is inverse_attribute_constraint: "
              + inverted_attribute);
        }
      }
      else if (attribute instanceof EAggregate_member_constraint) {
        ...
        if (attribute2 instanceof EExplicit_attribute) {
          for (int i = 0; i < instances.length; i++) {
            ...
            if (instance.testAttribute((EAttribute) attribute2, select_path) == 1) {
              ...
              printWarning("in exact_entity_constraint, in attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
                  + instance + ", attribute: " + attribute2);
            }
          }
          // requires sorting !!
      ...
      if (attribute instanceof EExplicit_attribute) {
        ...
        if (((EInverse_attribute_constraint) attribute).testInverted_attribute(null)) {
          ...
          printError("mandatory attribute inverted_attribute unset in entity_constraint, where attribute is inverse_attribute_constraint: "
              + attribute);
        }
        ...
        if (inverted_attribute instanceof EExplicit_attribute) {
          ...
          if (((EAggregate_member_constraint) inverted_attribute).testAttribute(null)) {
            ...
            printError("mandatory attribute attribute unset in aggregate_member_constraint in inverse_attribute_constraint in entity_constraint: "
                + inverted_attribute);
          }
          ...
          for (int i = 0; i < instances.length; i++) {
            ...
            if (instance.isKindOf(domain)) {
               ...
               * } else {
               * printWarning(
               * "in entity_constraint, in attribute = inverse_attribute_constraint, inverse_attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
               * + instance + ", attribute: " + attribute2);
               * }
               */
            }
          ...
          printError("NOT YET SUPPORTED OR WRONG inverted_attribute_type in entity_constraint where attribute is inverse_attribute_constraint: "
              + inverted_attribute);
        }
      }
      else if (attribute instanceof EAggregate_member_constraint) {
        ...
        /*
         * take as an input the instance array from the aggregate_member_constraint (parent entity population of its
         * attribute)
        ...
        if (attribute2 instanceof EExplicit_attribute) {
          for (int i = 0; i < instances.length; i++) {
            ...
            if (instance.testAttribute((EAttribute) attribute2, select_path) == 1) {
              ...
              printWarning("in entity_constraint, in attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
                  + instance + ", attribute: " + attribute2);
            }
          }
          // requires sorting !!
  ...
  void processInstance_constraint(EInstance_constraint constraint, EEntity element2) throws SdaiException {
    ...
    if (constraint instanceof EAnd_constraint_relationship) {
      ...
      if (elements1.length == 0) {
        ...
        printInstanceArray("or_constraint_relationship - (at least) elements1 is empty, element1: " + element1 + ", element2: " + element2, elements2,
            constraint, 3);
        constraint.setTemp(elements2);
      }
      else if (elements2.length == 0) {
        printInstanceArray("or_constraint_relationship - element2 is empty, element1: " + element1 + ", element2: " + element2, elements1, constraint,
            3);
    ...
    if (element1 instanceof EAggregate_member_constraint) {
      ...
      for (int i = 0; i < elements1.length; i++) {
        ...
        if (attribute instanceof EExplicit_attribute) {
          if (instance.testAttribute((EAttribute) attribute, select_path) == 1) {
            ...
            printWarning("in path_constraint, in element1 aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
      ...
      if (inverted_attribute instanceof EExplicit_attribute) {
        ...
        if (((EAggregate_member_constraint) inverted_attribute).testAttribute(null)) {
          ...
          printError("mandatory attribute attribute unset in aggregate_member_constraint in inverse_attribute_constraint in element1 of path_constraint: "
              + inverted_attribute);
        }
        ...
        if (attribute instanceof EExplicit_attribute) {
          for (int i = 0; i < and_array.length; i++) {
            ...
            if (instance.testAttribute((EAttribute) attribute, select_path) == 1) {
              ...
              printWarning(
                  "in path_constraint, in element1 = inverse_attribute_constraint, inverse_attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
                      + instance + ", attribute: " + attribute);
            }
          } // for
          // requires sorting!!
          ...
          printError(
              "NOT YET SUPPORTED OR WRONG inverted_attribute = aggregate_member_constraint attribute type in path_constraint where element1 is inverse_attribute_constraint: "
                  + attribute);
        }

      }
      else {
        printError("NOT YET SUPPORTED OR WRONG inverted_attribute type in path_constraint where element1 is inverse_attribute_constraint: "
            + inverted_attribute);
      }
      ...
      if (attribute instanceof EExplicit_attribute) {
        ...
        if (((EAggregate_member_constraint) attribute).testAttribute(null)) {
          ...
          printError("mandatory attribute attribute unset in aggregate_member_constraint in entity_constraint - element1 of path_constraint: "
              + attribute);
        }
        ...
        if (attribute2 instanceof EExplicit_attribute) {
          for (int i = 0; i < elements1.length; i++) {
            ...
            if (instance.testAttribute((EAttribute) attribute2, select_path) == 1) {
              ...
              printWarning(
                  "in path_constraint in element1 = entity_constraint, in attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
                      + instance + ", attribute: " + attribute2);
            }
          }
          printInstanceArray("path_constraint, element1 = entity_constraint, attribute = aggregate_member_constraint, element1: " + element1
              + ", element2: " + element2, null, constraint, 3);
          // the result is sorted, does not require sorting
          moveInstanceArray2Constraint(constraint);
        }
        else {
          printError("NOT YET SUPPORTED OR WRONG attribute type in aggregate_member_constraint in entity_constraint in path_constraint element1: "
              + attribute2);
        }
        ...
        if (((EInverse_attribute_constraint) attribute).testInverted_attribute(null)) {
          ...
          printError(
              "mandatory attribute inverted_attribute unset in path_constraint where element1 is entity_constraint and its attribute is inverse_attribute_constraint:: "
                  + attribute);
        }
        ...
        if (inverted_attribute instanceof EExplicit_attribute) {
          ...
          for (int i = 0; i < and_array.length; i++) {
            ...
            if (instance.testAttribute((EAttribute) inverted_attribute, select_path) == 1) {
              ...
              printWarning("instance value unset in path_constraint, element1 = entity_constraint, attribute = inverse_attribute_constraint: "
                  + instance);
            }
          } // for
          // requires sorting!!
          ...
          if (((EAggregate_member_constraint) inverted_attribute).testAttribute(null)) {
            ...
            printError(
                "mandatory attribute attribute unset in aggregate_member_constraint in inverse_attribute_constraint in entity_constraint in element1 of path_constraint: "
                    + inverted_attribute);
          }
          ...
          if (attribute2 instanceof EExplicit_attribute) {
            for (int i = 0; i < and_array.length; i++) {
              ...
              if (instance.testAttribute((EAttribute) attribute2, select_path) == 1) {
                ...
                printWarning(
                    "in path_constraint, in element1 = entity_constraint, attribute = inverse_attribute_constraint, inverted_attribute = aggregate_member_constraint, instance attribute unset or of wrong type, instance: "
                        + instance + ", attribute: " + attribute2);
              }
            } // for
            // requires sorting!!
            ...
            printError(
                "NOT YET SUPPORTED OR WRONG attribute type in aggregate_member_constraint in inverse_attribute_constraint in entity_constraint in path_constraint element1: "
                    + attribute2);
          }
        }
        else {
          printError("NOT YET SUPPORTED OR WRONG inverted attribute type in inverse_attribute_constraint in entity_constraint in path_constraint element1: "
              + inverted_attribute);
        }
  ...
  void processInverse_attribute_constraint(EInverse_attribute_constraint constraint) throws SdaiException {
     ...
     * then for every instance of the population, with USEDIN (findEntityInstanceUsedin(attribute, where_to_search,
     * result_aggregate)
  ...
  void processNegation_constraint(ENegation_constraint constraint) throws SdaiException {
    ...
    else if (constraints instanceof EConstraint) {
...
//##################################### DEBUGGING #############################
  ...
  void printInstanceArray(String message, EEntity[] array, EEntity constraint, int debug_detail_level) {
    ...
    if (flag_debug_detail_level < debug_detail_level) {
      return;
    }
    int count;
    if (array == null) {
  ...
  void printElement1(EEntity element1) {
    ...
    if (element1 instanceof EConstraint) {
      if (element1 instanceof EConstraint_attribute) { // ABSTRACT
        ...
        else if (element1 instanceof ESelect_constraint) {
      ...
      else if (element1 instanceof EConstraint_relationship) { // ABSTRACT
        if (element1 instanceof EInstance_constraint) { // ABSTRACT
          ...
          else if (element1 instanceof EInstance_equal) {
            System.out.println("@@@###-17 ELEMENT1: instance_equal: " + element1);
          }
          else if (element1 instanceof EOr_constraint_relationship) {
        ...
        else if (element1 instanceof EPath_constraint) {
      ...
      else if (element1 instanceof EEnd_of_path_constraint) {
      ...
      else if (element1 instanceof ENegation_constraint) {
        System.out.println("@@@###-25 ELEMENT1: negation_constraint: " + element1);
      }
      else if (element1 instanceof EType_constraint) {
    ...
    else if (element1 instanceof EExplicit_attribute) {
