 ...
 */
public class ExpressStringScanner extends RuleBasedScanner implements IExpressKeywords {
...
//	private ExpressColorProvider fColorProvider;
	 ...
	 */
	public ExpressStringScanner(ExpressColorProvider provider) {	
...
*/
		...
		Color str_bg = null;
		boolean str_transparent = store.getBoolean(ExpressEditorPreferences.EXPRESS_STRING_TRANSPARENT);
		if (!str_transparent) {
			str_bg = provider.getColor(provider.getColorPreference(ExpressEditorPreferences.EXPRESS_STRING_BG_COLOR));
		}
...
//		ExpressColorProvider provider = ExpressCompilerPlugin.getDefault().getExpressColorProvider();
		...
		IToken stringToken = new Token(new TextAttribute(provider.getColor(provider.getColorPreference(ExpressEditorPreferences.EXPRESS_STRING_COLOR)), str_bg, str_flags));
