 ...
 *
 */
public class EGPageFromRef extends EGPageFrom implements IPageOverrider {
	protected TextWrapper wrapper2 = null;
	protected EGEntityRef referenceEG = null;

	public EGPageFromRef(PropertySharing prop, AbstractEGBox referencedObject) {
	...
	public void setType(int type) {
		EGEntityRef refEG = getEGRef();
		refEG.setType(type);
	}
  
	public int getType() {
		EGEntityRef refEG = getEGRef();
		return refEG == null ? EGEntityRef.TYPE_USED : refEG.getType();
	}

  	public void setSchemaName(String schemaName) {
		EGEntityRef refEG = getEGRef();
  		refEG.setSchemaName(schemaName);
		wrapper.setText(getText());
  	}
  
  	public String getSchemaName() {
		EGEntityRef refEG = getEGRef();
  		return refEG == null ? "" : refEG.getSchemaName();
  	}
  
  	public AbstractEGBox getEGEntity() {
		EGEntityRef refEG = getEGRef();
  		return refEG.getEGEntity();
  	}
  
  	public void setRename(String rename) {
		EGEntityRef refEG = getEGRef();
  		refEG.setRename(rename);
  		wrapper2.setText(rename);
  	}
  
  	public String getRename() {
		EGEntityRef refEG = getEGRef();
  		return refEG == null ? "" : refEG.getRename();
  	}
...
//	  			System.err.println("Failed to set item size properly");
	...
	/* (non-Javadoc)
	 * @see jsdai.express_g.exp2.eg.EGPageFrom#addReference(jsdai.express_g.exp2.eg.EGPageTo)
	 */
	...
	/* (non-Javadoc)
	 * @see jsdai.express_g.exp2.eg.EGPageFrom#removeReference(jsdai.express_g.exp2.eg.EGPageTo)
	 */
	...
	public String getName() {
		EGEntityRef refEG = getEGRef();
		return refEG == null ? "" : refEG.getName();
	}
	...
	public EGEntityRef getEGRef() {
		if ((referenceEG == null)&&
				(references != null)&&(references.size() > 0)) {
			EGPageTo ref = (EGPageTo)references.iterator().next();
			AbstractEGRelation rel = ref.getRefRelation();
			if (rel != null) {
				AbstractEGBox parent = rel.getParent();
				if (parent instanceof EGEntityRef)
					referenceEG = (EGEntityRef)parent;
			}
