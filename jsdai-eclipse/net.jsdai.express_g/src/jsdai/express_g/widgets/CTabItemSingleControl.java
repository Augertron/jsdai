 ...
 * </p>
 */
public class CTabItemSingleControl extends Item {
	CTabFolderSingleControl parent;
	...
	int closeImageState = CTabFolderSingleControl.NONE;
 ...
 * @see Widget#getStyle()
 */
public CTabItemSingleControl (CTabFolderSingleControl parent, int style) {
 ...
 * @see Widget#getStyle()
 */
public CTabItemSingleControl (CTabFolderSingleControl parent, int style, int index) {
...
void drawClose(GC gc) {
	...
	// draw X 9x9
	int indent = Math.max(1, (CTabFolderSingleControl.BUTTON_SIZE-9)/2);
	int x = closeRect.x + indent;
	int y = closeRect.y + indent;
	y += parent.onBottom ? -1 : 1;
	
	Color closeBorder = display.getSystemColor(CTabFolderSingleControl.BUTTON_BORDER);
	switch (closeImageState) {
		case CTabFolderSingleControl.NORMAL: {
			...
			gc.setBackground(display.getSystemColor(CTabFolderSingleControl.BUTTON_FILL));
			gc.fillPolygon(shape);
			gc.setForeground(closeBorder);
			gc.drawPolygon(shape);
			break;
		}
		case CTabFolderSingleControl.HOT: {
			int[] shape = new int[] {x,y, x+2,y, x+4,y+2, x+5,y+2, x+7,y, x+9,y, 
					                 x+9,y+2, x+7,y+4, x+7,y+5, x+9,y+7, x+9,y+9,
			                         x+7,y+9, x+5,y+7, x+4,y+7, x+2,y+9, x,y+9,
			                         x,y+7, x+2,y+5, x+2,y+4, x,y+2};
			Color fill = new Color(display, CTabFolderSingleControl.CLOSE_FILL);
		...
		case CTabFolderSingleControl.SELECTED: {
		...
		case CTabFolderSingleControl.NONE: {
...
void drawSelected(GC gc ) {
	...
	if (parent.single) {
		...
		if (!isShowing()){
			...
			gc.setForeground(CTabFolderSingleControl.borderColor);
		...
		if (this.parent.onBottom) {
			int[] left = parent.simple ? CTabFolderSingleControl.SIMPLE_BOTTOM_LEFT_CORNER : CTabFolderSingleControl.BOTTOM_LEFT_CORNER;
			int[] right = parent.simple ? CTabFolderSingleControl.SIMPLE_BOTTOM_RIGHT_CORNER : parent.curve;
			...
			int[] left = parent.simple ? CTabFolderSingleControl.SIMPLE_TOP_LEFT_CORNER : CTabFolderSingleControl.TOP_LEFT_CORNER;
			int[] right = parent.simple ? CTabFolderSingleControl.SIMPLE_TOP_RIGHT_CORNER : parent.curve;
			if (parent.borderLeft == 0 && parent.indexOf(this) == parent.firstIndex) {
				left = new int[]{x, y};
			}
			shape = new int[left.length+right.length+8];
		...
		parent.antialias(shape, CTabFolderSingleControl.borderColor.getRGB(), inside, outside, gc);
		gc.setForeground(CTabFolderSingleControl.borderColor);
	...
	if (parent.single && (parent.showClose || showClose)) xDraw += CTabFolderSingleControl.BUTTON_SIZE; 
	...
	if (textWidth > 0) {
		...
		if (parent.isFocusControl()) {
			...
			if (parent.simple || parent.single) {
				...
				gc.setForeground(display.getSystemColor(CTabFolderSingleControl.BUTTON_BORDER));
 ...
 * </ul>
 */
public CTabFolderSingleControl getParent () {
	//checkWidget();
	return parent;
}
/**
...
int preferredWidth(GC gc, boolean isSelected, boolean minimum) {
	...
	if (parent.showClose || showClose) {
		if (isSelected || parent.showUnselectedClose) {
			if (w > 0) w += INTERNAL_SPACING;
			w += CTabFolderSingleControl.BUTTON_SIZE;
		}
	}
