...
public class CTabFolderSingleControl extends Composite {
	...
	/* item management */
	CTabItemSingleControl items[] = new CTabItemSingleControl[0];
	int selectedIndex = -1;
	int firstIndex = -1; // index of the left most visible tab.

	/* External Listener management */
	CTabFolderSingleControl2Listener[] folderListeners = new CTabFolderSingleControl2Listener[0];
	// support for deprecated listener mechanism
	CTabFolderSingleControlListener[] tabListeners = new CTabFolderSingleControlListener[0]; 
 ...
 * @see #getStyle()
 */
public CTabFolderSingleControl(Composite parent, int style) {
 ...
 * @since 3.0
 */
public void addCTabFolder2Listener(CTabFolderSingleControl2Listener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	// add to array
	CTabFolderSingleControl2Listener[] newListeners = new CTabFolderSingleControl2Listener[folderListeners.length + 1];
 ...
 */
public void addCTabFolderListener(CTabFolderSingleControlListener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	// add to array
	CTabFolderSingleControlListener[] newTabListeners = new CTabFolderSingleControlListener[tabListeners.length + 1];
...
void createItem (CTabItemSingleControl item, int index) {
	if (0 > index || index > getItemCount ()){ 
		SWT.error (SWT.ERROR_INVALID_RANGE);
	}
	// grow by one and rearrange the array.
	CTabItemSingleControl[] newItems = new CTabItemSingleControl [items.length + 1];
...
void destroyItem (CTabItemSingleControl item) {
	if (inDispose) return;
	...
	if (items.length == 1) {
		items = new CTabItemSingleControl[0];
		selectedIndex = -1;
		firstIndex = 0;
...
void drawMaximize(GC gc) {
	...
	Display display = getDisplay();
	// 5x4 or 7x9
	int x = maxRect.x + (CTabFolderSingleControl.BUTTON_SIZE - 10)/2;
	int y = maxRect.y + 3;
...
void drawTabArea(Event event) {
	...
	// Draw selected tab
	if (selectedIndex != -1) {
		CTabItemSingleControl item = items[selectedIndex];
 ...
 * </ul>
 */
public CTabItemSingleControl getItem (int index) {
	//checkWidget();
 ...
 *	</ul>
 */
public CTabItemSingleControl [] getItems() {
	//checkWidget();
	CTabItemSingleControl[] tabItems = new CTabItemSingleControl [items.length];
...
int getLastIndex() {
	...
	for (int i = firstIndex; i < items.length; i++) {
		CTabItemSingleControl item = items[i];
 ...
 *	</ul>
 */
public CTabItemSingleControl getSelection() {
	//checkWidget();
 ...
 * </ul>
 */
public int indexOf(CTabItemSingleControl item) {
	checkWidget();
	if (item == null) {
		SWT.error(SWT.ERROR_NULL_ARGUMENT);
	}
...
void onMouse(Event event) {
	...
	switch (event.type) {
		...
		case SWT.MouseDown: {
			...
			CTabItemSingleControl item = null;
			if (single) {
				if (selectedIndex != -1) {
		...
		case SWT.MouseUp: {
			...
			if (chevronRect.contains(x, y)) {
				...
				if (!selected) return;
				CTabFolderEventSingleControl e = new CTabFolderEventSingleControl(this);
				e.widget = this;
				e.time = event.time;
				e.x = chevronRect.x;
				e.y = chevronRect.y;
			...
			if (item != null) {
				if (item.closeRect.contains(x,y)) {
					...
					for (int j = 0; j < folderListeners.length; j++) {
						CTabFolderSingleControl2Listener listener = folderListeners[j];
						listener.close(e);
					}
					for (int j = 0; j < tabListeners.length; j++) {
						CTabFolderSingleControlListener listener = tabListeners[j];
 ...
 * @since 3.0
 */
public void removeCTabFolder2Listener(CTabFolderSingleControl2Listener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	...
	if (folderListeners.length == 1) {
		folderListeners = new CTabFolderSingleControl2Listener[0];
		return;
	}
	CTabFolderSingleControl2Listener[] newTabListeners = new CTabFolderSingleControl2Listener[folderListeners.length - 1];
 ...
 */
public void removeCTabFolderListener(CTabFolderSingleControlListener listener) {
	checkWidget();
	if (listener == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	...
	if (tabListeners.length == 1) {
		tabListeners = new CTabFolderSingleControlListener[0];
		return;
	}
	CTabFolderSingleControlListener[] newTabListeners = new CTabFolderSingleControlListener[tabListeners.length - 1];
...
void setButtonBounds() {
			...
			case SWT.FILL: {
				...
				if (single) {
					...
					if (items.length == 0) {
						...
						CTabItemSingleControl item = items[items.length - 1];
	...
	if (single) {
		...
		if (showChevron) {
			...
			chevronRect.height = BUTTON_SIZE;
			int lastIndex = getLastIndex();
			CTabItemSingleControl lastItem = items[lastIndex];
 ...
 * </ul>
 */
public void setInsertMark(CTabItemSingleControl item, boolean after) {
	checkWidget();
	int index = -1;
	if (item != null) {
		index = indexOf(item);
...
boolean setItemLocation() {
	...
	if (single) {
		...
		for (int i = 0; i < items.length; i++) {
			if (i == selectedIndex) {
				...
				if (showClose || item.showClose) {
					item.closeRect.x = borderLeft + CTabItemSingleControl.LEFT_MARGIN;
		...
		for (int i = firstIndex - 1; i >= 0; i--) { 
			...
			item.x = x;
			item.y = y;
			item.closeRect.x = item.x + item.width - BUTTON_SIZE - CTabItemSingleControl.RIGHT_MARGIN;
			item.closeRect.y = onBottom ? size.y - borderBottom - tabHeight + (tabHeight - BUTTON_SIZE)/2 : borderTop + (tabHeight - BUTTON_SIZE)/2;
		}
		...
		for (int i = firstIndex; i < items.length; i++) {
			...
			if (i == selectedIndex) {
				...
				item.closeRect.x = edge - CTabItemSingleControl.RIGHT_MARGIN - BUTTON_SIZE;
			} else {
				item.closeRect.x = item.x + item.width - CTabItemSingleControl.RIGHT_MARGIN - BUTTON_SIZE;
			}
			item.closeRect.y = onBottom ? size.y - borderBottom - tabHeight + (tabHeight - BUTTON_SIZE)/2: borderTop + (tabHeight - BUTTON_SIZE)/2;
			x = x + item.width;
...
boolean setItemSize() {
	...
	if (single) {
		showChevron = true;
		if (selectedIndex != -1) {
			CTabItemSingleControl tab = items[selectedIndex];
			GC gc = new GC(this);
...
//	widths = new int[items.length];
	...
	for (int i = 0; i < items.length; i++) {
		CTabItemSingleControl tab = items[i];
		int width = widths[i];
		if (tab.height != tabHeight || tab.width != width) {
			changed = true;
			tab.shortenedText = null;
 ...
 * </ul>
 */
public void setSelection(CTabItemSingleControl item) {
	checkWidget();
...
public void setSelection(int index) {
	/** XXX
	 * modified for use with same control in multiple tabs
	 */
	checkWidget();
...
/*	if (selectedIndex == index) {
	...
	if (oldIndex != -1) {
		...
		Control control = items[oldIndex].control;
		if (control != null && !control.isDisposed()) {
			control.setVisible(false);
		}
	}
 ...
 * @since 2.0
 */
public void showItem (CTabItemSingleControl item) {
	checkWidget();
	if (item == null) SWT.error (SWT.ERROR_NULL_ARGUMENT);
	...
	for (int i = 0; i < items.length; i++) {
		...
		item.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				int index = indexOf((CTabItemSingleControl)menuItem.getData(id));
				CTabFolderSingleControl.this.setSelection(index, true);
			}
		});
	}
	int x = rect.x;
...
String _getToolTip(int x, int y) {
	...
	CTabItemSingleControl item = getItem(new Point (x, y));
