 ...
 *
 */
public class ExportSTEPPage1 extends WizardPage {
	...
	private static final String INVALID_DESTINATION_FILE_MESSAGE = "No valid destination file selected";
	...
	private FileSelectGroup destFileGroup;

	/**
	 * changing of this string may require clearing metadata info associated with ExportSTEPPage1.STORE_NAMES_ID
	 */
	public final static String STORE_NAMES_ID =	"ExportSTEPPage1.STORE_NAMES_ID"; //$NON-NLS-1$
	public final static String STORE_LAST_NAME_ID =	"ExportSTEPPage1.STORE_LAST_NAME_ID"; //$NON-NLS-1$

	/**
	 * 
	 * @param name
	 */
	public ExportSTEPPage1(IPath projectPath, String name) {
		super(name);
	...
	public void createControl(Composite parent) {
		...
		destFileGroup =
			new FileSelectGroup(mainComposite, "stepFile", "Select STEP file", "File:",
					"Select STEP file", DIALOG_FILE, SWT.SAVE,
					null, INVALID_DESTINATION_FILE_MESSAGE);
		destFileGroup.createGroupControls();
	...
	public String getDestinationFile() {
		try {
			File selectedFile = destFileGroup.getSelectedFile();
			return selectedFile != null ? selectedFile.getCanonicalPath() : null;
		} catch (IOException e) {
			throw (IllegalArgumentException)new IllegalArgumentException(e.getMessage()).initCause(e);
		}
	...
	public void restoreWidgetValues() {
			...
			if (settings != null) {
				...
				destFileGroup.restoreSettings(settings);
				updateWidgetEnablements();
			}
	}
	...
	public void saveWidgetValues() {
		...
		if (settings != null) {
			...
			destFileGroup.saveSettings(settings);
		}
	}
	...
	protected boolean determinePageCompletion() {
		boolean complete =
			sourceFileGroup.validateSourceGroup() && destFileGroup.validateSourceGroup();
...
//			enableButtonGroup(true);
		...
		private void restoreSettings(IDialogSettings settings) {
			String[] sourceNames = settings.getArray(groupId + STORE_NAMES_ID);
			if (sourceNames != null) {
				for (int i = 0; i < sourceNames.length; i++)
					if (!NONE_AVAILABLE.equalsIgnoreCase(sourceNames[i]))
						fileNameField.add(sourceNames[i]);
				String lastName = settings.get(groupId + STORE_LAST_NAME_ID);
				if(lastName != null) {
		...
		private void saveSettings(IDialogSettings settings) {
			try {
				...
				settings.put(groupId + STORE_NAMES_ID, items);
				settings.put(groupId + STORE_LAST_NAME_ID, getSelectedFile().getCanonicalPath());
			} catch (IOException e) {
				throw (IllegalArgumentException)new IllegalArgumentException(e.getMessage()).initCause(e);
			}
		}
