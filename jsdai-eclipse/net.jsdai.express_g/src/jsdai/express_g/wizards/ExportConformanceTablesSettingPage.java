 ...
 *
 */
public class ExportConformanceTablesSettingPage extends WizardPage {
	private static final String NONE_AVAILABLE = "<nothing selected>";
	
	public static final String NAME_BROWSE = "Browse...";
	private static final String INVALID_CSS_FILE_MESSAGE = "No valid CSS file selected";
	private static final String INVALID_ARM_FILE_MESSAGE = "No valid ARM repository file selected";
	private static final String INVALID_MIM_FILE_MESSAGE = "No valid MIM repository file selected";
	private static final String INVALID_SOURCE_FILE_MESSAGE = "No valid source file selected";
	private static final String INVALID_DESTINATION_DIRECTORY_MESSAGE = "No valid directory selected";
	...
	private FileSelectGroup sourceFileGroupCss;
	private FileSelectGroup sourceFileGroupArm;
	private FileSelectGroup sourceFileGroupMim;
	 ...
	 */
	public final static String STORE_SOURCE_NAMES_ID =	"ExportConformanceTablesSettingPage.STORE_SOURCE_NAMES_ID"; //$NON-NLS-1$
	public final static String STORE_LAST_SOURCE_NAME_ID =	"ExportConformanceTablesSettingPage.STORE_LAST_SOURCE_NAME_ID"; //$NON-NLS-1$
	public final static String STORE_JAVA_OPTION_ID =	"ExportConformanceTablesSettingPage.STORE_JAVA_OPTION_ID"; //$NON-NLS-1$

	/**
	 * 
	 * @param name
	 */
	public ExportConformanceTablesSettingPage(IPath projectPath, String name) {
		super(name);
	...
	public void createControl(Composite parent) {
		...
		sourceFileGroupCss =
			new FileSelectGroup(mainComposite, "css", "Choose CSS file", "File:", "Choose CSS file",
					DIALOG_FILE, SWT.OPEN, new String[]{"*.xml"}, INVALID_CSS_FILE_MESSAGE);
		sourceFileGroupArm =
			new FileSelectGroup(mainComposite, "ARMrepoFile", "Choose ARM dictionary data repository file", "File:", "Choose ARM dictionary data repository file",
					DIALOG_FILE, SWT.OPEN, new String[]{"*.exd;*.sdai"}, INVALID_ARM_FILE_MESSAGE);
		sourceFileGroupMim =
			new FileSelectGroup(mainComposite, "MIMrepoFile", "Choose MIM dictionary data repository file", "File:", "Choose MIM dictionary data repository file",
					DIALOG_FILE, SWT.OPEN, new String[]{"*.exd;*.sdai"}, INVALID_MIM_FILE_MESSAGE);
        sourceFileGroupCss.createGroupControls();
        sourceFileGroupArm.createGroupControls();
        sourceFileGroupMim.createGroupControls();

		destDirectoryGroup =
			new FileSelectGroup(mainComposite, "coarmComimDir", "Select directory for generated conformance tables", "Directory:",
					"Select directory for generated conformance tables", DIALOG_DIRECTORY, SWT.SAVE,
					null, INVALID_DESTINATION_DIRECTORY_MESSAGE);
		destDirectoryGroup.createGroupControls();


// <XML> perhaps some option checkbox will be useful, but trying to suppress for now
/*

		Group optionGroup = new Group(mainComposite, SWT.NONE);
        optionGroup.setText("XML Index Options");
        ...
        generateJavaDocPart.setText("IGNORE FOR NOW");
...
*/
	...
	public void setSourceFileNameCss(String name) {
		try {
			sourceFileGroupCss.setFileName(name);
	...
	public void setSourceFileNameArm(String name) {
			sourceFileGroupArm.setFileName(name);
	}
	public void setSourceFileNameMim(String name) {
			sourceFileGroupMim.setFileName(name);
	}

	public String getSourceFileNameCss() {
		try {
			File selectedFile = sourceFileGroupCss.getSelectedFile();
			return selectedFile != null ? selectedFile.getCanonicalPath() : null;
		} catch (IOException e) {
			throw (IllegalArgumentException)new IllegalArgumentException(e.getMessage()).initCause(e);
		}
	}
	public String getSourceFileNameArm() {
		try {
			File selectedFile = sourceFileGroupArm.getSelectedFile();
			return selectedFile != null ? selectedFile.getCanonicalPath() : null;
		} catch (IOException e) {
			throw (IllegalArgumentException)new IllegalArgumentException(e.getMessage()).initCause(e);
		}
	}
	public String getSourceFileNameMim() {
		try {
			File selectedFile = sourceFileGroupMim.getSelectedFile();
			return selectedFile != null ? selectedFile.getCanonicalPath() : null;
		} catch (IOException e) {
			throw (IllegalArgumentException)new IllegalArgumentException(e.getMessage()).initCause(e);
		}
	...
	public boolean isGenerateJavaDocPart() {
		return false;
		//<XML> return generateJavaDocPart.getSelection();
	}
	...
	public void restoreWidgetValues() {
		try {
			...
			if (settings != null) {
				sourceFileGroupCss.restoreSettings(settings);
				sourceFileGroupArm.restoreSettings(settings);
				sourceFileGroupMim.restoreSettings(settings);
				destDirectoryGroup.restoreSettings(settings);
				if(destDirectoryGroup.getSelectedFile() == null) {
					setInitialDestDirectory();
				}
				// <XML>generateJavaDocPart.setSelection(settings.getBoolean(STORE_JAVA_OPTION_ID));
				updateWidgetEnablements();
			}
		} catch (IOException e) {
			throw (IllegalArgumentException)new IllegalArgumentException(e.getMessage()).initCause(e);
	...
	public void saveWidgetValues() {
		IDialogSettings settings = getDialogSettings();
		if (settings != null) {
			sourceFileGroupCss.saveSettings(settings);
			sourceFileGroupArm.saveSettings(settings);
			sourceFileGroupMim.saveSettings(settings);
			destDirectoryGroup.saveSettings(settings);
			//<XML> settings.put(STORE_JAVA_OPTION_ID, generateJavaDocPart.getSelection());
		}
	}
	...
	protected boolean determinePageCompletion() {
		boolean complete =
			sourceFileGroupCss.validateSourceGroup() && 
			sourceFileGroupArm.validateSourceGroup() &&
			sourceFileGroupMim.validateSourceGroup() && 
			destDirectoryGroup.validateSourceGroup();
	...
	private void fileSelectGroupChanged(FileSelectGroup fileSelectGroup) throws IOException {
		if(fileSelectGroup == sourceFileGroupCss) {
			setInitialDestDirectory();
		}
	}

	private void setInitialDestDirectory() throws IOException {
		File sourceFile = sourceFileGroupCss.getSelectedFile();
		if(sourceFile != null) {
			if(sourceFile.isFile()) {
				sourceFile = sourceFile.getParentFile();
			}
			File destDir = new File(sourceFile, "xml");
			if(!destDir.exists()) {
				destDir = sourceFile;
			}
